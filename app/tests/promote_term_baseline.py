...
(tmp_verified_term_audit_variants). It regenerates the full variant list
...
def _repo_relative_path(path: Path) -> Path:
    return path.resolve().relative_to(APP_DIR)
...
def _generate_fresh_variants() -> list[dict]:
    ...
    from tests.run_verified_term_audit import build_variants, _variant_row
...
def _get_unique_coverage_key_count() -> int:
    ...
    return len(_registry_coverage_keys())
...
def _coverage_key(v: dict) -> tuple[str, str, str, str, str, str]:
    return (
        str(v.get("language") or "sv"),
        str(v.get("market") or "SE"),
...
def _registry_coverage_keys() -> set[tuple[str, str, str, str, str, str]]:
...
def promote(
    *,
    ...
    if new_variants:
        registry_coverage_keys = _registry_coverage_keys()
        uncovered = [
            v for v in new_variants
            if _coverage_key(v) not in registry_coverage_keys
        ]
        if uncovered:
    ...
    if not new_variants and not hash_migrations:
        ...
        target = _write_json(BASELINE_PATH, baseline, output_dir=output_dir)
        _write_output_manifest(
            output_dir=output_dir,
            changed_files=[(BASELINE_PATH, target)],
    ...
    changed_files: list[tuple[Path, Path]] = []
    changed_files.append((BASELINE_PATH, _write_json(BASELINE_PATH, baseline, output_dir=output_dir)))

    # --- Update the frozen verified-term variant count in contract checks ---
    target = _update_py_constant(
        CONTRACT_CHECKS_PATH,
        "EXPECTED_VERIFIED_TERM_VARIANT_COUNT",
        new_count,
        output_dir=output_dir,
    )
    if target:
        changed_files.append((CONTRACT_CHECKS_PATH, target))
        print(f"Updated EXPECTED_VERIFIED_TERM_VARIANT_COUNT → {new_count}")
    else:
        print(f"WARNING: could not update EXPECTED_VERIFIED_TERM_VARIANT_COUNT in {CONTRACT_CHECKS_PATH.name}")
    ...
    target = _update_py_constant(
        ADD_TERM_CHECKS_PATH,
        "EXPECTED_VERIFIED_TERM_UNIQUE_COVERAGE_KEYS",
        new_unique_key_count,
        output_dir=output_dir,
    )
    if target:
        changed_files.append((ADD_TERM_CHECKS_PATH, target))
        print(f"Updated EXPECTED_VERIFIED_TERM_UNIQUE_COVERAGE_KEYS → {new_unique_key_count}")
    else:
        print(f"WARNING: could not update EXPECTED_VERIFIED_TERM_UNIQUE_COVERAGE_KEYS in {ADD_TERM_CHECKS_PATH.name}")
    ...
    if target:
        ...
        if n:
            target = _write_text(SANITY_CHECKS_PATH, new_text, output_dir=output_dir)
            changed_files.append((SANITY_CHECKS_PATH, target))
        ...
        print(f"Skipped sanity-check constant update; {SANITY_CHECKS_PATH.name} is not present.")
