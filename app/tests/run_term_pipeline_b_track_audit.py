#!/usr/bin/env python3
"""Build the Track B term-pipeline audit ledger.

This is the B0/B1 initializer. It does not rebuild cache and does not decide
whether terms are semantically correct. It rebuilds a deterministic, persistent
dev-only working table that later B waves can update as variants are audited.
"""
...
from tests.matcher_layer_diagnostics import diagnose_case  # noqa: E402
...
class AuditVariant:
    ...
    def with_identity(self, *, row_index: int, batch_size: int) -> "AuditVariant":
        ...
        return AuditVariant(
            **{
                **asdict(self),
                "variant_id": f"btrack-{digest[:16]}",
                "batch_id": f"B{((row_index - 1) // batch_size) + 1:03d}",
...
def write_batch_audit_report(report: dict[str, Any], report_dir: Path) -> tuple[Path, Path]:
    ...
    md_path.write_text(
        "\n".join([
            f"# B-Track Static Audit {report['batch_id']}",
            "",
...
def write_reports(variants: list[AuditVariant], report_dir: Path, *, batch_size: int) -> tuple[Path, Path]:
    ...
    md_path.write_text(
        "\n".join([
            ...
            "## Notes",
            "",
            "- B0 only initializes the ledger; all variant statuses start as `pending`.",
            "- This report is generated/local and should not be committed unless explicitly promoted.",
            "- Drop the working table after the frozen B-track baseline is complete.",
            "",
        ]),
        encoding="utf-8",
    )
...
def main() -> int:
    ...
    parser.add_argument("--audit-batch", help="Run static contract checks for one generated batch id, e.g. B001.")
