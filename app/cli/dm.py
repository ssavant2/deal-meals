...
@dataclass(frozen=True)
class KeywordExtraParentChange:
    canonical: str
    kids: tuple[str, ...]
...
def _append_keyword_extra_parent_entries(
    *,
    ...
    paths.keyword_extra_parent_file.write_text(existing_text + separator + append_text, encoding="utf-8")
    return tuple(entry_ids), start_line, append_text
...
def _append_fixtures(
    *,
    ...
    if not dry_run:
        _write_json_list(paths.fixture_file, fixtures)
...
def _append_inventory(
    *,
    ...
    if not dry_run:
        _write_json_list(paths.inventory_file, inventory)
...
def _append_deep_sanity_stub(
    *,
    ...
    block = "\n".join(lines) + "\n"
    if not dry_run:
        existing_text = paths.deep_sanity_file.read_text(encoding="utf-8")
        paths.deep_sanity_file.write_text(existing_text.rstrip() + "\n" + block, encoding="utf-8")
    return block
...
def _write_runtime_delta_entries(paths: MatcherPaths, toml_text: str) -> Path:
    runtime_entries_dir = paths.repo_root / ".dm_matcher_runtime_entries"
    runtime_entries_dir.mkdir(parents=True, exist_ok=True)
    (runtime_entries_dir / "keyword_extra_parent.toml").write_text(toml_text, encoding="utf-8")
...
def add_keyword_extra_parent(
    ...
    change = KeywordExtraParentChange(
        canonical=canonical,
        kids=kids,
        offer_names=offer_names,
        recipe_name=recipe_name,
    ...
    runtime_entries_dir = (
        _write_runtime_delta_entries(paths, toml_preview)
        if paths.app_dir != APP_DIR
        else None
    )
    gate_status = _run_track_b_gates(
        paths=paths,
        policy_ref=change.policy_ref,
        first_fixture_id=change.fixture_ids[0],
