#!/usr/bin/env python3
"""Contract and coverage checks for the term registry.
...
EXPECTED_VERIFIED_TERM_VARIANT_COUNT = 5492  # updated by support_checks/promote_term_baseline.py
    ...
    *,
    ...
    if not path.exists():
        return {}, set(), set(), [_issue(
            ...
            "verified-term baseline JSON is required before registry contract checks can run",
            item_id=str(path),
            details={
                "hint": (
                    "Recover or regenerate app/languages/sv/ingredient_matching/"
                    "term_registry/baselines/verified_matcher_terms.json "
                    "before running registry contract checks."
                )
            },
        )]
...
def _coverage_exception_path(language: str) -> Path:
    if language != "sv":
        raise ValueError("term registry coverage exceptions currently support --language sv only")
    return (
        APP_DIR
        / "languages"
        / "sv"
        / "ingredient_matching"
        / "term_registry"
        / "coverage_exceptions.toml"
    )


def _load_coverage_exceptions(path: Path, *, language: str, market: str) -> tuple[set[CoverageKey], list[CheckIssue], list[dict[str, Any]]]:
    issues: list[CheckIssue] = []
    if not path.exists():
        return set(), [_issue(
            "error",
            "coverage_exceptions_file_missing",
            "term registry coverage checks require an explicit coverage_exceptions.toml file",
            item_id=str(path),
        )], []
    ...
    if not isinstance(raw_exceptions, list):
        return set(), [_issue(
            "error",
            "coverage_exceptions_payload_invalid",
            "coverage exceptions must be an array of tables",
            item_id=str(path),
        )], []
    ...
    for index, raw_item in enumerate(raw_exceptions):
        if not isinstance(raw_item, dict):
            issues.append(_issue(
                "error",
                "coverage_exception_row_invalid",
                "coverage exception rows must be tables",
                item_id=str(path),
                details={"index": index},
            ))
            continue
        ...
        if missing:
            issues.append(_issue(
                "error",
                "coverage_exception_missing_fields",
                "coverage exceptions require exact ownership, coverage, reason, and expiry fields",
                item_id=item_id,
        ...
        if exception_id in seen_ids:
            issues.append(_issue(
                "error",
                "coverage_exception_duplicate_id",
                "coverage exception id is duplicated",
                item_id=item_id,
            ))
        ...
        if key is None:
            issues.append(_issue(
                "error",
                "coverage_exception_coverage_key_incomplete",
                "coverage exception coverage key is incomplete",
                item_id=item_id,
            ))
            continue
        if any(_has_broad_coverage_value(value) for value in key[2:]):
            issues.append(_issue(
                "error",
                "coverage_exception_too_broad",
                "coverage exceptions must cover exactly one source family/variant/layer",
                item_id=item_id,
        ...
        if expires_on:
            if isinstance(expires_on, date):
                ...
                try:
                    ...
                    issues.append(_issue(
                        "error",
                        "coverage_exception_invalid_expires_on",
            ...
            if expires_on_date < today:
                issues.append(_issue(
                    "error",
                    "coverage_exception_expired",
                    "coverage exception has passed its expires_on date",
                    item_id=item_id,
...
def _run_new_term_gate(
    *,
    ...
    coverage_exceptions: list[dict[str, Any]],
) -> tuple[dict[str, Any], list[CheckIssue]]:
    issues: list[CheckIssue] = []
    ...
    if uncovered:
        ...
        issues.append(_issue(
            "error",
            "new_source_term_missing_registry",
            "new source-derived matcher terms must be added to TOML registry coverage or coverage_exceptions.toml",
            details={
                "count": len(uncovered),
                "sample": sample,
                "hint": (
    ...
    probe_key: CoverageKey = (
        probe_language,
        probe_market,
        "coverage_gate_failure_probe",
        "coverage_gate_failure_probe",
        "coverage gate failure probe",
        "coverage_gate_failure_probe_layer",
    )
    probe_uncovered = (
    ...
    if not failure_probe_passed:
        issues.append(_issue(
            ...
            "coverage gate failure probe did not detect an uncovered synthetic source key",
    ...
    if stale_registry_keys:
        issues.append(_issue(
            "error",
            "registry_coverage_key_not_in_current_sources",
            "active registry coverage claims source keys that do not exist in current matcher sources",
            details={
    ...
    for key in sorted(exception_coverage_keys - current_keys):
        issues.append(_issue(
            "error",
            "coverage_exception_stale",
            "coverage exception covers a source key that no longer exists",
    ...
    for key in sorted(exception_coverage_keys & baseline_coverage_keys):
        issues.append(_issue(
            "error",
            "coverage_exception_covers_verified_terms_baseline",
            "coverage exception is unnecessary because this key is already in the verified-term baseline",
            item_id=exception_ids_by_key.get(key, ""),
            details={"coverage_key": _coverage_key_to_dict(key)},
        ))
    for key in sorted(exception_coverage_keys & registry_coverage_keys):
        issues.append(_issue(
            "error",
            "coverage_exception_covers_registry_key",
            "coverage exception is stale because an active registry entry covers the same key",
            item_id=exception_ids_by_key.get(key, ""),
            details={"coverage_key": _coverage_key_to_dict(key)},
        ))
    ...
    summary = {
        "enabled": True,
        "current_source_coverage_keys": len(current_keys),
        "verified_term_baseline_coverage_keys": len(baseline_coverage_keys & current_keys),
        "new_source_coverage_keys": len(new_keys),
        "new_keys_covered_by_registry": len(covered_by_registry),
        "new_keys_covered_by_coverage_exception": len(covered_by_exception),
        "new_keys_uncovered": len(uncovered),
        "active_registry_coverage_keys": len(registry_coverage_keys),
        "coverage_exception_coverage_keys": len(exception_coverage_keys),
        "failure_probe_passed": failure_probe_passed,
        "failure_probe_error_code": "new_source_term_missing_registry",
    }
...
def run_checks(args: argparse.Namespace) -> tuple[dict[str, Any], list[CheckIssue]]:
    ...
    exception_path = _coverage_exception_path(args.language)
    exception_coverage_keys, exception_issues, coverage_exceptions = _load_coverage_exceptions(
        exception_path,
        language=args.language,
        market=args.market,
    )
    ...
    summary = {
        ...
        "source_inventory_view": summarize_variants(variants),
        ...
        "coverage_exceptions_file": str(exception_path.relative_to(REPO_DIR)),
