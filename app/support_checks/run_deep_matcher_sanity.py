...
# Behavioral guard: "Vårgårda Ris" is AXA plain dry rice. The brand word
# vårgårda used to provide an independent match path that bypassed the
# puffat-ris carrier-context suppression, so a puffed-rice cereal recipe
# wrongly matched ordinary rice. With vårgårda as a stop word the puffed-rice
# recipe extracts only ris (suppressed by puffat) and the product is blocked,
# while a plain rice recipe still matches the product via ris.
test("Vårgårda Ris blocked for puffed-rice recipe",
     bool(legacy_match("Vårgårda Ris 250g AXA", "vårgårda puffat rostat ris")), False)
test("Vårgårda Ris still matches plain ris recipe",
     bool(legacy_match("Vårgårda Ris 250g AXA", "2 dl ris")), True)
