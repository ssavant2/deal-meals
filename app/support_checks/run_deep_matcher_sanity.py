...
test("english is stop word", "english" in STOP_WORDS, True)  # "Marmelad Old English" should not match "Te English Breakfast" recipe
...
test("svamp blocked by kombuchasvamp", blocked("svamp", "200 ml kombuchasvamp"), True)
...
# Bouillon meat-type compatibility (Q146-1): nöt = kött = oxe are interchangeable
# meat broths, but a meat broth stays isolated from vegetable/poultry/fish stock,
# and a generic "buljong" still defaults to the vegetable variant only.
test("nötbuljong matches köttbuljong", recipe_match_num(["3 nötbuljongtärningar"], {"name": "Köttbuljong Tärningar Kockens", "category": "pantry"}), 1)
test("nötbuljong matches oxbuljong", recipe_match_num(["2 nötbuljongtärningar"], {"name": "Oxbuljong Kockens 1l", "category": "pantry"}), 1)
test("nötbuljong matches Buljong Mörk Oxe", recipe_match_num(["3 nötbuljongtärningar"], {"name": "Buljong Mörk Oxe Kockens", "category": "pantry"}), 1)
test("nötbuljong blocked from kycklingbuljong", recipe_match_num(["3 nötbuljongtärningar"], {"name": "Kycklingbuljong Kockens 1l", "category": "pantry"}), 0)
test("nötbuljong blocked from grönsaksbuljong", recipe_match_num(["3 nötbuljongtärningar"], {"name": "Grönsaksbuljong Kockens", "category": "pantry"}), 0)
test("generic buljong still defaults to grönsak", recipe_match_num(["2 buljongtärningar"], {"name": "Grönsaksbuljong Kockens", "category": "pantry"}), 1)
test("generic buljong blocked from kycklingbuljong", recipe_match_num(["2 buljongtärningar"], {"name": "Kycklingbuljong Kockens 1l", "category": "pantry"}), 0)
test("köttbuljong blocked from kycklingbuljong", recipe_match_num(["2 köttbuljongtärningar"], {"name": "Kycklingbuljong Kockens 1l", "category": "pantry"}), 0)
test("lantbuljong matches grönsaksbuljong (lant≡grönsak per QUALIFIER_EQUIVALENTS)", recipe_match_num(["1 tsk lantbuljong"], {"name": "Grönsaks Buljong Tärningar Garant Eko", "category": "pantry"}), 1)
test("lantbuljong blocked from köttbuljong", recipe_match_num(["1 tsk lantbuljong"], {"name": "Köttbuljong Kockens", "category": "pantry"}), 0)
...
# identities, while fallback identities stay guarded.
test(
    "Chorizo with smoked paprika flavor still matches chorizo",
    match("Färskkorv Chorizo Kryddad Med Rökt Paprika 280g Köttkultur", "200 g chorizo", "meat"),
    "chorizo",
)
test(
    "Chorizo with smoked paprika flavor backend match stays chorizo",
    recipe_match_num_named(
        "Portugisiska musslor",
        ["200 g chorizo"],
        {"name": "Färskkorv Chorizo Kryddad Med Rökt Paprika 280g Köttkultur", "category": "meat"},
    ),
    1,
)
test(
    "preserved champinjoner still blocked from plain svamp fallback",
    match("Champinjoner Tetra 290g ICA", "200 g svamp", "vegetables"),
    None,
)
test(
    "preserved champinjoner still allowed for preserved svamp fallback",
    match("Champinjoner Tetra 290g ICA", "200 g konserverad svamp", "vegetables"),
    "svamp",
)
...
test("PNB majs → tortillabröd", 'tortillabröd' in PRODUCT_NAME_BLOCKERS.get('majs', set()), True)
...
# Flavored ice cream recipe lines: "glass (blåbär)" is rewritten to the compound
# "blåbärsglass" (in both extraction and the normalized fast-path text) so the
# line routes to its specific flavor and COMPOUND_STRICT['glass'] blocks plain
# vanilla products. Composite/novelty flavors keep the plain-glass default.
test("glass (blåbär) extracts blåbärsglass", extract_keywords_from_ingredient("ca 3 dl glass (blåbär)"), ['blåbärsglass'])
test("glass (jordgubb) extracts jordgubbsglass", extract_keywords_from_ingredient("3 dl glass (jordgubb)"), ['jordgubbsglass'])
test("plain glass still defaults to vaniljglass", "vaniljglass" in extract_keywords_from_ingredient("3 dl glass"), True)
test("glass (blåbär) matches blåbärsglass product", recipe_match_num_cached(["ca 3 dl glass (blåbär)"], {"name": "Blåbärsglass Hemglass 0.5l", "category": "frozen"}), 1)
test("glass (blåbär) blocks plain vanilla ice cream", recipe_match_num_cached(["ca 3 dl glass (blåbär)"], {"name": "Vanilj Gräddglass Sia Glass", "category": "frozen"}), 0)
test("glass (blåbär) blocks composite himmelsk röra glass", recipe_match_num_cached(["ca 3 dl glass (blåbär)"], {"name": "Glass Himmelsk Röra GB", "category": "frozen"}), 0)
...
test("teriyaki woksås product extracts teriyaki", kw("Woksås Teriyaki 120g Blue Dragon"), ["teriyaki", "teriyakiwoksas", "woksås"])
test(
...
test(
    "KW växtbaserad naturell dryck → växtdryck (adjective mid-phrase does not break merge)",
    extract_keywords_from_ingredient("2 dl växtbaserad naturell dryck"),
    ["växtdryck"],
)
test(
...
test(
    # "organiskt"/"organisk" are organic descriptors (a variant of the already
    # stop-worded "ekologisk") and are never a food on their own. They must be
    # dropped so they cannot leak as a keyword.
    "KW organiskt cocosfett drops organic descriptor",
    extract_keywords_from_ingredient("1 tsk organiskt cocosfett"),
    ["cocosfett"],
)
test(
...
test(
    "KW vegetabilisk mjölk → växtdryck",
    extract_keywords_from_ingredient("500 g Vegetabilisk mjölk"),
    ["växtdryck"],
)
test(
    "MAT vegetabilisk mjölk matches havredryck",
    match_kw("Havredryck Naturell 1l ICA", "500 g Vegetabilisk mjölk", "dairy"),
    "växtdryck",
)
test(
    "MAT vegetabilisk mjölk blocks dairy mjölk",
    match_kw("Mellanmjölk 1,5% 1l Arla", "500 g Vegetabilisk mjölk", "dairy"),
    None,
)
test(
...
test(
    "MAT babyspenat blocks bladspenat fallback (Q65-2 isolation)",
...
test(
    "MAT babyspenat blocks generic fresh spenat pack (Q65-2 isolation)",
    match_kw("Spenat i storpack Sköljd 200g ICA", "200 g babyspenat"),
    None,
)
test(
    "MAT babyspenat blocks hackad fryst spenat fallback (Q65-2 isolation)",
...
test(
    "Spice msk heuristic: 1 msk gurkmeja matches Gurkmeja Malen (tablespoon = dry spice amount)",
    match("Gurkmeja Malen 43g ICA", "1 msk gurkmeja", "spices"),
    "gurkmeja",
)
test(
    "Spice dry-marker heuristic: torkad gurkmeja matches Gurkmeja Malen (explicit dry marker)",
    match("Gurkmeja Malen 43g ICA", "torkad gurkmeja", "spices"),
    "gurkmeja",
)
test(
    "Spice msk heuristic: 1 msk ingefära matches Ingefära Malen (tablespoon = dry spice amount)",
    match("Ingefära Malen 30g Santa Maria", "1 msk ingefära", "spices"),
    "ingefära",
)
test(
...
test(
    # Q86-2 update (2026-05-25): Stefan extended Q79-10 (färsk vs fryst OK)
    # to include sojabönor konserv↔fryst. Frozen soybeans require boiling
    # anyway — practically identical use case as canned. Expectation flipped 0→1.
    "Q86-2 sojabönor konserv matches frozen soybeans (extended Q79-10 substitution)",
    recipe_match_num(
        ["sojabönor konserv"],
...
test(
    "fresh chili product with Swedish adjective-first name ('Röd Chili 50g') matches tsk-measured chilifrukt ingredient",
    recipe_match_num(
        ["1 tsk chilifrukt, finhackad"],
        {"name": "Röd Chili 50g", "category": "vegetables"},
    ),
    1,
)
test(
    "fresh chili product with Swedish adjective-first name ('Grön Chili 100g') matches hackad chilifrukt ingredient",
    recipe_match_num(
        ["hackad chilifrukt"],
        {"name": "Grön Chili 100g", "category": "vegetables"},
    ),
    1,
)
# Plural/definite color form "röda chili" is the fresh red pepper, same as singular
# "röd chili". It must match the fresh red chili product and the color-isolation /
# fresh-vs-dried rules must still hold (no green, no powder/flakes).
test(
    "plural 'röda chili i tunna skivor' matches fresh red chili product",
    recipe_match_num(
        ["röda chili, i tunna skivor"],
        {"name": "Chili Röd 50g", "category": "vegetables"},
    ),
    1,
)
test(
    "plural 'röda chili' does not match green chili (color isolation)",
    recipe_match_num(
        ["röda chili"],
        {"name": "Chili Grön 50g", "category": "vegetables"},
    ),
    0,
)
test(
    "plural 'röda chili' does not match chili flakes (fresh-only)",
    recipe_match_num(
        ["röda chili"],
        {"name": "Chiliflakes 30g Santa Maria", "category": "pantry"},
    ),
    0,
)
test(
...
# "gul curry" in Swedish home cooking is the yellow curry-powder spice, not Thai
# yellow curry paste. It must match the curry spice jar and must NOT match concentrated
# Thai currypaste; "gul currypasta" (the paste) keeps matching the paste product.
test(
    "gul curry routes to plain curry spice keyword",
    match("Curry 34g Santa Maria", "2 tsk gul curry", "spices"),
    "curry",
)
test(
    "gul curry does not match Thai yellow currypaste",
    recipe_match_num(
        ["2 tsk gul curry"],
        {"name": "Currypasta Gul Thai 400g Mae Ploy", "category": "pantry"},
    ),
    0,
)
test(
    "gul currypasta still matches Thai yellow currypaste",
    recipe_match_num(
...
test(
    "gulcurry compound form routes to plain curry spice, not Thai paste",
    recipe_match_num(
        ["Gulcurry"],
        {"name": "Currypasta Gul Thai 400g Mae Ploy", "category": "pantry"},
    ),
...
test(
    "gulcurry compound form matches curry powder jar",
    match("Curry 34g Santa Maria", "Gulcurry", "spices"),
    "curry",
)
test(
...
test("PNB hasselnöt does NOT block start (Start is a nut brand, not cereal)",
     'start' in PRODUCT_NAME_BLOCKERS.get('hasselnöt', set()), False)
test("Hasselnöt 750g Start matches hackad hasselnöt after removing false brand block",
     recipe_match_num(["hackad hasselnöt"], {"name": "Hasselnöt 750g Start", "category": "pantry"}), 1)
...
# "Ölkorv Chili" is a Swedish beer-flavored sausage with chili seasoning,
# not chili pepper itself — should not appear under recipes wanting chili.
test("PNB chili → ölkorv", 'ölkorv' in PRODUCT_NAME_BLOCKERS.get('chili', set()), True)
test(
...
test("Grana Padano matches parmesanost ingredient (padano→parmigiano ingredient-parent)", recipe_match_num(["50 g parmesanost"], {"name": "Grana Padano 12 Månader Garant", "category": "dairy"}), 1)
test("Grana Padano matches riven parmesan ingredient", recipe_match_num(["2 msk riven parmesan"], {"name": "Grana Padano 12 Månader Garant", "category": "dairy"}), 1)
...
test(
    ...
    extract_keywords_from_product("Smör & Rapsolja Flytande 500ml Valio", "pantry"),
...
test(
    ...
    recipe_match_num(
        ["2 msk Flytande Smör"],
        {"name": "Smör & Rapsolja Flytande 500ml Valio", "category": "pantry"},
    ),
    1,
)
test(
...
test(
    "generic ost does NOT match riven ost gratäng (recipe wants gratängost specifically)",
    recipe_match_num(
        ["150 g riven ost gratäng"],
...
test(
    "generic ost cached does NOT match riven ost gratäng",
    recipe_match_num_cached(
        ["150 g riven ost gratäng"],
        {"name": "Hushallsost 26% Arla", "category": "dairy"},
    ),
...
test(
    "plain salsiccia matches plant-based salsiccia (substitute policy)",
    recipe_match_num(
...
test(
    "skogschampinjon singular blocks preserved sliced champignons",
    recipe_match_num(
        ["250 g skogschampinjon, skivad"],
        {"name": "Champinjoner Skivade 290g ICA", "category": "vegetables", "savings": 10},
    ),
    0,
)
test(
    "skogschampinjon singular still matches fresh champignons",
    recipe_match_num(
        ["250 g skogschampinjon, skivad"],
        {"name": "Champinjoner 250g Klass 1 ICA", "category": "vegetables", "savings": 10},
    ),
...
test(
    "skogschampinjon singular also accepts frozen champignons",
    recipe_match_num(
        ["250 g skogschampinjon, skivad"],
        {"name": "Champinjoner Frysta 250g ICA", "category": "frozen", "savings": 10},
    ),
...
test(
    "linguine ingredient does NOT match short pasta (long vs short shape)",
    recipe_match_num(
        ["1 förp pasta linguine"],
...
section("batch 145 q8 regressions")
test(
    "batch 145 porter ingredient extracts porter and öl via eller-split",
...
test("Root salsa carrier pineapple-vodka blocked from plain salsa (PNB pineapple)", recipe_match_num_cached(["2 dl salsa"], {"name": "Salsa Pineapple Vodka El Taco Truck", "category": "fruit", "brand": "EL TACO TRUCK"}), 0)
...
test("senap dijonsenap blocks sweet mustard (dijon qualifier requires dijon product)", recipe_match_num(["2 msk fransk senap"], {"name": "Senap Sötstark Johnny's", "category": "pantry"}), 0)
test("senap dijonsenap blocks original mustard (dijon qualifier requires dijon product)", recipe_match_num_cached(["2 msk fransk senap"], {"name": "Original Senap Slotts", "category": "pantry"}), 0)
...
test("Batch 14 rimmad skivad lax blocks gravad skivad lax", recipe_match_num_cached(["ca 250 g rimmad lax i tunna skivor"], {"name": "Gravad Lax Skivad Falkenberg", "category": "fish"}), 0)  # Q92-8: KSBC lax←rimmad overrides old Batch 14 accept
test("Batch 14 rimmad skivad lax blocks kallrökt skivad lax", recipe_match_num(["ca 250 g rimmad lax i tunna skivor"], {"name": "Kallrökt Lax Skivad Falkenberg", "category": "fish"}), 0)  # Q92-8
test("Batch 14 rimmad skivad lax blocks najadlax", recipe_match_num_cached(["ca 250 g rimmad lax i tunna skivor"], {"name": "Najadlax Skivad Leröy", "category": "fish"}), 0)  # Q92-8
...
test("Step 0 plain majs blocks tortilla bread carrier", recipe_match_num(["200 g majs"], {"name": "Tortillabröd Majs", "category": "bread"}), 0)
test("Step 0 plain majs still accepts loose corn", recipe_match_num_cached(["200 g majs"], {"name": "Majs Fryst Findus", "category": "frozen"}), 1)
test("Step 0 plain wokmix blocks Thai Style", recipe_match_num(["400 g wokmix"], {"name": "Wokmix Thai Style Findus 750g", "category": "frozen", "brand": "Findus"}), 0)
...
# Ground-spice adjective forms are the same word: a recipe asking for "Nejlikor Malda"
# (plural, parent keyword) must match a "Nejlika Malen" product even though the product
# stores its qualifier under the singular child keyword. malda↔malen↔mald↔malet are
# equivalent via QUALIFIER_EQUIVALENTS; Direction A rolls the child qualifier up to the
# matched parent so the equivalence is actually consulted.
test("Ground cloves malda matches malen product across plural parent keyword", recipe_match_num(["1 tsk Nejlikor Malda"], {"name": "Krydda Nejlika Malen påse 20g Santa Maria", "category": "pantry"}), 1)
...
test("senap isolering: grovkornig senap matchar INTE skånsk senap-produkt (skånsk är self-only)", recipe_match_num(["1 msk grovkornig senap"], {"name": "Skånsk Senap Slotts", "category": "pantry"}), 0)
...
# Q139-1: "puffat rostat ris" (adjective between puffat and ris) still yields rispuffar
test("puffat rostat ris ingredient extracts rispuffar",
     extract_keywords_from_ingredient("3 dl puffat rostat ris"), ["rispuffar"])
test("Q139-1 puffat rostat ris matches Vårgårda Ris AXA",
     recipe_match_num(["3 dl puffat rostat ris"], {"name": "Vårgårda Ris 250g AXA", "category": "pantry"}), 1)
test("Q139-1 puffat rostat ris matches Rice Krispies",
     recipe_match_num(["3 dl puffat rostat ris"], {"name": "Rice Krispies 360g Kelloggs", "category": "pantry"}), 1)
...
test("Track A bulk nougat ingredient matches Nougat Frondi", recipe_match_num(["250 g nougat"], {"name": "Nougat Frondi 250g Kras", "category": "candy"}), 1)
test("Track A bulk nougat ingredient does not match nougat ice cream", recipe_match_num(["250 g nougat"], {"name": "Nougat Gräddglass 1L", "category": "frozen"}), 0)
...
test("isbergssallad normalizes to isbergssallat canonical", match("Isbergssallad 1-p Klass 1", "isbergssallad", "fruit"), "isbergssallat")
...
test("isbergssallatshuvud matches isbergssallad product via canonical",
     match("Isbergssallad 1-p Klass 1", "1 isbergssallatshuvud", "vegetables"), "isbergssallat")
...
# Curry cuisine variants on kryddmix — distinct cuisines block each other
test("paneng kryddmix recipe does NOT match Massaman kryddmix",
     recipe_match_num(["kryddmix"], {"name": "Kryddmix Thai Massaman curry Medium 400ml Mrs Cheng's", "category": "pantry"}), 0)
# Pak choy (UK spelling) normalises to pakchoy, then keyword_synonym rolls to pakchoi
test("pak choy recipe matches Pak Choi product",
     match("Pak Choi 250g klass 1 ICA", "2 huvuden pak choy", "vegetables"), "pakchoi")
# FPB mango += mangocurry — curry spice ingredient should not match fresh mango fruit
test("mangocurry recipe does NOT match plain Mango fruit",
     match("Mango ca 325g Klass 1", "40 g mangocurry", "pantry"), None)
# Cuisine-curry grytbas blockers — paneng recipe should NOT match massaman grytbas
test("paneng grytbas recipe does NOT match massaman grytbas",
     recipe_match_num(["grytbas"], {"name": "Grytbas Thai Massaman curry Medium 400ml Mrs Cheng's", "category": "pantry"}), 0)
# Opastöriserad ingredient should not match pastöriserad products via substring
test("opastöriserad recipe does NOT match Pastöriserad äggvita",
     match("Äggvita Pastöriserad flytande 1kg Kronägg", "1 dl Zeta Pesto opastöriserad", "dairy"), None)
# Bruschetta/Crostini carrier suppresses plain vitlök/persilja keywords
test("Bruschetta recipe does NOT match plain vitlök",
     match("Vitlök 250g Klass 1 ICA", "1 frp Zeta Bruschetta Vitlök & Persilja", "pantry"), None)
test("Bruschetta recipe does NOT match plain persilja",
     match("Persilja 8g Santa Maria", "1 frp Zeta Bruschetta Vitlök & Persilja", "pantry"), None)
# Maretti is a snack-chip brand; their "Bruschetta" line is bread chips/crackers,
# not the tomato-bread topping/spread that the recipe wants
test("bruschetta recipe does NOT match Maretti snack chips",
     recipe_match_num(["1 frp Zeta Bruschetta Naturell"], {"name": "Bruschetta sourcream & onion 150g Maretti", "category": "pantry"}), 0)
test("bruschetta recipe still matches plain Zeta Bruschetta",
     recipe_match_num(["1 frp Zeta Bruschetta Naturell"], {"name": "Bruschetta Naturell 175g Zeta", "category": "pantry"}), 1)
# "ostig smak" / "ostigt resultat" is an adjective describing cheesy flavor,
# not a cheese ingredient (e.g. nutritional yeast for cheesy taste)
test("ostig smak does NOT match plain ost product",
     match("Herrgårdsost 18m 28% 400g Allerum",
...
# Note: original 'spadet' context-word replaced with 'kikärtsspad' to avoid
# false substring match on 'bönspadet' (cooking instruction residual, not brine).
test("kikärtor blocked when ingredient asks for aquafaba",
     match("Zeta Kikärtor Ekologiska 400g", "aquafaba (kikärtsspad)", "pantry"), None)
test("kikärtor blocked when ingredient asks for kikärtsspad",
     match("Kikärtor ICA Basic 800g", "2 dl kikärtsspad", "pantry"), None)
test("kikärtor still matches plain chickpea recipe",
     match("Zeta Kikärtor Ekologiska 400g", "400g kikärtor", "pantry"), "kikärtor")
test("kikärtor matches chickpea recipe with bönspadet cooking instruction",
     match("Kikärtor 380g ICA", "1 burk Zeta Kikärtor, plus lite av bönspadet", "pantry"), "kikärtor")
...
# PNB nuggets: plant-based vego nuggets are NOT blocked (substitute policy — vego
# nuggets may match a plain/chicken nuggets recipe). fish/couscous distinctions stay.
test("nuggets PNB does not block vegan", 'vegan' in PRODUCT_NAME_BLOCKERS.get('nuggets', set()), False)
test("nuggets PNB does not block quorn", 'quorn' in PRODUCT_NAME_BLOCKERS.get('nuggets', set()), False)
test("nuggets PNB still has couscous", 'couscous' in PRODUCT_NAME_BLOCKERS.get('nuggets', set()), True)
test("nuggets PNB still has fisk", 'fisk' in PRODUCT_NAME_BLOCKERS.get('nuggets', set()), True)
...
test("Thaikryddad blocked in italian recipe containing 'padano' — 'pad' must not substring-match 'padano'",
     recipe_match_num_named("One pot chicken Alfredo", ["400 g kycklingfilé", "grana padano", "pasta"], THAI_OFFER), 0)
...
# generic_oil_no_match exemption: ingredient "tonfisk i olja" has \bolja\b but 'olja' is the
# preservation medium, not an oil ingredient — tonfisk in allowed_specifics bypasses the policy.
test("tonfisk i olja product matches tonfisk-i-olja ingredient (generic_oil_no_match exemption)",
     recipe_match_num_named("Tonfisk i olja", ["4 st tonfisk i olja"], _fish_offer("Tonfisk i olja 170g ICA")), 1)
test("generic oil policy still blocks oil product from plain oil ingredient (regression)",
     recipe_match_num_named("Sallad", ["2 dl olja"], {"name": "Olivolja Extra Virgin 500ml", "category": "grocery"}), 0)
...
test("färgmixade småtomater are not treated as processed tomato mix",
     precompute_offer_data("Färgmixade småtomater 350g Klass 1 ICA", "fruit")["processed_checks"], [])
test("färgmixade småtomater fast-match explicit småtomater",
     match("Färgmixade småtomater 350g Klass 1 ICA", "200 g småtomater", "fruit"), "småtomat")
test("färgmixade småtomater backend-match explicit småtomater",
     recipe_match_num_named("Tomatsallad", ["200 g småtomater"], {"name": "Färgmixade småtomater 350g Klass 1 ICA", "category": "fruit"}), 1)
test("KW polpa di pomodoro extracts tomato",
     extract_keywords_from_product("Krossade Tomater Polpa di Pomodoro 3-p 1200g Garofalo", category="vegetables"), ["tomat"])
test("KW pomodoro-only vegetable product extracts tomato",
     extract_keywords_from_product("Pomodoro 400g", category="vegetables"), ["tomat"])
test("polpa di pomodoro matches krossade tomater recipe",
     recipe_match_num_named("Tomatsås", ["200 g krossade tomater"], {"name": "Krossade Tomater Polpa di Pomodoro 3-p 1200g Garofalo", "category": "vegetables"}), 1)
...
test("Saltgurka blocked from smörgåsgurka recipe (KSBC: variants isolate via context)",
     match("Saltgurka 720g Felix", "1 msk smörgåsgurka"), None)
...
# BreOliv isolation — premium olive-oil spread should match branded BreOliv
# recipe wording narrowly, while BreOliv products can answer generic
...
test("BreOliv product matches bordsmargarin recipe wording",
     recipe_match_num(["1 dl bordsmargarin"], {"name": "BreOliv Mild 91% 200g Zeta", "category": "dairy"}), 1)
test("BreOliv recipe wording still rejects Bregott",
     recipe_match_num(["1/2 frp Zeta BreOliv"], {"name": "Bregott Normalsaltat 600g", "category": "dairy"}), 0)
test("KW BreOliv Mild 91% 200g Zeta ingredient -> breoliv",
     extract_keywords_from_ingredient("BreOliv Mild 91% 200g Zeta"), ["breoliv"])
test("KW BreOliv Mild 91% 200g Zeta product -> breoliv+bordsmargarin",
     extract_keywords_from_product("BreOliv Mild 91% 200g Zeta", ""), ["breoliv", "bordsmargarin"])

# keyword_synonym_pakchoi_pakchoy: generated by dm matcher add keyword-synonym
test("Keyword synonym pakchoy matches pakchoi",
     match("Pak Choi 250g klass 1 ICA", "pakchoy", "vegetables"), "pakchoi")

# keyword_extra_parent_tomat_family: generated by dm matcher add keyword-extra-parent
test("Tomat recipe matches småtomat",
     match("Cocktailtomater 250g", "400 g små tomater", "vegetables"), "småtomat")

# keyword_extra_parent_smatomat_family: generated by dm matcher add keyword-extra-parent
test("Småtomat recipe matches snacktomater",
     match("Snacktomater 250g", "400 g småtomater", "vegetables"), "småtomat")

# keyword_synonym_balsamvinager_balsamico: generated by dm matcher add keyword-synonym
test("Keyword synonym balsamico matches balsamvinäger",
     match("Balsamico 250ml Zeta", "balsamico", "pantry"), "balsamvinäger")

# offer_extra_keyword_mjol_vetemjol: generated by dm matcher add offer-extra-keyword
test("offer-extra-keyword Vetemjöl matches mjöl",
     match("Vetemjöl", "mjöl", "pantry"), "mjöl")
...
# runtime_pnb_rom_sikrom: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
...
# Ölkorv (Swedish beer-flavored sausage) products carry flavour words like
# 'vitlök' or 'parmesan' but are sausages, not the named ingredient.
test("PNB vitlök has ölkorv",
     "ölkorv" in PRODUCT_NAME_BLOCKERS.get("vitlök", set()), True)
test("PNB parmigiano has ölkorv",
     "ölkorv" in PRODUCT_NAME_BLOCKERS.get("parmigiano", set()), True)
test("PNB parmigiano reggiano has ölkorv",
     "ölkorv" in PRODUCT_NAME_BLOCKERS.get("parmigiano reggiano", set()), True)
# "Ölkorv Cheddar Snack Strings" — Swedish beer-sausage with cheddar flavor;
# extracted-cheddar keyword routes via cheddarost recipe keyword, so the
# ölkorv blocker must also exist on the cheddarost key (parallel to cheddar).
test("PNB cheddarost has ölkorv",
     "ölkorv" in PRODUCT_NAME_BLOCKERS.get("cheddarost", set()), True)
# Compound forms (vitlöksklyftor/vitlöksklyfta) also matched by the
# garlic-flavored beer-sausage products via the compound keyword, so the
# ölkorv blocker must be present on those keys too.
test("PNB vitlöksklyftor has ölkorv",
     "ölkorv" in PRODUCT_NAME_BLOCKERS.get("vitlöksklyftor", set()), True)
test("PNB vitlöksklyfta has ölkorv",
     "ölkorv" in PRODUCT_NAME_BLOCKERS.get("vitlöksklyfta", set()), True)
# "Ölkorv Cheddar Snack Strings" — Swedish beer-sausage product with cheddar
# flavour, not an actual block/sliced cheddar cheese for grating.
test("PNB cheddar has ölkorv",
     "ölkorv" in PRODUCT_NAME_BLOCKERS.get("cheddar", set()), True)

# runtime_fpb_ingefara_balsamico: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB ingefära has balsamico",
     "balsamico" in FALSE_POSITIVE_BLOCKERS.get("ingefära", set()), True)
test("FPB ingefära has balsamicoingefara",
     "balsamicoingefara" in FALSE_POSITIVE_BLOCKERS.get("ingefära", set()), True)
test("FPB ingefära has balsamicoingefära",
     "balsamicoingefära" in FALSE_POSITIVE_BLOCKERS.get("ingefära", set()), True)

# keyword_synonym_isbergssallat_isbergssallad: generated by dm matcher add keyword-synonym
test("Keyword synonym isbergssallad matches isbergssallat",
     match("Isbergssallat ca 440g Klass 1 ICA", "isbergssallad", "vegetables"), "isbergssallat")


# runtime_fpb_bitter_bittermandel: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB bitter has bittermandel",
     "bittermandel" in FALSE_POSITIVE_BLOCKERS.get("bitter", set()), True)

# runtime_pnb_cappuccino_karamell: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB cappuccino has karamell",
     "karamell" in PRODUCT_NAME_BLOCKERS.get("cappuccino", set()), True)
test("PNB cappuccino has kopiko",
     "kopiko" in PRODUCT_NAME_BLOCKERS.get("cappuccino", set()), True)

# runtime_fpb_korsbar_korsbarssylt: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB körsbär has körsbärssylt",
     "körsbärssylt" in FALSE_POSITIVE_BLOCKERS.get("körsbär", set()), True)

# runtime_ksbc_ingefara_kombucha: generated by dm matcher add ksbc
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC ingefära has kombucha",
     "kombucha" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("ingefära", set()), True)

# runtime_pnb_pollock_panerad: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB pollock has panerad",
     "panerad" in PRODUCT_NAME_BLOCKERS.get("pollock", set()), True)
test("PNB pollock has panerade",
     "panerade" in PRODUCT_NAME_BLOCKERS.get("pollock", set()), True)
test("PNB pollock has sprodpanerad",
...
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC fond has andfond",
     "andfond" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("fond", set()), True)

# runtime_important_short_keyword_add_hirs: generated by dm matcher add important-short-keyword
from languages.sv.ingredient_matching.keywords import IMPORTANT_SHORT_KEYWORDS
test("important-short-keyword add hirs",
     "hirs" in IMPORTANT_SHORT_KEYWORDS, True)

# runtime_pnb_philadelphiaost_tomatsopp: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB philadelphiaost has tomatsopp",
     "tomatsopp" in PRODUCT_NAME_BLOCKERS.get("philadelphiaost", set()), True)
test("PNB philadelphiaost has chips",
     "chips" in PRODUCT_NAME_BLOCKERS.get("philadelphiaost", set()), True)
test("PNB philadelphiaost has dessertost",
     "dessertost" in PRODUCT_NAME_BLOCKERS.get("philadelphiaost", set()), True)
test("PNB philadelphiaost has salad",
     "salad" in PRODUCT_NAME_BLOCKERS.get("philadelphiaost", set()), True)
test("PNB philadelphiaost has mediterranean",
     "mediterranean" in PRODUCT_NAME_BLOCKERS.get("philadelphiaost", set()), True)
test("PNB philadelphiaost does not block vegansk (plant-based matches plain)",
     "vegansk" in PRODUCT_NAME_BLOCKERS.get("philadelphiaost", set()), False)
test("PNB philadelphiaost has kajmak",
     "kajmak" in PRODUCT_NAME_BLOCKERS.get("philadelphiaost", set()), True)
test("PNB philadelphiaost has pineapple",
     "pineapple" in PRODUCT_NAME_BLOCKERS.get("philadelphiaost", set()), True)
test("PNB philadelphiaost has almond",
     "almond" in PRODUCT_NAME_BLOCKERS.get("philadelphiaost", set()), True)
test("PNB philadelphiaost has chevre",
     "chevre" in PRODUCT_NAME_BLOCKERS.get("philadelphiaost", set()), True)
test("PNB philadelphiaost has pepper",
     "pepper" in PRODUCT_NAME_BLOCKERS.get("philadelphiaost", set()), True)
test("PNB philadelphiaost has 4 peppar",
     "4 peppar" in PRODUCT_NAME_BLOCKERS.get("philadelphiaost", set()), True)
test("PNB philadelphiaost does not block plant based (plant-based matches plain)",
     "plant based" in PRODUCT_NAME_BLOCKERS.get("philadelphiaost", set()), False)
test("PNB philadelphiaost does not block växtbaserad (plant-based matches plain)",
     "växtbaserad" in PRODUCT_NAME_BLOCKERS.get("philadelphiaost", set()), False)

# Vego/plant-based variants may match a plain meat/dairy ingredient (substitute
# policy: material is irrelevant for a plain ingredient; only flavor/specific-type
# words stay blocked). These offers extract the base keyword + a vego word and
# were previously blocked by removed material-vego PNBs.
test("vego korv matches plain korv",
     recipe_match_num(["200 g korv"], {"name": "Vego Korv Grillfärdig 300g", "category": "deli"}), 1)
test("vegansk hamburgare matches plain hamburgare",
     recipe_match_num(["2 hamburgare"], {"name": "Vegansk Hamburgare 2-pack", "category": "deli"}), 1)
test("beyond burger matches plain hamburgare",
     recipe_match_num(["4 hamburgare"], {"name": "Beyond Burger Fryst 226g Beyond Meat", "category": "deli"}), 1)
test("vegansk bacon matches plain bacon",
     recipe_match_num(["100 g bacon"], {"name": "Bacon Vegansk Skivad 100g", "category": "deli"}), 1)
test("vegansk schnitzel matches plain schnitzel",
     recipe_match_num(["4 schnitzel"], {"name": "Vegansk Schnitzel 300g", "category": "deli"}), 1)
test("vegansk salame matches plain salame",
     recipe_match_num(["50 g salame"], {"name": "Salame Vegansk 80g", "category": "deli"}), 1)
test("vegansk philadelphia matches plain philadelphiaost",
     recipe_match_num(["1 dl philadelphiaost"], {"name": "Philadelphia Vegansk 150g", "category": "dairy"}), 1)
# Guards: flavor/other-animal blocks on these keywords must STILL block.
test("kalkonbacon still blocked from plain bacon",
     recipe_match_num(["100 g bacon"], {"name": "Kalkonbacon Skivad 100g", "category": "deli"}), 0)
test("mediterranean färskost still blocked from plain färskost",
     recipe_match_num(["1 dl färskost"], {"name": "Färskost Mediterranean 150g Boursin", "category": "dairy"}), 0)
test("lammkorv still blocked from plain korv",
     recipe_match_num(["200 g korv"], {"name": "Lammkorvar Grill 400g", "category": "deli"}), 0)
# Nuanced material-vego removals (nötfärs/salsiccia/nuggets): plant-based variants
# may match the plain ingredient; form/protein/grain distinctions stay blocked.
test("vego salsiccia matches plain salsiccia",
     recipe_match_num(["2 färska salsicciakorvar"], {"name": "Vegansk Salsiccia 300g", "category": "deli"}), 1)
test("vego nuggets (quorn) matches plain nuggets",
     recipe_match_num(["kycklingnuggets 10-pack"], {"name": "Vegetariska Crispy Nuggets Quorn 250g", "category": "deli"}), 1)
test("vegonuggets routes to nuggets",
     recipe_match_num(["kycklingnuggets 10-pack"], {"name": "Vegonuggets 250g", "category": "deli"}), 1)
test("vegansk nötfärs matches plain nötfärs",
     recipe_match_num(["400 g nötfärs"], {"name": "Vegansk Nötfärs 400g", "category": "deli"}), 1)
# Guards: salsiccia stays distinct from loose mince; fish nuggets ≠ chicken nuggets;
# nötfärs stays distinct from whole beef cuts.
test("loose mince still blocked from salsiccia",
     recipe_match_num(["2 färska salsicciakorvar"], {"name": "Blandfärs 500g Nöt Fläsk", "category": "deli"}), 0)
test("fish nuggets still blocked from chicken nuggets",
     recipe_match_num(["kycklingnuggets 10-pack"], {"name": "Fisk Nuggets 250g Findus", "category": "deli"}), 0)
test("högrev (beef cut) still blocked from nötfärs",
     recipe_match_num(["400 g nötfärs"], {"name": "Högrev av nöt i bit 700g", "category": "meat"}), 0)


# "Corned beef" — cured/canned beef product, not fresh raw beef intended
# for slow-simmered stews/soups like köttsoppa with märgpipa.
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB nötkött has corned",
     "corned" in PRODUCT_NAME_BLOCKERS.get("nötkött", set()), True)
test("PNB notkott has corned",
     "corned" in PRODUCT_NAME_BLOCKERS.get("notkott", set()), True)


# "Rotfruktschips" — snack chip product should extract 'rotfruktschips' keyword
# (not decompose into raw root vegetable keywords like morot/palsternacka/kålrot).
from languages.sv.ingredient_matching.extraction import extract_keywords_from_ingredient
test("KW rotfruktschips extracts rotfruktschips not raw veg",
     extract_keywords_from_ingredient("125 g Rotfruktschips"), ["rotfruktschips"])
test("KW rotfrukter still decomposes to raw veg mix",
     "morot" in extract_keywords_from_ingredient("400 g rotfrukter"), True)

# runtime_pnb_persiljestjalkar_kryddsmor: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB persiljestjälkar has kryddsmör",
     "kryddsmör" in PRODUCT_NAME_BLOCKERS.get("persiljestjälkar", set()), True)
test("PNB persiljestjälkar has bruschetta",
     "bruschetta" in PRODUCT_NAME_BLOCKERS.get("persiljestjälkar", set()), True)
test("PNB persiljestjälkar has krutonger",
     "krutonger" in PRODUCT_NAME_BLOCKERS.get("persiljestjälkar", set()), True)

# runtime_stop_word_add_kommer: generated by dm matcher add stop-word
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add kommer",
     "kommer" in STOP_WORDS, True)
test("stop-word filters extraction kommer",
     "kommer" in kw("kommer"), False)

# runtime_fpb_fiskfile_tonfiskfile: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB fiskfilé has tonfiskfilé",
     "tonfiskfilé" in FALSE_POSITIVE_BLOCKERS.get("fiskfilé", set()), True)
test("FPB fiskfilé has tonfiskfilé",
     "tonfiskfilé" in FALSE_POSITIVE_BLOCKERS.get("fiskfilé", set()), True)

# runtime_fpb_oliver_snackoliver: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB oliver has snackoliver",
     "snackoliver" in FALSE_POSITIVE_BLOCKERS.get("oliver", set()), True)
test("FPB oliver has snacksoliver",
     "snacksoliver" in FALSE_POSITIVE_BLOCKERS.get("oliver", set()), True)


# extraction_helper_rotfruktschips: generated by dm matcher add extraction-helper
test("KW 125 g Rotfruktschips ingredient -> rotfruktschips", extract_keywords_from_ingredient("125 g Rotfruktschips"), ["rotfruktschips"])
# runtime_space_normalization_karl_johan_svamp_karl_johansvamp: generated by dm matcher add space-normalization
test("space-normalization karl-johan svamp -> karl-johansvamp",
     _apply_space_normalizations("karl-johan svamp"), "karl-johansvamp")

# runtime_space_normalization_karl_johan_svamp_karl_johansvamp: generated by dm matcher add space-normalization
test("space-normalization karl johan-svamp -> karl-johansvamp",
     _apply_space_normalizations("karl johan-svamp"), "karl-johansvamp")

# offer_extra_keyword_guanciale_pancetta: generated by dm matcher add offer-extra-keyword
test("offer-extra-keyword Pancetta 100g ICA matches guanciale",
     match("Pancetta 100g ICA", "1 frp Zeta Guanciale tärnad", "pantry"), "guanciale")

# runtime_pnb_prosciutto_salami: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB prosciutto has salami",
     "salami" in PRODUCT_NAME_BLOCKERS.get("prosciutto", set()), True)

# runtime_fpb_spenat_babyspenat: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB spenat has babyspenat",
     "babyspenat" in FALSE_POSITIVE_BLOCKERS.get("spenat", set()), True)


# Q65-3: Strip preference-marker sub-clauses ("förslagsvis", "gärna",
# "om möjligt", "exempelvis"). Recipe-author suggestions often reference
# OTHER ingredients and would leak unrelated keywords. The famous case is
# "Mandelmassa förslagsvis från samma semlor som grädden kommer ifrån"
# which leaked 'grädde' into the mandelmassa ingredient line.
test("KW mandelmassa preference clause strips grädde leak",
     extract_keywords_from_ingredient('2 dl Mandelmassa Förslagsvis från samma semlor som grädden kommer ifrån'),
     ['mandelmassa'])
test("KW grädde with förslagsvis sub-clause keeps main keyword",
     extract_keywords_from_ingredient('2 dl grädde, förslagsvis från Arla'),
     ['grädde'])
test("KW potatis with gärna sub-clause keeps main keyword",
     extract_keywords_from_ingredient('500 g potatis, gärna fast'),
     ['potatis'])

# offer_extra_keyword_farskost_vitloksost: generated by dm matcher add offer-extra-keyword
# Plain "färskost" recipe (no qualifier) is intentionally NOT matched — specialty
# qualifier 'vitlök' on färskost requires matching qualifier in ingredient text.
test("offer-extra-keyword Vitlöksost matches färskost med vitlök",
     match("Vitlöksost ca 500g Gäsene Mejeri", "200 g färskost med vitlök", "dairy"), "färskost")

# offer_extra_keyword_fetaost_vaccaost: generated by dm matcher add offer-extra-keyword
test("offer-extra-keyword Vaccaost 150g Wapnö matches fetaost",
     match("Vaccaost 150g Wapnö", "200 g fetaost", "dairy"), "fetaost")

# offer_extra_keyword_gratangost_matlagningsost: generated by dm matcher add offer-extra-keyword
test("offer-extra-keyword Matlagningsost Riven 12% 150g ICA matches gratängost",
     match("Matlagningsost Riven 12% 150g ICA", "200 g gratängost", "dairy"), "gratängost")

# offer_extra_keyword_dessertost_vitmogelost: generated by dm matcher add offer-extra-keyword
# Specialty qualifier 'chili' on dessertost requires matching qualifier in ingredient text.
test("offer-extra-keyword White chili vitmögelost matches white chili dessertost",
     match("White chili vitmögelost 150g Castello", "200 g white chili dessertost", "dairy"), "dessertost")

# offer_extra_keyword_dessertost_chaumes: generated by dm matcher add offer-extra-keyword
test("offer-extra-keyword Chaumes 25% ca 170g Falbygdens ost matches dessertost",
     match("Chaumes 25% ca 170g Falbygdens ost", "200 g dessertost", "dairy"), "dessertost")

# runtime_stop_word_add_brodrost: generated by dm matcher add stop-word
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add brödrost",
     "brödrost" in STOP_WORDS, True)
test("stop-word filters extraction brödrost",
     "brödrost" in kw("brödrost"), False)

# runtime_stop_word_add_kaffebryggare: generated by dm matcher add stop-word
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add kaffebryggare",
     "kaffebryggare" in STOP_WORDS, True)
test("stop-word filters extraction kaffebryggare",
     "kaffebryggare" in kw("kaffebryggare"), False)

# runtime_space_normalization_polpa_di_pomodoro_krossade_tomater: generated by dm matcher add space-normalization
test("space-normalization polpa di pomodoro -> krossade tomater",
     _apply_space_normalizations("polpa di pomodoro"), "krossade tomater")

# runtime_specialty_qualifier_tomat: generated by dm matcher add specialty-qualifier
from languages.sv.ingredient_matching.specialty_rules import SPECIALTY_QUALIFIERS, BIDIRECTIONAL_SPECIALTY_QUALIFIERS
test("specialty-qualifier tomat has datterini",
     "datterini" in SPECIALTY_QUALIFIERS.get("tomat", []), True)
test("specialty-qualifier tomat has datterino",
     "datterino" in SPECIALTY_QUALIFIERS.get("tomat", []), True)
test("specialty bidirectional tomat has datterini",
     "datterini" in BIDIRECTIONAL_PER_KEYWORD.get("tomat", set()), True)
test("bidirectional specialty qualifier hela blocks canned tomater from fresh recipes",
     "hela" in BIDIRECTIONAL_PER_KEYWORD.get("tomat", set()), True)
test("specialty bidirectional tomat has datterino",
     "datterino" in BIDIRECTIONAL_PER_KEYWORD.get("tomat", set()), True)

# runtime_specialty_qualifier_tomater: generated by dm matcher add specialty-qualifier
test("specialty-qualifier tomater has datterini",
     "datterini" in SPECIALTY_QUALIFIERS.get("tomater", []), True)
test("specialty-qualifier tomater has datterino",
     "datterino" in SPECIALTY_QUALIFIERS.get("tomater", []), True)
test("specialty bidirectional tomater has datterini",
     "datterini" in BIDIRECTIONAL_PER_KEYWORD.get("tomater", set()), True)
test("specialty bidirectional tomater has datterino",
     "datterino" in BIDIRECTIONAL_PER_KEYWORD.get("tomater", set()), True)

# runtime_qualifier_equivalent_datterini: generated by dm matcher add qualifier-equivalent
from languages.sv.ingredient_matching.specialty_rules import QUALIFIER_EQUIVALENTS
test("qualifier-equivalent datterini has burk",
     "burk" in QUALIFIER_EQUIVALENTS.get("datterini", set()), True)
test("qualifier-equivalent datterini has konserverad",
     "konserverad" in QUALIFIER_EQUIVALENTS.get("datterini", set()), True)
test("qualifier-equivalent datterini has konserverade",
     "konserverade" in QUALIFIER_EQUIVALENTS.get("datterini", set()), True)
test("qualifier-equivalent datterini has krossad",
     "krossad" in QUALIFIER_EQUIVALENTS.get("datterini", set()), True)
test("qualifier-equivalent datterini has krossade",
     "krossade" in QUALIFIER_EQUIVALENTS.get("datterini", set()), True)
test("qualifier-equivalent datterini has finkrossad",
     "finkrossad" in QUALIFIER_EQUIVALENTS.get("datterini", set()), True)
test("qualifier-equivalent datterini has finkrossade",
     "finkrossade" in QUALIFIER_EQUIVALENTS.get("datterini", set()), True)
test("qualifier-equivalent datterini has passerad",
     "passerad" in QUALIFIER_EQUIVALENTS.get("datterini", set()), True)
test("qualifier-equivalent datterini has passerade",
     "passerade" in QUALIFIER_EQUIVALENTS.get("datterini", set()), True)
test("qualifier-equivalent datterini has skalad",
     "skalad" in QUALIFIER_EQUIVALENTS.get("datterini", set()), True)
test("qualifier-equivalent datterini has skalade",
     "skalade" in QUALIFIER_EQUIVALENTS.get("datterini", set()), True)
test("qualifier-equivalent datterini has polpa",
     "polpa" in QUALIFIER_EQUIVALENTS.get("datterini", set()), True)

# runtime_qualifier_equivalent_datterino: generated by dm matcher add qualifier-equivalent
from languages.sv.ingredient_matching.specialty_rules import QUALIFIER_EQUIVALENTS
test("qualifier-equivalent datterino has burk",
     "burk" in QUALIFIER_EQUIVALENTS.get("datterino", set()), True)
test("qualifier-equivalent datterino has konserverad",
     "konserverad" in QUALIFIER_EQUIVALENTS.get("datterino", set()), True)
test("qualifier-equivalent datterino has konserverade",
     "konserverade" in QUALIFIER_EQUIVALENTS.get("datterino", set()), True)
test("qualifier-equivalent datterino has krossad",
     "krossad" in QUALIFIER_EQUIVALENTS.get("datterino", set()), True)
test("qualifier-equivalent datterino has krossade",
     "krossade" in QUALIFIER_EQUIVALENTS.get("datterino", set()), True)
test("qualifier-equivalent datterino has finkrossad",
     "finkrossad" in QUALIFIER_EQUIVALENTS.get("datterino", set()), True)
test("qualifier-equivalent datterino has finkrossade",
     "finkrossade" in QUALIFIER_EQUIVALENTS.get("datterino", set()), True)
test("qualifier-equivalent datterino has passerad",
     "passerad" in QUALIFIER_EQUIVALENTS.get("datterino", set()), True)
test("qualifier-equivalent datterino has passerade",
     "passerade" in QUALIFIER_EQUIVALENTS.get("datterino", set()), True)
test("qualifier-equivalent datterino has skalad",
     "skalad" in QUALIFIER_EQUIVALENTS.get("datterino", set()), True)
test("qualifier-equivalent datterino has skalade",
     "skalade" in QUALIFIER_EQUIVALENTS.get("datterino", set()), True)
test("qualifier-equivalent datterino has polpa",
     "polpa" in QUALIFIER_EQUIVALENTS.get("datterino", set()), True)

# runtime_qualifier_equivalent_burk: generated by dm matcher add qualifier-equivalent
from languages.sv.ingredient_matching.specialty_rules import QUALIFIER_EQUIVALENTS
test("qualifier-equivalent burk has datterini",
     "datterini" in QUALIFIER_EQUIVALENTS.get("burk", set()), True)
test("qualifier-equivalent burk has datterino",
     "datterino" in QUALIFIER_EQUIVALENTS.get("burk", set()), True)

# runtime_qualifier_equivalent_konserverad: generated by dm matcher add qualifier-equivalent
from languages.sv.ingredient_matching.specialty_rules import QUALIFIER_EQUIVALENTS
test("qualifier-equivalent konserverad has datterini",
     "datterini" in QUALIFIER_EQUIVALENTS.get("konserverad", set()), True)
test("qualifier-equivalent konserverad has datterino",
     "datterino" in QUALIFIER_EQUIVALENTS.get("konserverad", set()), True)

# runtime_qualifier_equivalent_konserverade: generated by dm matcher add qualifier-equivalent
from languages.sv.ingredient_matching.specialty_rules import QUALIFIER_EQUIVALENTS
test("qualifier-equivalent konserverade has datterini",
     "datterini" in QUALIFIER_EQUIVALENTS.get("konserverade", set()), True)
test("qualifier-equivalent konserverade has datterino",
     "datterino" in QUALIFIER_EQUIVALENTS.get("konserverade", set()), True)

# runtime_qualifier_equivalent_krossad: generated by dm matcher add qualifier-equivalent
from languages.sv.ingredient_matching.specialty_rules import QUALIFIER_EQUIVALENTS
test("qualifier-equivalent krossad has datterini",
     "datterini" in QUALIFIER_EQUIVALENTS.get("krossad", set()), True)
test("qualifier-equivalent krossad has datterino",
     "datterino" in QUALIFIER_EQUIVALENTS.get("krossad", set()), True)

# runtime_qualifier_equivalent_krossade: generated by dm matcher add qualifier-equivalent
from languages.sv.ingredient_matching.specialty_rules import QUALIFIER_EQUIVALENTS
test("qualifier-equivalent krossade has datterini",
     "datterini" in QUALIFIER_EQUIVALENTS.get("krossade", set()), True)
test("qualifier-equivalent krossade has datterino",
     "datterino" in QUALIFIER_EQUIVALENTS.get("krossade", set()), True)

# runtime_qualifier_equivalent_finkrossad: generated by dm matcher add qualifier-equivalent
from languages.sv.ingredient_matching.specialty_rules import QUALIFIER_EQUIVALENTS
test("qualifier-equivalent finkrossad has datterini",
     "datterini" in QUALIFIER_EQUIVALENTS.get("finkrossad", set()), True)
test("qualifier-equivalent finkrossad has datterino",
     "datterino" in QUALIFIER_EQUIVALENTS.get("finkrossad", set()), True)

# runtime_qualifier_equivalent_finkrossade: generated by dm matcher add qualifier-equivalent
from languages.sv.ingredient_matching.specialty_rules import QUALIFIER_EQUIVALENTS
test("qualifier-equivalent finkrossade has datterini",
     "datterini" in QUALIFIER_EQUIVALENTS.get("finkrossade", set()), True)
test("qualifier-equivalent finkrossade has datterino",
     "datterino" in QUALIFIER_EQUIVALENTS.get("finkrossade", set()), True)

# runtime_qualifier_equivalent_passerad: generated by dm matcher add qualifier-equivalent
from languages.sv.ingredient_matching.specialty_rules import QUALIFIER_EQUIVALENTS
test("qualifier-equivalent passerad has datterini",
     "datterini" in QUALIFIER_EQUIVALENTS.get("passerad", set()), True)
test("qualifier-equivalent passerad has datterino",
     "datterino" in QUALIFIER_EQUIVALENTS.get("passerad", set()), True)

# runtime_qualifier_equivalent_passerade: generated by dm matcher add qualifier-equivalent
from languages.sv.ingredient_matching.specialty_rules import QUALIFIER_EQUIVALENTS
test("qualifier-equivalent passerade has datterini",
     "datterini" in QUALIFIER_EQUIVALENTS.get("passerade", set()), True)
test("qualifier-equivalent passerade has datterino",
     "datterino" in QUALIFIER_EQUIVALENTS.get("passerade", set()), True)

# runtime_qualifier_equivalent_skalad: generated by dm matcher add qualifier-equivalent
from languages.sv.ingredient_matching.specialty_rules import QUALIFIER_EQUIVALENTS
test("qualifier-equivalent skalad has datterini",
     "datterini" in QUALIFIER_EQUIVALENTS.get("skalad", set()), True)
test("qualifier-equivalent skalad has datterino",
     "datterino" in QUALIFIER_EQUIVALENTS.get("skalad", set()), True)

# runtime_qualifier_equivalent_skalade: generated by dm matcher add qualifier-equivalent
from languages.sv.ingredient_matching.specialty_rules import QUALIFIER_EQUIVALENTS
test("qualifier-equivalent skalade has datterini",
     "datterini" in QUALIFIER_EQUIVALENTS.get("skalade", set()), True)
test("qualifier-equivalent skalade has datterino",
     "datterino" in QUALIFIER_EQUIVALENTS.get("skalade", set()), True)

# runtime_qualifier_equivalent_polpa: generated by dm matcher add qualifier-equivalent
from languages.sv.ingredient_matching.specialty_rules import QUALIFIER_EQUIVALENTS
test("qualifier-equivalent polpa has datterini",
     "datterini" in QUALIFIER_EQUIVALENTS.get("polpa", set()), True)
test("qualifier-equivalent polpa has datterino",
     "datterino" in QUALIFIER_EQUIVALENTS.get("polpa", set()), True)

# runtime_space_normalization_choklad_mork_morkchoklad: generated by dm matcher add space-normalization
test("space-normalization choklad mörk -> mörkchoklad",
     _apply_space_normalizations("choklad mörk"), "mörkchoklad")

# runtime_pnb_macadamianotter_haagen_dazs: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB macadamianötter has häagen-dazs",
     "häagen-dazs" in PRODUCT_NAME_BLOCKERS.get("macadamianötter", set()), True)
test("PNB macadamianötter has haagen-dazs",
     "haagen-dazs" in PRODUCT_NAME_BLOCKERS.get("macadamianötter", set()), True)
test("PNB macadamianötter has brittle",
     "brittle" in PRODUCT_NAME_BLOCKERS.get("macadamianötter", set()), True)
test("PNB macadamianötter has glass",
     "glass" in PRODUCT_NAME_BLOCKERS.get("macadamianötter", set()), True)
test("PNB macadamianötter has gelato",
     "gelato" in PRODUCT_NAME_BLOCKERS.get("macadamianötter", set()), True)

# runtime_ksbc_formbrod_grov: FPB was attempted but doesn't work here (smart-blocker bypass
# due to _append_canonical_keyword_synonyms adding 'formbröd' standalone). KSBC operates
# at matched_keyword level and correctly suppresses formbröd when ingredient says grov/grova.
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC formbröd has grov",
     "grov" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("formbröd", set()), True)
test("KSBC formbröd has grova",
     "grova" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("formbröd", set()), True)
test("KSBC formbröd: grov brödskivor should not match Formfranska",
     match("Formfranska 1000g ICA", "3 grova brödskivor t ex rågbröd", "bread"), None)
test("KSBC formbröd: plain brödskivor still matches Formfranska",
     match("Formfranska 1000g ICA", "3 brödskivor", "bread"), "formbröd")

# pangasiusmalfilé: keyword_synonym was REVERTED (broke vit-fiskfilé bridge);
# product/ingredient both extract pangasiusmalfilé directly, so direct keyword match wins.
test("Pangasiusmalfilé product matches pangasiusmalfilé ingredient (direct keyword)",
     match("Pangasiusmalfilé Fryst 600g ICA", "pangasiusmalfilé", "fish"), "pangasiusmalfilé")

# extraction_helper_tomat: generated by dm matcher add extraction-helper
test("KW Polpa di Pomodoro 400g Zeta product -> tomat", extract_keywords_from_product("Polpa di Pomodoro 400g Zeta", ""), ["tomat"])

# runtime_stop_word_add_mosad: generated by dm matcher add stop-word
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add mosad",
     "mosad" in STOP_WORDS, True)
test("stop-word filters extraction mosad",
     "mosad" in kw("mosad"), False)
test("stop-word add mosade",
     "mosade" in STOP_WORDS, True)
test("stop-word filters extraction mosade",
     "mosade" in kw("mosade"), False)
test("stop-word add mosat",
     "mosat" in STOP_WORDS, True)
test("stop-word filters extraction mosat",
     "mosat" in kw("mosat"), False)
# Specific FP guard: "Mosade katrinplommon" baby-food product (the only product
# starting with "Mosade") must not bleed via "mosade" adjective into recipes
# asking for "mosade blåbär"/"mosade vinbär" (mashed berries). The adjective
# describes the cook's instruction, not an ingredient.
test("mosade adjective blocked from baby-food prune bleed",
     match("Mosade katrinplommon/prunes 4m Ekologisk 70g Ella´s Kitchen", "1/2 dl mosade blåbär till den blåa botten"),
     None)

# Pangasiusfilé product (no "mal" in name) gets pangasiusmalfilé as extra keyword
# via offer-extra-keyword bridge, but the direct match on pangasiusfilé wins.
test("Pangasiusfilé product matches pangasiusfilé ingredient (direct keyword)",
     match("Pangasiusfilé", "800 g pangasiusfilé", "pantry"), "pangasiusfilé")

# runtime_pnb_nutella_kex: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB nutella has kex",
     "kex" in PRODUCT_NAME_BLOCKERS.get("nutella", set()), True)
test("PNB nutella has croissanter",
     "croissanter" in PRODUCT_NAME_BLOCKERS.get("nutella", set()), True)
test("PNB nutella has hasselnötsglass",
     "hasselnötsglass" in PRODUCT_NAME_BLOCKERS.get("nutella", set()), True)
test("PNB nutella has mellanmål",
     "mellanmål" in PRODUCT_NAME_BLOCKERS.get("nutella", set()), True)

# runtime_pnb_cajun_sas: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB cajun has sås",
     "sås" in PRODUCT_NAME_BLOCKERS.get("cajun", set()), True)
test("PNB cajun has bbq",
     "bbq" in PRODUCT_NAME_BLOCKERS.get("cajun", set()), True)

# runtime_fpb_socker_sockermassa: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB socker has sockermassa",
     "sockermassa" in FALSE_POSITIVE_BLOCKERS.get("socker", set()), True)

# runtime_fpb_marsipan_marsipanlock: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB marsipan has marsipanlock",
     "marsipanlock" in FALSE_POSITIVE_BLOCKERS.get("marsipan", set()), True)

# runtime_fpb_mango_mangopure: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB mango has mangopure",
     "mangopure" in FALSE_POSITIVE_BLOCKERS.get("mango", set()), True)
test("FPB mango has mangopure",
     "mangopure" in FALSE_POSITIVE_BLOCKERS.get("mango", set()), True)

# runtime_pnb_paprikor_crackers: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB paprikor has crackers",
     "crackers" in PRODUCT_NAME_BLOCKERS.get("paprikor", set()), True)
test("PNB paprikor has mild",
     "mild" in PRODUCT_NAME_BLOCKERS.get("paprikor", set()), True)
test("PNB paprikor has malen",
     "malen" in PRODUCT_NAME_BLOCKERS.get("paprikor", set()), True)

# runtime_pnb_pannkakor_bakmix: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB pannkakor has bakmix",
     "bakmix" in PRODUCT_NAME_BLOCKERS.get("pannkakor", set()), True)

# runtime_pnb_vitchoklad_havregrynsgrot: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB vitchoklad has havregrynsgröt",
     "havregrynsgröt" in PRODUCT_NAME_BLOCKERS.get("vitchoklad", set()), True)
test("PNB vitchoklad has proteinpudding",
     "proteinpudding" in PRODUCT_NAME_BLOCKERS.get("vitchoklad", set()), True)
test("PNB vitchoklad has dessertsås",
     "dessertsås" in PRODUCT_NAME_BLOCKERS.get("vitchoklad", set()), True)
test("PNB vitchoklad has dessertsås",
     "dessertsås" in PRODUCT_NAME_BLOCKERS.get("vitchoklad", set()), True)

# runtime_fpb_ris_riskakor: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB ris has riskakor",
     "riskakor" in FALSE_POSITIVE_BLOCKERS.get("ris", set()), True)
test("FPB ris has riskaka",
     "riskaka" in FALSE_POSITIVE_BLOCKERS.get("ris", set()), True)

# runtime_fpb_lax_gravlaxsas: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB lax has gravlaxsås",
     "gravlaxsås" in FALSE_POSITIVE_BLOCKERS.get("lax", set()), True)
test("FPB lax has gravlaxsås",
     "gravlaxsås" in FALSE_POSITIVE_BLOCKERS.get("lax", set()), True)

# runtime_fpb_vinager_balsamvinager: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB vinäger has balsamvinäger",
     "balsamvinäger" in FALSE_POSITIVE_BLOCKERS.get("vinäger", set()), True)
test("FPB vinäger has äppelcidervinäger",
     "äppelcidervinäger" in FALSE_POSITIVE_BLOCKERS.get("vinäger", set()), True)
test("FPB vinäger has vitvinsvinäger",
     "vitvinsvinäger" in FALSE_POSITIVE_BLOCKERS.get("vinäger", set()), True)
test("FPB vinäger has rödvinsvinäger",
     "rödvinsvinäger" in FALSE_POSITIVE_BLOCKERS.get("vinäger", set()), True)
test("FPB vinäger has risvinäger",
     "risvinäger" in FALSE_POSITIVE_BLOCKERS.get("vinäger", set()), True)
test("FPB vinäger has sherryvinäger",
     "sherryvinäger" in FALSE_POSITIVE_BLOCKERS.get("vinäger", set()), True)
test("FPB vinäger has champagnevinäger",
     "champagnevinäger" in FALSE_POSITIVE_BLOCKERS.get("vinäger", set()), True)
test("FPB vinäger has merlotvinäger",
     "merlotvinäger" in FALSE_POSITIVE_BLOCKERS.get("vinäger", set()), True)
test("FPB vinäger has cavavinäger",
     "cavavinäger" in FALSE_POSITIVE_BLOCKERS.get("vinäger", set()), True)
test("FPB vinäger has portvinsvinäger",
     "portvinsvinäger" in FALSE_POSITIVE_BLOCKERS.get("vinäger", set()), True)

# runtime_pnb_lask_tonic: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB läsk has tonic",
     "tonic" in PRODUCT_NAME_BLOCKERS.get("läsk", set()), True)
test("PNB läsk has tonicvatten",
     "tonicvatten" in PRODUCT_NAME_BLOCKERS.get("läsk", set()), True)

# runtime_ksbc_kex_rawfood: FPB was attempted but doesn't work (smart-blocker bypass —
# 'kex' is a standalone word in "Rawfood Kex"). KSBC suppresses at matched_keyword level.
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC kex has rawfood",
     "rawfood" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kex", set()), True)
test("KSBC kex has rawfoods",
     "rawfoods" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kex", set()), True)
test("KSBC kex: Rawfood Kex blocks conventional crackers",
     match("Mariekex 215g Göteborgs", "Rawfood Kex", "pantry"), None)
test("KSBC kex: plain kex still matches",
     match("Mariekex 215g Göteborgs", "2 dl smulade kex", "pantry"), "kex")

# runtime_ksbc_persilja_krutonger: generated by dm matcher add ksbc
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC persilja has krutonger",
     "krutonger" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("persilja", set()), True)

# runtime_ksbc_chili_barta: generated by dm matcher add ksbc
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC chili has bärta",
     "bärta" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("chili", set()), True)

# runtime_pnb_chili_barta: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB chili has bärta",
     "bärta" in PRODUCT_NAME_BLOCKERS.get("chili", set()), True)

# Q67-6: eller-group parent + child compound both extract
# Without the fix, "lök eller rödlök" collapses to just ['rödlök']
from languages.sv.ingredient_matching.extraction import extract_keywords_from_ingredient as _ext_kw_ing
test("eller-split: lök eller rödlök yields both",
     _ext_kw_ing("lök eller rödlök"), ['lök', 'rödlök'])
test("eller-split: 2 gula eller röda lökar yields both via rewrite",
     _ext_kw_ing("2 gula eller röda lökar"), ['lök', 'rödlök'])
test("eller-split: 1 gul eller röd lök yields both via rewrite",
     _ext_kw_ing("1 gul eller röd lök"), ['lök', 'rödlök'])
test("eller-split: existing rödkål eller vitkål still yields both",
     _ext_kw_ing("rödkål eller vitkål"), ['rödkål', 'vitkål'])

# runtime_fpb_bittermandel_bittermandelarom: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB bittermandel has bittermandelarom",
     "bittermandelarom" in FALSE_POSITIVE_BLOCKERS.get("bittermandel", set()), True)

# keyword_synonym_bittermandel_bittermandlar: generated by dm matcher add keyword-synonym
test("Keyword synonym bittermandlar matches bittermandel",
     match("Bittermandel 50g ICA", "bittermandlar", "pantry"), "bittermandel")

# runtime_fpb_sirap_jordgubbssirap: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB sirap has jordgubbssirap",
     "jordgubbssirap" in FALSE_POSITIVE_BLOCKERS.get("sirap", set()), True)
test("FPB sirap has hallonsirap",
     "hallonsirap" in FALSE_POSITIVE_BLOCKERS.get("sirap", set()), True)
test("FPB sirap has mintsirap",
     "mintsirap" in FALSE_POSITIVE_BLOCKERS.get("sirap", set()), True)
test("FPB sirap has fläderblomssirap",
     "fläderblomssirap" in FALSE_POSITIVE_BLOCKERS.get("sirap", set()), True)
test("FPB sirap has vaniljsirap",
     "vaniljsirap" in FALSE_POSITIVE_BLOCKERS.get("sirap", set()), True)
test("FPB sirap has karamellsirap",
     "karamellsirap" in FALSE_POSITIVE_BLOCKERS.get("sirap", set()), True)
test("FPB sirap has citronsirap",
     "citronsirap" in FALSE_POSITIVE_BLOCKERS.get("sirap", set()), True)

# runtime_pnb_sockerkaka_basic: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB sockerkaka has basic",
     "basic" in PRODUCT_NAME_BLOCKERS.get("sockerkaka", set()), True)

# runtime_ksbc_mjukost_raksmak: generated by dm matcher add ksbc
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC mjukost has räksmak",
     "räksmak" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("mjukost", set()), True)
test("KSBC mjukost has raksmak",
     "raksmak" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("mjukost", set()), True)
test("KSBC mjukost has skinksmak",
     "skinksmak" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("mjukost", set()), True)
test("KSBC mjukost has baconsmak",
     "baconsmak" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("mjukost", set()), True)
test("KSBC mjukost has champinjonsmak",
     "champinjonsmak" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("mjukost", set()), True)

# Q68-3: mjukost/Xost flavor isolation system
test("mjukost-Xost: räksoppa-recept matches Räkost Kavli",
     match("Räkost 330g Kavli", "330 g mjukost, med räksmak", "dairy"), "räkost")
test("mjukost-Xost: räksoppa-recept matches Mjukost Räkor (via prepended räkost)",
     match("Mjukost Räkor 330g Fjällbrynt", "330 g mjukost, med räksmak", "dairy"), "räkost")
test("mjukost-Xost: räksoppa-recept BLOCKS Mjukost Päron (wrong flavor)",
     match("Mjukost Päron 175g Kavli", "330 g mjukost, med räksmak", "dairy"), None)
test("mjukost-Xost: skinksoppa-recept matches Skinkost",
     match("Skinkost 330g Kavli", "mjukost med skinksmak", "dairy"), "skinkost")
test("mjukost-Xost: plain mjukost matches Räkost (broad family for plain recipes)",
     match("Räkost 330g Kavli", "200 g mjukost", "dairy"), "mjukost")
test("mjukost-Xost: plain mjukost matches Mjukost Naturell",
     match("Mjukost Naturell 5% 330g Fjällbrynt", "200 g mjukost", "dairy"), "mjukost")

# runtime_ksbc_woksas_pad_thai: generated by dm matcher add ksbc
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC woksås has pad thai",
     "pad thai" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("woksås", set()), True)
test("KSBC woksås has padthai",
     "padthai" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("woksås", set()), True)
test("KSBC woksås has teriyaki",
     "teriyaki" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("woksås", set()), True)
test("KSBC woksås has satay",
     "satay" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("woksås", set()), True)
test("KSBC woksås has korean bbq",
     "korean bbq" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("woksås", set()), True)
test("KSBC woksås has koreanbbq",
     "koreanbbq" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("woksås", set()), True)
test("KSBC woksås has sweet sour",
     "sweet sour" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("woksås", set()), True)
test("KSBC woksås has sweet & sour",
     "sweet & sour" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("woksås", set()), True)
test("KSBC woksås has sweet&sour",
     "sweet&sour" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("woksås", set()), True)
test("KSBC woksås has sweetsour",
     "sweetsour" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("woksås", set()), True)

# Q68-4: woksås/Xwoksås flavor isolation system
test("woksås-Xwoksås: Pad Thai-recept matches Pad Thai products",
     match("Woksås Pad Thai 120g Blue Dragon", "120 g Woksås Pad Thai", "pantry"), "padthaiwoksas")
test("woksås-Xwoksås: Pad Thai-recept matches Asian Pad Thai products",
     match("Woksås Asian Pad Thai 250ml Santa Maria", "120 g Woksås Pad Thai", "pantry"), "padthaiwoksas")
test("woksås-Xwoksås: Pad Thai-recept BLOCKS Satay (wrong flavor)",
     match("Woksås Satay 120g Blue Dragon", "120 g Woksås Pad Thai", "pantry"), None)
test("woksås-Xwoksås: Teriyaki-recept matches Teriyaki products",
     match("Woksås Teriyaki 120g Blue dragon", "250 ml woksås teriyaki", "pantry"), "teriyakiwoksas")
test("woksås-Xwoksås: plain woksås matches all flavors (broad family)",
     match("Woksås Satay 120g Blue Dragon", "2 dl woksås", "pantry"), "woksås")
test("woksås-Xwoksås: Sweet&Sour-recept BLOCKS all wrong flavors",
     match("Woksås Satay 120g Blue Dragon", "120 g Woksås Sweet&Sour", "pantry"), None)

# runtime_ksbc_ost_mjolkfri: generated by dm matcher add ksbc
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC ost has mjölkfri",
     "mjölkfri" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("ost", set()), True)
test("KSBC ost has mjölkfritt",
     "mjölkfritt" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("ost", set()), True)
test("KSBC ost has mjolkfri",
     "mjolkfri" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("ost", set()), True)
test("KSBC ost has mjolkfritt",
     "mjolkfritt" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("ost", set()), True)
test("KSBC ost has laktosfri",
     "laktosfri" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("ost", set()), True)
test("KSBC ost has laktosfritt",
     "laktosfritt" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("ost", set()), True)
test("KSBC ost has vegan",
     "vegan" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("ost", set()), True)
test("KSBC ost has vegansk",
     "vegansk" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("ost", set()), True)
test("KSBC ost has växtbaserad",
     "växtbaserad" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("ost", set()), True)
test("KSBC ost has vaxtbaserad",
     "vaxtbaserad" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("ost", set()), True)

# Q68-5: ost dietary modifier isolation system
test("ost-dietary: mjölkfri-recipe matches Violife via veganost",
     match("Vegansk Ost Violife 200g", "ev ost, mjölkfri", "dairy"), "veganost")
test("ost-dietary: mjölkfri-recipe matches Greenvie via veganost",
     match("Ost Greenvie 200g", "ev ost, mjölkfri", "dairy"), "veganost")
test("ost-dietary: laktosfri-recipe matches Violife",
...
# A qualifier living only inside an "(alternativt X)" alternative must NOT be
# treated as a requirement on the primary ingredient. Plain fresh tomat must
# still match when the recipe lists "6 tomater (alternativt 1 burk krossade tomater)".
test("alternativt: plain fresh tomat matches despite krossade in alternative",
     recipe_match_num(["6 tomater (alternativt 1 burk krossade tomater)"],
...
test("alternativt-guard: soltorkad-only recipe still blocks plain fresh tomat",
     recipe_match_num(["2 dl soltorkade tomater"],
                      {"name": "Tomat Kvist 500g Klass 1", "category": "vegetables"}), 0)

# runtime_ksbc_choklad_rispuffar: generated by dm matcher add ksbc
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC choklad has rispuffar",
     "rispuffar" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("choklad", set()), True)

# runtime_pnb_rorsocker_panela: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB rörsocker has panela",
     "panela" in PRODUCT_NAME_BLOCKERS.get("rörsocker", set()), True)

# runtime_ksbc_margarin: margarin is inherently dairy-free, so dairy-free wording
# must NOT suppress the margarin keyword (a "mjölkfritt margarin" recipe matches
# plain bak/stek-margarin). Plant-based wording (vegansk/växtbaserad) still does.
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC margarin does NOT suppress on mjölkfritt",
     "mjölkfritt" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("margarin", set()), False)
test("KSBC margarin does NOT suppress on mjölkfri",
     "mjölkfri" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("margarin", set()), False)
test("KSBC margarin still suppresses on vegansk",
     "vegansk" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("margarin", set()), True)
test("KSBC margarin still suppresses on växtbaserad",
     "växtbaserad" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("margarin", set()), True)

# runtime_fpb_choklad_chokladmjolk: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB choklad has chokladmjölk",
     "chokladmjölk" in FALSE_POSITIVE_BLOCKERS.get("choklad", set()), True)
test("FPB choklad has chokladmjolk",
     "chokladmjolk" in FALSE_POSITIVE_BLOCKERS.get("choklad", set()), True)

# runtime_fpb_choklad_pralin: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB choklad has pralin",
     "pralin" in FALSE_POSITIVE_BLOCKERS.get("choklad", set()), True)

# runtime_stop_word_add_liknande: generated by dm matcher add stop-word
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add liknande",
     "liknande" in STOP_WORDS, True)
test("stop-word filters extraction liknande",
     "liknande" in kw("liknande"), False)

# runtime_fpb_hallon_hallonpulver: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB hallon has hallonpulver",
     "hallonpulver" in FALSE_POSITIVE_BLOCKERS.get("hallon", set()), True)

# runtime_pnb_macadamianotter_notmix: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB macadamianötter has nötmix",
     "nötmix" in PRODUCT_NAME_BLOCKERS.get("macadamianötter", set()), True)
test("PNB macadamianötter has notmix",
     "notmix" in PRODUCT_NAME_BLOCKERS.get("macadamianötter", set()), True)

# runtime_ksbc_margarin_vaxtbaserat: generated by dm matcher add ksbc
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC margarin has växtbaserat",
     "växtbaserat" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("margarin", set()), True)
test("KSBC margarin has vaxtbaserat",
     "vaxtbaserat" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("margarin", set()), True)
test("KSBC margarin has växtbaserad",
     "växtbaserad" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("margarin", set()), True)
test("KSBC margarin has vaxtbaserad",
     "vaxtbaserad" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("margarin", set()), True)
test("KSBC margarin has vegan",
     "vegan" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("margarin", set()), True)
test("KSBC margarin has vegansk",
     "vegansk" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("margarin", set()), True)

# keyword_synonym_mandelmassa_mandelmass: generated by dm matcher add keyword-synonym
test("Keyword synonym mandelmass matches mandelmassa",
     match("Mandelmassa 200g ICA", "mandelmass", "baking"), "mandelmassa")

# runtime_pnb_vallmofron_vitt: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB vallmofrön has vitt",
     "vitt" in PRODUCT_NAME_BLOCKERS.get("vallmofrön", set()), True)
test("PNB vallmofrön has vit",
     "vit" in PRODUCT_NAME_BLOCKERS.get("vallmofrön", set()), True)

# runtime_fpb_tonfisk_tonfiskpastej: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB tonfisk has tonfiskpastej",
     "tonfiskpastej" in FALSE_POSITIVE_BLOCKERS.get("tonfisk", set()), True)
test("FPB tonfisk has tonfiskpastejer",
     "tonfiskpastejer" in FALSE_POSITIVE_BLOCKERS.get("tonfisk", set()), True)

# runtime_ksbc_woksas_sweet_chili: generated by dm matcher add ksbc
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC woksås has sweet chili",
     "sweet chili" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("woksås", set()), True)
test("KSBC woksås has sweetchili",
     "sweetchili" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("woksås", set()), True)
test("KSBC woksås has sweet&chili",
     "sweet&chili" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("woksås", set()), True)
test("KSBC woksås has sweetchilisås",
     "sweetchilisås" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("woksås", set()), True)
test("KSBC woksås has sweet chilisås",
     "sweet chilisås" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("woksås", set()), True)

# runtime_fpb_svamp_svampburgare: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS

# runtime_pnb_fron_musli: blocks musli/müsli (granola/cereal carrier products) from
# matching "blandade frön"-style ingredient that wants loose seeds for bread topping
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
...
# runtime_pnb_chips_tortilla: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB chips has tortilla",
     "tortilla" in PRODUCT_NAME_BLOCKERS.get("chips", set()), True)
test("PNB chips has tortillachips",
     "tortillachips" in PRODUCT_NAME_BLOCKERS.get("chips", set()), True)
test("PNB chips has nacho",
     "nacho" in PRODUCT_NAME_BLOCKERS.get("chips", set()), True)
test("PNB chips has nachochips",
     "nachochips" in PRODUCT_NAME_BLOCKERS.get("chips", set()), True)
test("PNB chips has nachos",
     "nachos" in PRODUCT_NAME_BLOCKERS.get("chips", set()), True)
test("PNB chips has corn",
     "corn" in PRODUCT_NAME_BLOCKERS.get("chips", set()), True)
test("PNB chips has bananchips",
     "bananchips" in PRODUCT_NAME_BLOCKERS.get("chips", set()), True)
test("PNB chips has bananachips",
     "bananachips" in PRODUCT_NAME_BLOCKERS.get("chips", set()), True)

# runtime_gpb_toffifee: generated by dm matcher add gpb
from languages.sv.ingredient_matching.blocker_data import GLOBAL_PRODUCT_NAME_BLOCKERS
test("gpb overlay contains toffifee",
     "toffifee" in GLOBAL_PRODUCT_NAME_BLOCKERS, True)
test("gpb blocks backend match toffifee",
     recipe_match_num(['ost'], {"name": "toffifee Ost", "category": "dairy"}), 0)

# runtime_ksbc_ost_farost: generated by dm matcher add ksbc
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC ost has fårost",
     "fårost" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("ost", set()), True)
test("KSBC ost has farost",
     "farost" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("ost", set()), True)

# offer_extra_keyword_farost_manchego: generated by dm matcher add offer-extra-keyword
test("offer-extra-keyword Manchego matches fårost",
     match("Manchego", "fårost", "cheese"), "fårost")

# vegomargarin routing: plant-based margarin brands route via extraction.py bridges
test("vegomargarin Flora plant margarin matches vegosmör",
     match("Flora Växtbaserat Margarin 500g", "vegosmör", "dairy"), "vegomargarin")

# ========================================================================
# runtime_ksbc_kaka_mjuk: generated by dm matcher add ksbc
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC kaka has mjuk",
     "mjuk" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kaka", set()), True)
# runtime_ksbc_gradde_syrad: generated by dm matcher add ksbc
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC grädde has syrad",
     "syrad" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("grädde", set()), True)
# runtime_pnb_olivolja_basilika: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB olivolja has basilika",
     "basilika" in PRODUCT_NAME_BLOCKERS.get("olivolja", set()), True)
test("PNB olivolja has vitlök",
     "vitlök" in PRODUCT_NAME_BLOCKERS.get("olivolja", set()), True)
test("PNB olivolja has vitlök",
     "vitlök" in PRODUCT_NAME_BLOCKERS.get("olivolja", set()), True)
test("PNB olivolja has rosmarin",
     "rosmarin" in PRODUCT_NAME_BLOCKERS.get("olivolja", set()), True)
# runtime_ksbc_lime_dressing: generated by dm matcher add ksbc
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC lime has dressing",
     "dressing" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("lime", set()), True)
# runtime_ksbc_bonor_matvete: generated by dm matcher add ksbc
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC bönor has matvete",
     "matvete" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("bönor", set()), True)
# runtime_pnb_chokladsmak_mjolk: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB chokladsmak has mjölk",
     "mjölk" in PRODUCT_NAME_BLOCKERS.get("chokladsmak", set()), True)
test("PNB chokladsmak has mjölk",
     "mjölk" in PRODUCT_NAME_BLOCKERS.get("chokladsmak", set()), True)
test("PNB chokladsmak has dryck",
     "dryck" in PRODUCT_NAME_BLOCKERS.get("chokladsmak", set()), True)
# runtime_pnb_coconut_pineapple: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB coconut has pineapple",
     "pineapple" in PRODUCT_NAME_BLOCKERS.get("coconut", set()), True)
test("PNB coconut has nick",
     "nick" in PRODUCT_NAME_BLOCKERS.get("coconut", set()), True)
test("PNB coconut has skumtoppar",
     "skumtoppar" in PRODUCT_NAME_BLOCKERS.get("coconut", set()), True)
test("PNB coconut has chokladbar",
     "chokladbar" in PRODUCT_NAME_BLOCKERS.get("coconut", set()), True)
test("PNB coconut has aminos",
     "aminos" in PRODUCT_NAME_BLOCKERS.get("coconut", set()), True)
# runtime_pnb_apelsinsmak_kondisrullar: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB apelsinsmak has kondisrullar",
     "kondisrullar" in PRODUCT_NAME_BLOCKERS.get("apelsinsmak", set()), True)
# runtime_fpb_biff_bonbiffar: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB biff has bönbiffar",
     "bönbiffar" in FALSE_POSITIVE_BLOCKERS.get("biff", set()), True)
test("FPB biff has bönbiff",
     "bönbiff" in FALSE_POSITIVE_BLOCKERS.get("biff", set()), True)
test("FPB biff has bonbiffar",
     "bonbiffar" in FALSE_POSITIVE_BLOCKERS.get("biff", set()), True)
test("FPB biff has bonbiff",
     "bonbiff" in FALSE_POSITIVE_BLOCKERS.get("biff", set()), True)
# runtime_pnb_philadelphiaost_gasene: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB philadelphiaost has gäsene",
     "gäsene" in PRODUCT_NAME_BLOCKERS.get("philadelphiaost", set()), True)
test("PNB philadelphiaost has gasene",
     "gasene" in PRODUCT_NAME_BLOCKERS.get("philadelphiaost", set()), True)
# runtime_pnb_gullok_granulerad: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB gullök has granulerad",
     "granulerad" in PRODUCT_NAME_BLOCKERS.get("gullök", set()), True)
test("PNB gullök has granulerad lök",
     "granulerad lök" in PRODUCT_NAME_BLOCKERS.get("gullök", set()), True)
test("PNB gullök has granulerat",
     "granulerat" in PRODUCT_NAME_BLOCKERS.get("gullök", set()), True)
# runtime_fpb_bitter_bittermandlar: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB bitter has bittermandlar",
     "bittermandlar" in FALSE_POSITIVE_BLOCKERS.get("bitter", set()), True)
# runtime_fpb_applejuice_granat: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB äpplejuice has granat",
     "granat" in FALSE_POSITIVE_BLOCKERS.get("äpplejuice", set()), True)
test("FPB äpplejuice has granatäpple",
     "granatäpple" in FALSE_POSITIVE_BLOCKERS.get("äpplejuice", set()), True)
test("FPB äpplejuice has granatappel",
     "granatappel" in FALSE_POSITIVE_BLOCKERS.get("äpplejuice", set()), True)
test("FPB äpplejuice has granatäpplejuice",
     "granatäpplejuice" in FALSE_POSITIVE_BLOCKERS.get("äpplejuice", set()), True)
test("FPB äpplejuice has granatappeljuice",
     "granatappeljuice" in FALSE_POSITIVE_BLOCKERS.get("äpplejuice", set()), True)
# runtime_pnb_pumpakarnor_musli: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB pumpakärnor has musli",
     "musli" in PRODUCT_NAME_BLOCKERS.get("pumpakärnor", set()), True)
test("PNB pumpakärnor has musli",
     "musli" in PRODUCT_NAME_BLOCKERS.get("pumpakärnor", set()), True)
# runtime_pnb_limejuice_coriander: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB limejuice has coriander",
     "coriander" in PRODUCT_NAME_BLOCKERS.get("limejuice", set()), True)
# keyword_synonym_kokosflingor_kokosflakes: generated by dm matcher add keyword-synonym
test("Keyword synonym kokosflakes matches kokosflingor",
     match("Kokosflakes Ekologiska ICA I love eco", "kokosflakes", "pantry"), "kokosflingor")
test("Keyword synonym kokos-flakes matches via kokos (hyphen splits compound before synonym lookup)",
     match("Kokosflakes Ekologiska ICA I love eco", "kokos-flakes", "pantry"), "kokos")
# runtime_fpb_ost_balsamicostekt: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB ost has balsamicostekt",
     "balsamicostekt" in FALSE_POSITIVE_BLOCKERS.get("ost", set()), True)
# runtime_pnb_banana_bar: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB banana has bar",
     "bar" in PRODUCT_NAME_BLOCKERS.get("banana", set()), True)
test("PNB banana has energibar",
     "energibar" in PRODUCT_NAME_BLOCKERS.get("banana", set()), True)
# runtime_pnb_tomat_san_marzano: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB tomat has san marzano",
     "san marzano" in PRODUCT_NAME_BLOCKERS.get("tomat", set()), True)
test("PNB tomat has dop",
     "dop" in PRODUCT_NAME_BLOCKERS.get("tomat", set()), True)
# runtime_pnb_yoghurt_hallon: generated by dm matcher add pnb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB yoghurt has hallon",
     "hallon" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has jordgubb",
     "jordgubb" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has blåbär",
     "blåbär" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has körsbär",
     "körsbär" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has hjortron",
     "hjortron" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has havtorn",
     "havtorn" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has lingon",
     "lingon" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has fläder",
     "fläder" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has drottningbär",
     "drottningbär" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has persika",
     "persika" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has apelsin",
     "apelsin" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has citron",
     "citron" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has citrus",
     "citrus" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has lime",
     "lime" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has mango",
     "mango" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has ananas",
     "ananas" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has päron",
     "päron" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has kiwi",
     "kiwi" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has banan",
     "banan" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has plommon",
     "plommon" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has aprikos",
     "aprikos" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has choklad",
     "choklad" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has blandade smaker",
     "blandade smaker" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has fruktsmaker",
     "fruktsmaker" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has vår & sommar",
     "vår & sommar" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
test("PNB yoghurt has robby",
     "robby" in PRODUCT_NAME_BLOCKERS.get("yoghurt", set()), True)
# keyword_synonym_ciabatta_chiabatta: generated by dm matcher add keyword-synonym
test("Keyword synonym chiabatta matches ciabatta",
...
test("Salladslök knippe matches knipplök ingredient (canonical=salladslök via synonym)",
     match("Salladslök knippe 100g Klass 1", "3 knipplökar", "vegetables"), "salladslök")
...
# Q77/Q78: smörgåsgurka is isolated from the generic inlagdgurka family.
test("Smörgåsgurka matches its own canonical",
     match("Smörgåsgurka", "smörgåsgurka", "pantry"), "smörgåsgurka")
# keyword_synonym_smorgasgurka_smorgasgurkor: generated by dm matcher add keyword-synonym
test("Keyword synonym smörgåsgurkor matches smörgåsgurka",
     match("Smörgåsgurka 375g Kavli", "smörgåsgurkor", "pantry"), "smörgåsgurka")
# keyword_synonym_attiksgurka_attiksgurkor: generated by dm matcher add keyword-synonym
test("Keyword synonym ättiksgurkor matches ättiksgurka",
     match("Ättiksgurka 570g ICA", "ättiksgurkor", "pantry"), "ättiksgurka")
# runtime_fpb_inlagdgurka_smorgasgurka: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB inlagdgurka has smörgåsgurka",
     "smörgåsgurka" in FALSE_POSITIVE_BLOCKERS.get("inlagdgurka", set()), True)
test("FPB inlagdgurka has smorgasgurka",
     "smorgasgurka" in FALSE_POSITIVE_BLOCKERS.get("inlagdgurka", set()), True)
# runtime_fpb_inlagdgurka_attiksgurka: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB inlagdgurka has ättiksgurka",
     "ättiksgurka" in FALSE_POSITIVE_BLOCKERS.get("inlagdgurka", set()), True)
test("FPB inlagdgurka has attiksgurka",
     "attiksgurka" in FALSE_POSITIVE_BLOCKERS.get("inlagdgurka", set()), True)
# runtime_fpb_inlagdgurka_saltgurka: generated by dm matcher add fpb
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB inlagdgurka has saltgurka",
     "saltgurka" in FALSE_POSITIVE_BLOCKERS.get("inlagdgurka", set()), True)
# runtime_ksbc_chips_pommes: generated by dm matcher add ksbc
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC chips has pommes",
     "pommes" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("chips", set()), True)
# runtime_ksbc_paprika_snackpaprika: generated by dm matcher add ksbc
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC paprika has snackpaprika",
     "snackpaprika" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("paprika", set()), True)
# runtime_spice_fresh_rule_estragon: generated by dm matcher add spice-fresh-rule
from languages.sv.ingredient_matching.processed_rules import SPICE_VS_FRESH_RULES
test("spice-fresh-rule estragon dried_indicators torkad",
     "torkad" in SPICE_VS_FRESH_RULES.get("estragon", {}).get("dried_indicators", set()), True)
test("spice-fresh-rule estragon dried_indicators torkade",
     "torkade" in SPICE_VS_FRESH_RULES.get("estragon", {}).get("dried_indicators", set()), True)
test("spice-fresh-rule estragon fresh_product_words färsk",
     "färsk" in SPICE_VS_FRESH_RULES.get("estragon", {}).get("fresh_product_words", set()), True)
test("spice-fresh-rule estragon fresh_product_words färska",
     "färska" in SPICE_VS_FRESH_RULES.get("estragon", {}).get("fresh_product_words", set()), True)
# runtime_ksbc_inlagdgurka_saltgurka: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_inlagdgurka_saltgurka
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC inlagdgurka has saltgurka",
     "saltgurka" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("inlagdgurka", set()), True)
test("KSBC inlagdgurka has saltgurkor",
     "saltgurkor" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("inlagdgurka", set()), True)
test("KSBC inlagdgurka has smörgåsgurka",
     "smörgåsgurka" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("inlagdgurka", set()), True)
test("KSBC inlagdgurka has smorgasgurka",
     "smorgasgurka" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("inlagdgurka", set()), True)
test("KSBC inlagdgurka has ättiksgurka",
     "ättiksgurka" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("inlagdgurka", set()), True)
test("KSBC inlagdgurka has attiksgurka",
     "attiksgurka" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("inlagdgurka", set()), True)
# keyword_synonym_salladslok_knipplok: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_salladslok_knipplok
test("Keyword synonym knipplök matches salladslök",
     match("Salladslök 125g Klass 1 ICA", "knipplök", "vegetables"), "salladslök")
# keyword_synonym_salladslok_knipplokar: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_salladslok_knipplokar
test("Keyword synonym knipplökar matches salladslök",
     match("Salladslök 125g Klass 1 ICA", "knipplökar", "vegetables"), "salladslök")
# keyword_synonym_soja_soya: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_soja_soya
test("Keyword synonym soya matches soja",
     match("Japansk Soja 150ml Mrs Chengs", "soya", "pantry"), "soja")
# runtime_pnb_vitchoklad_glass: generated by dm matcher add pnb
# sanity-id: runtime_pnb_vitchoklad_glass
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB vitchoklad has glass",
     "glass" in PRODUCT_NAME_BLOCKERS.get("vitchoklad", set()), True)
# runtime_specialty_qualifier_bonor: generated by dm matcher add specialty-qualifier
# sanity-id: runtime_specialty_qualifier_bonor
from languages.sv.ingredient_matching.specialty_rules import SPECIALTY_QUALIFIERS
test("specialty-qualifier bönor has fermenterad",
     "fermenterad" in SPECIALTY_QUALIFIERS.get("bönor", []), True)
test("specialty-qualifier bönor has fermenterade",
     "fermenterade" in SPECIALTY_QUALIFIERS.get("bönor", []), True)
from languages.sv.ingredient_matching.specialty_rules import BIDIRECTIONAL_PER_KEYWORD
test("specialty bidirectional bönor has fermenterad",
     "fermenterad" in BIDIRECTIONAL_PER_KEYWORD.get("bönor", set()), True)
test("specialty bidirectional bönor has fermenterade",
     "fermenterade" in BIDIRECTIONAL_PER_KEYWORD.get("bönor", set()), True)
# runtime_specialty_qualifier_sojabonor: generated by dm matcher add specialty-qualifier
# sanity-id: runtime_specialty_qualifier_sojabonor
from languages.sv.ingredient_matching.specialty_rules import SPECIALTY_QUALIFIERS
test("specialty-qualifier sojabönor has fermenterad",
     "fermenterad" in SPECIALTY_QUALIFIERS.get("sojabönor", []), True)
test("specialty-qualifier sojabönor has fermenterade",
     "fermenterade" in SPECIALTY_QUALIFIERS.get("sojabönor", []), True)
from languages.sv.ingredient_matching.specialty_rules import BIDIRECTIONAL_PER_KEYWORD
test("specialty bidirectional sojabönor has fermenterad",
     "fermenterad" in BIDIRECTIONAL_PER_KEYWORD.get("sojabönor", set()), True)
test("specialty bidirectional sojabönor has fermenterade",
     "fermenterade" in BIDIRECTIONAL_PER_KEYWORD.get("sojabönor", set()), True)
# runtime_ksbc_flaskkarre_lufttorkad: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_flaskkarre_lufttorkad
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC fläskkarré has lufttorkad",
     "lufttorkad" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("fläskkarré", set()), True)
test("KSBC fläskkarré has lomo",
     "lomo" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("fläskkarré", set()), True)
# runtime_ksbc_must_mustigt: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_must_mustigt
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC must has mustigt",
     "mustigt" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("must", set()), True)
# runtime_fpb_pepparkaka_skumtomte: generated by dm matcher add fpb
# sanity-id: runtime_fpb_pepparkaka_skumtomte
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB pepparkaka has skumtomte",
     "skumtomte" in FALSE_POSITIVE_BLOCKERS.get("pepparkaka", set()), True)
# runtime_fpb_dillsas_gratang: generated by dm matcher add fpb
# sanity-id: runtime_fpb_dillsas_gratang
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB dillsås has gratäng",
     "gratäng" in FALSE_POSITIVE_BLOCKERS.get("dillsås", set()), True)
# runtime_pnb_plommontomater_kycklingpate: generated by dm matcher add pnb
# sanity-id: runtime_pnb_plommontomater_kycklingpate
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB plommontomater has kycklingpate",
     "kycklingpate" in PRODUCT_NAME_BLOCKERS.get("plommontomater", set()), True)
# runtime_pnb_kvisttomater_kycklingpate: generated by dm matcher add pnb
# sanity-id: runtime_pnb_kvisttomater_kycklingpate
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB kvisttomater has kycklingpate",
     "kycklingpate" in PRODUCT_NAME_BLOCKERS.get("kvisttomater", set()), True)
# runtime_pnb_plommontomat_kycklingpate: generated by dm matcher add pnb
# sanity-id: runtime_pnb_plommontomat_kycklingpate
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB plommontomat has kycklingpate",
     "kycklingpate" in PRODUCT_NAME_BLOCKERS.get("plommontomat", set()), True)
# runtime_pnb_kvisttomat_kycklingpate: generated by dm matcher add pnb
# sanity-id: runtime_pnb_kvisttomat_kycklingpate
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB kvisttomat has kycklingpate",
     "kycklingpate" in PRODUCT_NAME_BLOCKERS.get("kvisttomat", set()), True)
# runtime_pnb_halleflundra_panerad: generated by dm matcher add pnb
# sanity-id: runtime_pnb_halleflundra_panerad
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB hälleflundra has panerad",
     "panerad" in PRODUCT_NAME_BLOCKERS.get("hälleflundra", set()), True)
test("PNB hälleflundra has panerade",
     "panerade" in PRODUCT_NAME_BLOCKERS.get("hälleflundra", set()), True)
test("PNB hälleflundra has sprödpanerad",
     "sprödpanerad" in PRODUCT_NAME_BLOCKERS.get("hälleflundra", set()), True)
test("PNB hälleflundra has sprodpanerad",
     "sprodpanerad" in PRODUCT_NAME_BLOCKERS.get("hälleflundra", set()), True)
# runtime_pnb_karre_grillkryddad: generated by dm matcher add pnb
# sanity-id: runtime_pnb_karre_grillkryddad
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB karré has grillkryddad",
     "grillkryddad" in PRODUCT_NAME_BLOCKERS.get("karré", set()), True)
# Plain dairy 'grädde' ingredient matches plain plant-based cream (material is
# irrelevant for a plain ingredient per the substitute policy); only flavored
# plant-cream variants stay blocked. 'soja' is intentionally NOT a grädde PNB.
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB grädde does not block soja",
     "soja" in PRODUCT_NAME_BLOCKERS.get("grädde", set()), False)
test("plain grädde matches plain sojagrädde product",
     match("Matlagningsgrädde Soja 14% 250ml Alpro", "5 dl grädde", "dairy") is not None, True)
# runtime_pnb_vitchoklad_hallon: generated by dm matcher add pnb
# sanity-id: runtime_pnb_vitchoklad_hallon
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB vitchoklad has hallon",
     "hallon" in PRODUCT_NAME_BLOCKERS.get("vitchoklad", set()), True)
# runtime_pnb_margarin_proactiv: generated by dm matcher add pnb
# sanity-id: runtime_pnb_margarin_proactiv
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB margarin has proactiv",
     "proactiv" in PRODUCT_NAME_BLOCKERS.get("margarin", set()), True)
# runtime_pnb_bonor_laoganma: generated by dm matcher add pnb
# sanity-id: runtime_pnb_bonor_laoganma
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB bönor has laoganma",
     "laoganma" in PRODUCT_NAME_BLOCKERS.get("bönor", set()), True)
# runtime_pnb_bonor_fant: generated by dm matcher add pnb
# sanity-id: runtime_pnb_bonor_fant
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB bönor has fant",
     "fant" in PRODUCT_NAME_BLOCKERS.get("bönor", set()), True)
# keyword_synonym_lask_fruktsoda: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_lask_fruktsoda
test("Keyword synonym fruktsoda matches läsk",
     match("Läsk Hallonsoda 1,5l ICA", "fruktsoda", "beverages"), "läsk")
# policy_kycklingskinn_not_plain_kyckling_kyckling: generated by dm matcher add no-match-policy
# sanity-id: policy_kycklingskinn_not_plain_kyckling_kyckling
test("no-match-policy blocks Kycklingfilé Ca 700g Kronfågel",
     match("Kycklingfilé Ca 700g Kronfågel", "kycklingskinn", "meat"), None)
# runtime_pnb_lantbrod_kungsornen: generated by dm matcher add pnb
# sanity-id: runtime_pnb_lantbrod_kungsornen
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB lantbröd has kungsörnen",
     "kungsörnen" in PRODUCT_NAME_BLOCKERS.get("lantbröd", set()), True)
# runtime_pnb_surdegsbrod_kungsornen: generated by dm matcher add pnb
# sanity-id: runtime_pnb_surdegsbrod_kungsornen
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB surdegsbröd has kungsörnen",
     "kungsörnen" in PRODUCT_NAME_BLOCKERS.get("surdegsbröd", set()), True)
# runtime_pnb_ciabatta_kungsornen: generated by dm matcher add pnb
# sanity-id: runtime_pnb_ciabatta_kungsornen
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB ciabatta has kungsörnen",
     "kungsörnen" in PRODUCT_NAME_BLOCKERS.get("ciabatta", set()), True)
# runtime_pnb_mozzarella_scamorza: generated by dm matcher add pnb
# sanity-id: runtime_pnb_mozzarella_scamorza
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB mozzarella has scamorza",
     "scamorza" in PRODUCT_NAME_BLOCKERS.get("mozzarella", set()), True)
# runtime_ksbc_zucchini_butternut: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_zucchini_butternut
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC zucchini has butternut",
     "butternut" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("zucchini", set()), True)
test("KSBC zucchini has pumpa",
     "pumpa" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("zucchini", set()), True)
# runtime_ksbc_squash_butternut: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_squash_butternut
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC squash has butternut",
     "butternut" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("squash", set()), True)
test("KSBC squash has pumpa",
     "pumpa" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("squash", set()), True)
# runtime_ksbc_hummer_soppbas: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_hummer_soppbas
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC hummer has soppbas",
     "soppbas" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("hummer", set()), True)
# runtime_ksbc_morot_chips: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_morot_chips
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC morot has chips",
     "chips" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("morot", set()), True)
# runtime_ksbc_rotselleri_chips: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_rotselleri_chips
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC rotselleri has chips",
     "chips" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("rotselleri", set()), True)
# runtime_ksbc_kalrot_chips: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_kalrot_chips
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC kålrot has chips",
     "chips" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kålrot", set()), True)
# runtime_ksbc_palsternacka_chips: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_palsternacka_chips
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC palsternacka has chips",
     "chips" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("palsternacka", set()), True)
# runtime_ksbc_rodbeta_chips: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_rodbeta_chips
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC rödbeta has chips",
     "chips" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("rödbeta", set()), True)
# runtime_pnb_oliver_chili: generated by dm matcher add pnb
# sanity-id: runtime_pnb_oliver_chili
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB oliver has chili",
     "chili" in PRODUCT_NAME_BLOCKERS.get("oliver", set()), True)
test("PNB oliver has vitlök",
     "vitlök" in PRODUCT_NAME_BLOCKERS.get("oliver", set()), True)
test("PNB oliver has vitlök",
     "vitlök" in PRODUCT_NAME_BLOCKERS.get("oliver", set()), True)
test("PNB oliver has citron",
     "citron" in PRODUCT_NAME_BLOCKERS.get("oliver", set()), True)
test("PNB oliver has ört",
     "ört" in PRODUCT_NAME_BLOCKERS.get("oliver", set()), True)
test("PNB oliver has örter",
     "örter" in PRODUCT_NAME_BLOCKERS.get("oliver", set()), True)
test("PNB oliver has paprika",
     "paprika" in PRODUCT_NAME_BLOCKERS.get("oliver", set()), True)
test("PNB oliver has fetaost",
     "fetaost" in PRODUCT_NAME_BLOCKERS.get("oliver", set()), True)
test("PNB oliver has feta",
     "feta" in PRODUCT_NAME_BLOCKERS.get("oliver", set()), True)
test("PNB oliver has grillad",
     "grillad" in PRODUCT_NAME_BLOCKERS.get("oliver", set()), True)
test("PNB oliver has grillade",
     "grillade" in PRODUCT_NAME_BLOCKERS.get("oliver", set()), True)
test("PNB oliver has torkad",
     "torkad" in PRODUCT_NAME_BLOCKERS.get("oliver", set()), True)
test("PNB oliver has torkade",
     "torkade" in PRODUCT_NAME_BLOCKERS.get("oliver", set()), True)
# Q80-3: directional canonical override 'syrad grädde' -> 'syradgrädde'
# (same pattern as 'pepparrot på tub' -> 'pepparrotsvisp'). Both word orders
# (ingredient 'syrad grädde' and product 'Grädde Syrad') normalize to the same
# canonical so they match each other without exposing plain 'grädde'.
test("Q80-3 syrad grädde ingredient matches Grädde Syrad product",
     match("Grädde Syrad 30% 3dl Arla Ko", "2 dl syrad grädde", "dairy"), "syradgrädde")
test("Q80-3 plain grädde ingredient does not match Grädde Syrad product",
     match("Grädde Syrad 30% 3dl Arla Ko", "2 dl grädde", "dairy"), None)
test("Q80-3 plain grädde ingredient still matches Vispgrädde",
     match("Vispgrädde 36% 2dl Arla", "2 dl grädde", "dairy"), "grädde")
# runtime_ksbc_kladdkaka_mix: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_kladdkaka_mix
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC kladdkaka has mix",
     "mix" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kladdkaka", set()), True)
test("KSBC kladdkaka has kladdkakamix",
     "kladdkakamix" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kladdkaka", set()), True)
# runtime_ksbc_kaka_mix: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_kaka_mix
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC kaka has mix",
     "mix" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kaka", set()), True)
test("KSBC kaka has kakmix",
     "kakmix" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kaka", set()), True)
# runtime_ksbc_vasterbottensost_philadelphia: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_vasterbottensost_philadelphia
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC västerbottensost has philadelphia",
     "philadelphia" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("västerbottensost", set()), True)
# runtime_pnb_mozzarellaost_scamorza: generated by dm matcher add pnb
# sanity-id: runtime_pnb_mozzarellaost_scamorza
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB mozzarellaost has scamorza",
     "scamorza" in PRODUCT_NAME_BLOCKERS.get("mozzarellaost", set()), True)
# runtime_ksbc_chili_chilisas: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_chili_chilisas
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC chili has chilisås",
     "chilisås" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("chili", set()), True)
test("KSBC chili has chilisås",
     "chilisås" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("chili", set()), True)
# runtime_non_food_keyword_add_plastfolie: generated by dm matcher add non-food-keyword
# sanity-id: runtime_non_food_keyword_add_plastfolie
from languages.sv.ingredient_matching.keywords import NON_FOOD_KEYWORDS
test("non-food-keyword add plastfolie",
     "plastfolie" in NON_FOOD_KEYWORDS, True)
test("non-food-keyword filters product plastfolie",
     kw("plastfolie"), [])
# runtime_pnb_svartabonor_laoganma: generated by dm matcher add pnb
# sanity-id: runtime_pnb_svartabonor_laoganma
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB svartabönor has laoganma",
     "laoganma" in PRODUCT_NAME_BLOCKERS.get("svartabönor", set()), True)
test("PNB svartabönor has chiliolja",
     "chiliolja" in PRODUCT_NAME_BLOCKERS.get("svartabönor", set()), True)
# keyword_synonym_kronartskockshjarta_kronartskockhjarta: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_kronartskockshjarta_kronartskockhjarta
test("Keyword synonym kronärtskockhjärta matches kronärtskockshjärta",
     match("Kronärtskockshjärtan 390g ICA", "kronärtskockhjärta", "pantry"), "kronärtskockshjärta")
test("Keyword synonym kronärtskockhjärtan matches kronärtskockshjärta",
     match("Kronärtskockshjärtan 390g ICA", "kronärtskockhjärtan", "pantry"), "kronärtskockshjärta")
# keyword_synonym_rakor_rakstjartar: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_rakor_rakstjartar
test("Keyword synonym räkstjärtar matches räkor",
     match("Räkor i Lake 170g ICA", "räkstjärtar", "fish_seafood"), "räkor")
# runtime_fpb_sotpotatis_nudlar: generated by dm matcher add fpb
# sanity-id: runtime_fpb_sotpotatis_nudlar
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB sötpotatis has nudlar",
     "nudlar" in FALSE_POSITIVE_BLOCKERS.get("sötpotatis", set()), True)
test("FPB sötpotatis has sötpotatisnudlar",
     "sötpotatisnudlar" in FALSE_POSITIVE_BLOCKERS.get("sötpotatis", set()), True)
# runtime_pnb_flaskfileer_gronpepparsas: generated by dm matcher add pnb
# sanity-id: runtime_pnb_flaskfileer_gronpepparsas
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB fläskfileer has grönpepparsås",
     "grönpepparsås" in PRODUCT_NAME_BLOCKERS.get("fläskfileer", set()), True)
test("PNB fläskfileer has gronpepparsås",
     "gronpepparsås" in PRODUCT_NAME_BLOCKERS.get("fläskfileer", set()), True)
test("PNB fläskfileer has rökt",
     "rökt" in PRODUCT_NAME_BLOCKERS.get("fläskfileer", set()), True)
test("PNB fläskfileer has rokt",
     "rokt" in PRODUCT_NAME_BLOCKERS.get("fläskfileer", set()), True)
# runtime_pnb_digestive_oliv: generated by dm matcher add pnb
# sanity-id: runtime_pnb_digestive_oliv
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB digestive has oliv",
     "oliv" in PRODUCT_NAME_BLOCKERS.get("digestive", set()), True)
# runtime_non_food_keyword_add_blojhink: generated by dm matcher add non-food-keyword
# sanity-id: runtime_non_food_keyword_add_blojhink
from languages.sv.ingredient_matching.keywords import NON_FOOD_KEYWORDS
test("non-food-keyword add blöjhink",
     "blöjhink" in NON_FOOD_KEYWORDS, True)
test("non-food-keyword filters product blöjhink",
     kw("blöjhink"), [])
test("non-food-keyword add blöjhinksrefill",
     "blöjhinksrefill" in NON_FOOD_KEYWORDS, True)
test("non-food-keyword filters product blöjhinksrefill",
     kw("blöjhinksrefill"), [])
test("non-food-keyword add bakplåtspapper",
     "bakplåtspapper" in NON_FOOD_KEYWORDS, True)
test("non-food-keyword filters product bakplåtspapper",
     kw("bakplåtspapper"), [])
test("non-food-keyword add bakplatspapper",
     "bakplatspapper" in NON_FOOD_KEYWORDS, True)
test("non-food-keyword filters product bakplatspapper",
     kw("bakplatspapper"), [])
# runtime_ksbc_skinka_farsk: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_skinka_farsk
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC skinka has färsk",
     "färsk" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("skinka", set()), True)
# runtime_pnb_buljongpulver_dashi: generated by dm matcher add pnb
# sanity-id: runtime_pnb_buljongpulver_dashi
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB buljongpulver has dashi",
     "dashi" in PRODUCT_NAME_BLOCKERS.get("buljongpulver", set()), True)
# runtime_ksbc_formbrod_limpa: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_formbrod_limpa
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC formbröd has limpa",
     "limpa" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("formbröd", set()), True)
test("KSBC formbröd has rågbröd",
     "rågbröd" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("formbröd", set()), True)
test("KSBC formbröd has kavring",
     "kavring" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("formbröd", set()), True)
test("KSBC formbröd has skogaholmslimpa",
     "skogaholmslimpa" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("formbröd", set()), True)
# q83_2_yuzu_juice_synonym: generated by dm matcher add keyword-synonym
# sanity-id: q83_2_yuzu_juice_synonym
test("Keyword synonym yuzusaft matches yuzu",
     match("Yuzu juice 120ml Risberg", "yuzusaft", "pantry"), "yuzu")
test("Keyword synonym yuzujuice matches yuzu",
     match("Yuzu juice 120ml Risberg", "yuzujuice", "pantry"), "yuzu")
# q84_2_generic_kal_family_offer_keywords: generated by dm matcher add offer-extra-keyword
# sanity-id: q84_2_generic_kal_family_offer_keywords
test("offer-extra-keyword Vitkål Färsk ca 1,28kg Klass 1 ICA matches kål",
     match("Vitkål Färsk ca 1,28kg Klass 1 ICA", "1 dl kål, finstrimlad", "vegetables"), "kål")
test("Q83-2 yuzu- eller citronsaft accepts bottled yuzu juice",
     recipe_match_num(
         ["1 msk färskpressad yuzu- eller citronsaft"],
         {"name": "Yuzu juice 120ml Risberg", "category": "pantry", "brand": "Risberg"},
     ), 1)
test("Q83-5 plain potatis arm matches plain potato",
     recipe_match_num(
         ["400 g potatis eller sötpotatis"],
         {"name": "Potatis Klass 1 ICA", "category": "vegetables", "brand": "ICA"},
     ), 1)
test("Q83-5 sweet-potato arm still matches sötpotatis",
     recipe_match_num(
         ["400 g potatis eller sötpotatis"],
         {"name": "Sötpotatis Klass 1 ICA", "category": "vegetables", "brand": "ICA"},
     ), 1)
test("Q84-2 generic kål matches red cabbage family member",
     recipe_match_num(
         ["1 dl kål, finstrimlad"],
         {"name": "Rödkål ca 1,5kg Klass 1 ICA", "category": "vegetables", "brand": "ICA"},
     ), 1)
test("Q84-2 kålhuvud still blocks red cabbage through kål bridge",
     recipe_match_num(
         ["1 kålhuvud à 1 3/4 kg"],
         {"name": "Rödkål ca 1,5kg Klass 1 ICA", "category": "vegetables", "brand": "ICA"},
     ), 0)
test("Q85-8 Aperitivokex exact product matches brand line",
     recipe_match_num(
         ["1 frp Zeta Aperitivokex Ost & lök"],
         {"name": "Zeta Aperitivokex Ost & lök", "category": "snacks", "brand": "Zeta"},
     ), 1)
test("Q85-8 Aperitivokex flavor word does not match onion",
     recipe_match_num(
         ["1 frp Zeta Aperitivokex Ost & lök"],
         {"name": "Gul Lök Klass 1 ICA", "category": "vegetables", "brand": "ICA"},
     ), 0)
test("Q85-8 Aperitivokex flavor word does not match cheese",
     recipe_match_num(
         ["1 frp Zeta Aperitivokex Ost & lök"],
         {"name": "Prästost Arla", "category": "dairy", "brand": "Arla"},
     ), 0)
# runtime_pnb_sojagurt_jordgubb: generated by dm matcher add pnb
# sanity-id: runtime_pnb_sojagurt_jordgubb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB sojagurt has jordgubb",
     "jordgubb" in PRODUCT_NAME_BLOCKERS.get("sojagurt", set()), True)
test("PNB sojagurt has blåbär",
     "blåbär" in PRODUCT_NAME_BLOCKERS.get("sojagurt", set()), True)
test("PNB sojagurt has mango",
     "mango" in PRODUCT_NAME_BLOCKERS.get("sojagurt", set()), True)
test("PNB sojagurt has hallon",
     "hallon" in PRODUCT_NAME_BLOCKERS.get("sojagurt", set()), True)
test("PNB sojagurt has banan",
     "banan" in PRODUCT_NAME_BLOCKERS.get("sojagurt", set()), True)
test("PNB sojagurt has ananas",
     "ananas" in PRODUCT_NAME_BLOCKERS.get("sojagurt", set()), True)
test("PNB sojagurt has persika",
     "persika" in PRODUCT_NAME_BLOCKERS.get("sojagurt", set()), True)
test("PNB sojagurt has kiwi",
     "kiwi" in PRODUCT_NAME_BLOCKERS.get("sojagurt", set()), True)
test("PNB sojagurt has apelsin",
     "apelsin" in PRODUCT_NAME_BLOCKERS.get("sojagurt", set()), True)
test("PNB sojagurt has citron",
     "citron" in PRODUCT_NAME_BLOCKERS.get("sojagurt", set()), True)
test("PNB sojagurt has körsbär",
     "körsbär" in PRODUCT_NAME_BLOCKERS.get("sojagurt", set()), True)
test("PNB sojagurt has aprikos",
     "aprikos" in PRODUCT_NAME_BLOCKERS.get("sojagurt", set()), True)
test("PNB sojagurt has plommon",
     "plommon" in PRODUCT_NAME_BLOCKERS.get("sojagurt", set()), True)
test("PNB sojagurt has päron",
     "päron" in PRODUCT_NAME_BLOCKERS.get("sojagurt", set()), True)
test("PNB sojagurt has lingon",
     "lingon" in PRODUCT_NAME_BLOCKERS.get("sojagurt", set()), True)
test("PNB sojagurt has hjortron",
     "hjortron" in PRODUCT_NAME_BLOCKERS.get("sojagurt", set()), True)
test("PNB sojagurt has choklad",
     "choklad" in PRODUCT_NAME_BLOCKERS.get("sojagurt", set()), True)
test("PNB sojagurt has frukt",
     "frukt" in PRODUCT_NAME_BLOCKERS.get("sojagurt", set()), True)
test("Track A plain sojagurt blocks fruit-flavored sojagurt",
     recipe_match_num(["1 dl sojagurt naturell"], {"name": "Sojagurt Jordgubb 1,9% Oddlygood", "category": "dairy"}), 0)
test("Track A plain sojagurt still matches unflavored sojagurt",
     recipe_match_num(["1 dl sojagurt naturell"], {"name": "Sojagurt Naturell 1000g Oddlygood", "category": "dairy"}), 1)
# runtime_ksbc_korsbar_vedbit: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_korsbar_vedbit
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC körsbär has vedbit",
     "vedbit" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("körsbär", set()), True)
test("KSBC körsbär has vedbitar",
     "vedbitar" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("körsbär", set()), True)
test("KSBC körsbär has vedbitarna",
     "vedbitarna" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("körsbär", set()), True)
# ingredient_parent_ragbrod_grovlimpa: generated by dm matcher add ingredient-parent
# sanity-id: ingredient_parent_ragbrod_grovlimpa
test("ingredient-parent Rågbröd 500g KRAV Kung Markatta matches rågbröd",
     match("Rågbröd 500g KRAV Kung Markatta", "250 g grovlimpa", "pantry"), "grovlimpa")
# keyword_extra_parent_ragbrod_family: generated by dm matcher add keyword-extra-parent
# sanity-id: keyword_extra_parent_ragbrod_family
test("Rågbröd recipe matches kavring",
     match("Kavring", "250 g grovlimpa", "pantry"), "grovlimpa")
# runtime_ksbc_limpa_grov: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_limpa_grov
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC limpa has grov",
     "grov" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("limpa", set()), True)
test("KSBC limpa has grovlimpa",
     "grovlimpa" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("limpa", set()), True)
# runtime_ksbc_kal_rodkal: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_kal_rodkal
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC kål has rödkål",
     "rödkål" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kål", set()), True)
test("KSBC kål has rödkål",
     "rödkål" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kål", set()), True)
test("KSBC kål has vitkål",
     "vitkål" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kål", set()), True)
test("KSBC kål has vitkal",
     "vitkal" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kål", set()), True)
test("KSBC kål has spetskål",
     "spetskål" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kål", set()), True)
test("KSBC kål has spetskål",
     "spetskål" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kål", set()), True)
test("KSBC kål has grönkål",
     "grönkål" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kål", set()), True)
test("KSBC kål has gronkal",
     "gronkal" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kål", set()), True)
test("KSBC kål has svartkål",
     "svartkål" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kål", set()), True)
test("KSBC kål has svartkal",
     "svartkal" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kål", set()), True)
test("KSBC kål has savoykål",
     "savoykål" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kål", set()), True)
test("KSBC kål has savoykal",
     "savoykal" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kål", set()), True)
test("KSBC kål has savojkål",
     "savojkål" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kål", set()), True)
test("KSBC kål has savojkal",
     "savojkal" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kål", set()), True)
test("KSBC kål has salladskål",
     "salladskål" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kål", set()), True)
test("KSBC kål has salladskal",
     "salladskal" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kål", set()), True)
# runtime_ksbc_kal_brysselkal: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_kal_brysselkal
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC kål has brysselkål",
     "brysselkål" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kål", set()), True)
test("KSBC kål has brysselkal",
     "brysselkal" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kål", set()), True)
# runtime_ksbc_tandoori_nuggets: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_tandoori_nuggets
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC tandoori has nuggets",
     "nuggets" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("tandoori", set()), True)
test("KSBC tandoori has ärtnuggets",
     "ärtnuggets" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("tandoori", set()), True)
test("KSBC tandoori has artnuggets",
     "artnuggets" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("tandoori", set()), True)
# runtime_ksbc_kal_fankal: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_kal_fankal
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC kål has fänkål",
     "fänkål" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kål", set()), True)
test("KSBC kål has fänkål",
     "fänkål" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kål", set()), True)
# extraction_helper_aperitivokex: generated by dm matcher add extraction-helper
# sanity-id: extraction_helper_aperitivokex
test("KW 1 frp Zeta Aperitivokex Ost & lök ingredient -> aperitivokex", extract_keywords_from_ingredient("1 frp Zeta Aperitivokex Ost & lök"), ["aperitivokex"])
# runtime_pnb_pumpafron_musli: generated by dm matcher add pnb
# sanity-id: runtime_pnb_pumpafron_musli
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB pumpafrön has musli",
     "musli" in PRODUCT_NAME_BLOCKERS.get("pumpafrön", set()), True)
# runtime_pnb_matolja_vitlok: generated by dm matcher add pnb
# sanity-id: runtime_pnb_matolja_vitlok
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB matolja has vitlök",
     "vitlök" in PRODUCT_NAME_BLOCKERS.get("matolja", set()), True)
# runtime_pnb_kyckling_ortkryddad: generated by dm matcher add pnb
# sanity-id: runtime_pnb_kyckling_ortkryddad
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB kyckling has örtkryddad",
     "örtkryddad" in PRODUCT_NAME_BLOCKERS.get("kyckling", set()), True)
# runtime_pnb_kyckling_bbq: generated by dm matcher add pnb
# sanity-id: runtime_pnb_kyckling_bbq
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB kyckling has bbq",
     "bbq" in PRODUCT_NAME_BLOCKERS.get("kyckling", set()), True)
# runtime_stop_word_add_knivsudd: generated by dm matcher add stop-word
# sanity-id: runtime_stop_word_add_knivsudd
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add knivsudd",
     "knivsudd" in STOP_WORDS, True)
test("stop-word filters extraction knivsudd",
     "knivsudd" in kw("knivsudd"), False)
# runtime_stop_word_add_langden: generated by dm matcher add stop-word
# sanity-id: runtime_stop_word_add_langden
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add längden",
     "längden" in STOP_WORDS, True)
test("stop-word filters extraction längden",
     "längden" in kw("längden"), False)
test("stop-word add snedden",
     "snedden" in STOP_WORDS, True)
test("stop-word filters extraction snedden",
     "snedden" in kw("snedden"), False)
test("stop-word add stjälkarna",
     "stjälkarna" in STOP_WORDS, True)
test("stop-word filters extraction stjälkarna",
     "stjälkarna" in kw("stjälkarna"), False)
test("stop-word add finhacka",
     "finhacka" in STOP_WORDS, True)
test("stop-word filters extraction finhacka",
     "finhacka" in kw("finhacka"), False)
test("stop-word add strimla",
     "strimla" in STOP_WORDS, True)
test("stop-word filters extraction strimla",
     "strimla" in kw("strimla"), False)
test("stop-word add stammen",
     "stammen" in STOP_WORDS, True)
test("stop-word filters extraction stammen",
     "stammen" in kw("stammen"), False)
# runtime_ksbc_kal_blomkal: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_kal_blomkal
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC kål has blomkål",
     "blomkål" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kål", set()), True)
# runtime_pnb_kycklingfile_bbq: generated by dm matcher add pnb
# sanity-id: runtime_pnb_kycklingfile_bbq
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB kycklingfilé has bbq",
     "bbq" in PRODUCT_NAME_BLOCKERS.get("kycklingfilé", set()), True)
# runtime_pnb_kycklingfile_ortkryddad: generated by dm matcher add pnb
# sanity-id: runtime_pnb_kycklingfile_ortkryddad
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB kycklingfilé has örtkryddad",
     "örtkryddad" in PRODUCT_NAME_BLOCKERS.get("kycklingfilé", set()), True)
# policy_boveteflingor_not_raw_bovete_i_bbovete_b_flingor: generated by dm matcher add no-match-policy
# sanity-id: policy_boveteflingor_not_raw_bovete_i_bbovete_b_flingor
test("no-match-policy blocks Bovete 900g Nasha Kasha",
     match("Bovete 900g Nasha Kasha", "boveteflingor", "pantry"), None)
# runtime_pnb_kycklingfile_citron_chili: generated by dm matcher add pnb
# sanity-id: runtime_pnb_kycklingfile_citron_chili
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB kycklingfilé has citron/chili",
     "citron/chili" in PRODUCT_NAME_BLOCKERS.get("kycklingfilé", set()), True)
# runtime_pnb_kalrot_gronkal: generated by dm matcher add pnb
# sanity-id: runtime_pnb_kalrot_gronkal
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB kålrot has grönkål",
     "grönkål" in PRODUCT_NAME_BLOCKERS.get("kålrot", set()), True)
test("PNB kålrot has vitkål",
     "vitkål" in PRODUCT_NAME_BLOCKERS.get("kålrot", set()), True)
test("PNB kålrot has rödkål",
     "rödkål" in PRODUCT_NAME_BLOCKERS.get("kålrot", set()), True)
test("PNB kålrot has savoykål",
     "savoykål" in PRODUCT_NAME_BLOCKERS.get("kålrot", set()), True)
test("PNB kålrot has spetskål",
     "spetskål" in PRODUCT_NAME_BLOCKERS.get("kålrot", set()), True)
test("PNB kålrot has svartkål",
     "svartkål" in PRODUCT_NAME_BLOCKERS.get("kålrot", set()), True)
test("PNB kålrot has salladskål",
     "salladskål" in PRODUCT_NAME_BLOCKERS.get("kålrot", set()), True)
# runtime_pnb_parlcouscous_currysas: generated by dm matcher add pnb
# sanity-id: runtime_pnb_parlcouscous_currysas
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB pärlcouscous has currysås",
     "currysås" in PRODUCT_NAME_BLOCKERS.get("pärlcouscous", set()), True)
# salame vego-block removed: a plain "salame" ingredient may match vego variants
# (substitute policy — material is irrelevant for a plain ingredient).
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB salame does not block vegan",
     "vegan" in PRODUCT_NAME_BLOCKERS.get("salame", set()), False)
# runtime_pnb_fetaost_vaccaost: generated by dm matcher add pnb
# sanity-id: runtime_pnb_fetaost_vaccaost
# Q89-4: PNB set to status=inactive; vaccaost→fetaost alias kept instead
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB fetaost has vaccaost",
     "vaccaost" in PRODUCT_NAME_BLOCKERS.get("fetaost", set()), False)
# runtime_pnb_kycklingfile_kryddad: generated by dm matcher add pnb
# sanity-id: runtime_pnb_kycklingfile_kryddad
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB kycklingfilé has kryddad",
     "kryddad" in PRODUCT_NAME_BLOCKERS.get("kycklingfilé", set()), True)
# runtime_stop_word_add_ljummet: generated by dm matcher add stop-word
# sanity-id: runtime_stop_word_add_ljummet
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add ljummet",
     "ljummet" in STOP_WORDS, True)
test("stop-word filters extraction ljummet",
     "ljummet" in kw("ljummet"), False)
# runtime_stop_word_add_frittering: generated by dm matcher add stop-word
# sanity-id: runtime_stop_word_add_frittering
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add frittering",
     "frittering" in STOP_WORDS, True)
test("stop-word filters extraction frittering",
     "frittering" in kw("frittering"), False)
# runtime_fpb_acai_bowl: generated by dm matcher add fpb
# sanity-id: runtime_fpb_acai_bowl
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB acai has bowl",
     "bowl" in FALSE_POSITIVE_BLOCKERS.get("acai", set()), True)
# runtime_fpb_revbensspjall_kickin: generated by dm matcher add fpb
# sanity-id: runtime_fpb_revbensspjall_kickin
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB revbensspjäll has kickin",
     "kickin" in FALSE_POSITIVE_BLOCKERS.get("revbensspjäll", set()), True)
# runtime_specialty_qualifier_kapris: generated by dm matcher add specialty-qualifier
# sanity-id: runtime_specialty_qualifier_kapris
from languages.sv.ingredient_matching.specialty_rules import SPECIALTY_QUALIFIERS
test("specialty-qualifier kapris has cocktail",
     "cocktail" in SPECIALTY_QUALIFIERS.get("kapris", []), True)
from languages.sv.ingredient_matching.specialty_rules import BIDIRECTIONAL_PER_KEYWORD
test("specialty bidirectional kapris has cocktail",
     "cocktail" in BIDIRECTIONAL_PER_KEYWORD.get("kapris", set()), True)
# runtime_pnb_flaskkotlett_grillkryddad: generated by dm matcher add pnb
# sanity-id: runtime_pnb_flaskkotlett_grillkryddad
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB fläskkotlett has grillkryddad",
     "grillkryddad" in PRODUCT_NAME_BLOCKERS.get("fläskkotlett", set()), True)
# runtime_pnb_kotlett_grillkryddad: generated by dm matcher add pnb
# sanity-id: runtime_pnb_kotlett_grillkryddad
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB kotlett has grillkryddad",
     "grillkryddad" in PRODUCT_NAME_BLOCKERS.get("kotlett", set()), True)
# runtime_pnb_cashew_smoked: generated by dm matcher add pnb
# sanity-id: runtime_pnb_cashew_smoked
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB cashew has smoked",
     "smoked" in PRODUCT_NAME_BLOCKERS.get("cashew", set()), True)
test("PNB cashew has mexicana",
     "mexicana" in PRODUCT_NAME_BLOCKERS.get("cashew", set()), True)
# runtime_pnb_cashewnotter_mexicana: generated by dm matcher add pnb
# sanity-id: runtime_pnb_cashewnotter_mexicana
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB cashewnötter has mexicana",
     "mexicana" in PRODUCT_NAME_BLOCKERS.get("cashewnötter", set()), True)
test("PNB cashewnötter has smoked",
     "smoked" in PRODUCT_NAME_BLOCKERS.get("cashewnötter", set()), True)
# runtime_specialty_qualifier_tapenade: generated by dm matcher add specialty-qualifier
# sanity-id: runtime_specialty_qualifier_tapenade
from languages.sv.ingredient_matching.specialty_rules import SPECIALTY_QUALIFIERS
test("specialty-qualifier tapenade has soltorkad",
     "soltorkad" in SPECIALTY_QUALIFIERS.get("tapenade", []), True)
test("specialty-qualifier tapenade has tomat",
     "tomat" in SPECIALTY_QUALIFIERS.get("tapenade", []), True)
from languages.sv.ingredient_matching.specialty_rules import BIDIRECTIONAL_PER_KEYWORD
test("specialty bidirectional tapenade has soltorkad",
     "soltorkad" in BIDIRECTIONAL_PER_KEYWORD.get("tapenade", set()), True)
test("specialty bidirectional tapenade has tomat",
     "tomat" in BIDIRECTIONAL_PER_KEYWORD.get("tapenade", set()), True)
# runtime_pnb_havregurt_jordgubb: generated by dm matcher add pnb
# sanity-id: runtime_pnb_havregurt_jordgubb
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB havregurt has jordgubb",
     "jordgubb" in PRODUCT_NAME_BLOCKERS.get("havregurt", set()), True)
test("PNB havregurt has blåbär",
     "blåbär" in PRODUCT_NAME_BLOCKERS.get("havregurt", set()), True)
test("PNB havregurt has mango",
     "mango" in PRODUCT_NAME_BLOCKERS.get("havregurt", set()), True)
test("PNB havregurt has hallon",
     "hallon" in PRODUCT_NAME_BLOCKERS.get("havregurt", set()), True)
test("PNB havregurt has banan",
     "banan" in PRODUCT_NAME_BLOCKERS.get("havregurt", set()), True)
test("PNB havregurt has ananas",
     "ananas" in PRODUCT_NAME_BLOCKERS.get("havregurt", set()), True)
test("PNB havregurt has persika",
     "persika" in PRODUCT_NAME_BLOCKERS.get("havregurt", set()), True)
test("PNB havregurt has kiwi",
     "kiwi" in PRODUCT_NAME_BLOCKERS.get("havregurt", set()), True)
test("PNB havregurt has apelsin",
     "apelsin" in PRODUCT_NAME_BLOCKERS.get("havregurt", set()), True)
test("PNB havregurt has citron",
     "citron" in PRODUCT_NAME_BLOCKERS.get("havregurt", set()), True)
test("PNB havregurt has körsbär",
     "körsbär" in PRODUCT_NAME_BLOCKERS.get("havregurt", set()), True)
test("PNB havregurt has aprikos",
     "aprikos" in PRODUCT_NAME_BLOCKERS.get("havregurt", set()), True)
test("PNB havregurt has plommon",
     "plommon" in PRODUCT_NAME_BLOCKERS.get("havregurt", set()), True)
test("PNB havregurt has päron",
     "päron" in PRODUCT_NAME_BLOCKERS.get("havregurt", set()), True)
test("PNB havregurt has lingon",
     "lingon" in PRODUCT_NAME_BLOCKERS.get("havregurt", set()), True)
test("PNB havregurt has hjortron",
     "hjortron" in PRODUCT_NAME_BLOCKERS.get("havregurt", set()), True)
test("PNB havregurt has choklad",
     "choklad" in PRODUCT_NAME_BLOCKERS.get("havregurt", set()), True)
test("PNB havregurt has frukt",
     "frukt" in PRODUCT_NAME_BLOCKERS.get("havregurt", set()), True)
test("PNB havregurt has ugnsäpple",
     "ugnsäpple" in PRODUCT_NAME_BLOCKERS.get("havregurt", set()), True)
test("PNB havregurt has äpple",
     "äpple" in PRODUCT_NAME_BLOCKERS.get("havregurt", set()), True)
# runtime_pnb_appeljuice_hallon: generated by dm matcher add pnb
# sanity-id: runtime_pnb_appeljuice_hallon
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB äppeljuice has hallon",
     "hallon" in PRODUCT_NAME_BLOCKERS.get("äppeljuice", set()), True)
test("PNB äppeljuice has jordgubb",
     "jordgubb" in PRODUCT_NAME_BLOCKERS.get("äppeljuice", set()), True)
test("PNB äppeljuice has blåbär",
     "blåbär" in PRODUCT_NAME_BLOCKERS.get("äppeljuice", set()), True)
test("PNB äppeljuice has mango",
     "mango" in PRODUCT_NAME_BLOCKERS.get("äppeljuice", set()), True)
test("PNB äppeljuice has päron",
     "päron" in PRODUCT_NAME_BLOCKERS.get("äppeljuice", set()), True)
test("PNB äppeljuice has banan",
     "banan" in PRODUCT_NAME_BLOCKERS.get("äppeljuice", set()), True)
test("PNB äppeljuice has ananas",
     "ananas" in PRODUCT_NAME_BLOCKERS.get("äppeljuice", set()), True)
test("PNB äppeljuice has persika",
     "persika" in PRODUCT_NAME_BLOCKERS.get("äppeljuice", set()), True)
test("PNB äppeljuice has kiwi",
     "kiwi" in PRODUCT_NAME_BLOCKERS.get("äppeljuice", set()), True)
test("PNB äppeljuice has citron",
     "citron" in PRODUCT_NAME_BLOCKERS.get("äppeljuice", set()), True)
test("PNB äppeljuice has lime",
     "lime" in PRODUCT_NAME_BLOCKERS.get("äppeljuice", set()), True)
test("PNB äppeljuice has apelsin",
     "apelsin" in PRODUCT_NAME_BLOCKERS.get("äppeljuice", set()), True)
test("PNB äppeljuice has körsbär",
     "körsbär" in PRODUCT_NAME_BLOCKERS.get("äppeljuice", set()), True)
test("PNB äppeljuice has aprikos",
     "aprikos" in PRODUCT_NAME_BLOCKERS.get("äppeljuice", set()), True)
test("PNB äppeljuice has plommon",
     "plommon" in PRODUCT_NAME_BLOCKERS.get("äppeljuice", set()), True)
test("PNB äppeljuice has lingon",
     "lingon" in PRODUCT_NAME_BLOCKERS.get("äppeljuice", set()), True)
test("PNB äppeljuice has hjortron",
     "hjortron" in PRODUCT_NAME_BLOCKERS.get("äppeljuice", set()), True)
test("PNB äppeljuice has fläder",
     "fläder" in PRODUCT_NAME_BLOCKERS.get("äppeljuice", set()), True)
test("PNB äppeljuice has svartvinbär",
     "svartvinbär" in PRODUCT_NAME_BLOCKERS.get("äppeljuice", set()), True)
test("PNB äppeljuice has röda vinbär",
     "röda vinbär" in PRODUCT_NAME_BLOCKERS.get("äppeljuice", set()), True)
test("PNB äppeljuice has multifrukt",
     "multifrukt" in PRODUCT_NAME_BLOCKERS.get("äppeljuice", set()), True)
# runtime_specialty_qualifier_couscous: generated by dm matcher add specialty-qualifier
# sanity-id: runtime_specialty_qualifier_couscous
from languages.sv.ingredient_matching.specialty_rules import SPECIALTY_QUALIFIERS
test("specialty-qualifier couscous has pärl",
     "pärl" in SPECIALTY_QUALIFIERS.get("couscous", []), True)
from languages.sv.ingredient_matching.specialty_rules import BIDIRECTIONAL_PER_KEYWORD
test("specialty bidirectional couscous has pärl",
     "pärl" in BIDIRECTIONAL_PER_KEYWORD.get("couscous", set()), True)
# runtime_specialty_qualifier_brod: generated by dm matcher add specialty-qualifier
# sanity-id: runtime_specialty_qualifier_brod
from languages.sv.ingredient_matching.specialty_rules import SPECIALTY_QUALIFIERS
test("specialty-qualifier bröd has mörk",
     "mörk" in SPECIALTY_QUALIFIERS.get("bröd", []), True)
test("specialty-qualifier bröd has mörkt",
     "mörkt" in SPECIALTY_QUALIFIERS.get("bröd", []), True)
test("specialty-qualifier bröd has råg",
     "råg" in SPECIALTY_QUALIFIERS.get("bröd", []), True)
test("specialty-qualifier bröd has kavring",
     "kavring" in SPECIALTY_QUALIFIERS.get("bröd", []), True)
test("specialty-qualifier bröd has fullkorn",
     "fullkorn" in SPECIALTY_QUALIFIERS.get("bröd", []), True)
test("specialty-qualifier bröd has grov",
     "grov" in SPECIALTY_QUALIFIERS.get("bröd", []), True)
from languages.sv.ingredient_matching.specialty_rules import BIDIRECTIONAL_PER_KEYWORD
test("specialty bidirectional bröd has mörk",
     "mörk" in BIDIRECTIONAL_PER_KEYWORD.get("bröd", set()), True)
test("specialty bidirectional bröd has mörkt",
     "mörkt" in BIDIRECTIONAL_PER_KEYWORD.get("bröd", set()), True)
test("specialty bidirectional bröd has råg",
     "råg" in BIDIRECTIONAL_PER_KEYWORD.get("bröd", set()), True)
test("specialty bidirectional bröd has kavring",
     "kavring" in BIDIRECTIONAL_PER_KEYWORD.get("bröd", set()), True)
test("specialty bidirectional bröd has fullkorn",
     "fullkorn" in BIDIRECTIONAL_PER_KEYWORD.get("bröd", set()), True)
test("specialty bidirectional bröd has grov",
     "grov" in BIDIRECTIONAL_PER_KEYWORD.get("bröd", set()), True)
# runtime_specialty_qualifier_bonor: generated by dm matcher add specialty-qualifier
# sanity-id: runtime_specialty_qualifier_bonor
from languages.sv.ingredient_matching.specialty_rules import SPECIALTY_QUALIFIERS
test("specialty-qualifier bönor has vit",
     "vit" in SPECIALTY_QUALIFIERS.get("bönor", []), True)
test("specialty-qualifier bönor has vita",
     "vita" in SPECIALTY_QUALIFIERS.get("bönor", []), True)
test("specialty-qualifier bönor has röd",
     "röd" in SPECIALTY_QUALIFIERS.get("bönor", []), True)
test("specialty-qualifier bönor has röda",
     "röda" in SPECIALTY_QUALIFIERS.get("bönor", []), True)
test("specialty-qualifier bönor has kidney",
     "kidney" in SPECIALTY_QUALIFIERS.get("bönor", []), True)
test("specialty-qualifier bönor has svart",
     "svart" in SPECIALTY_QUALIFIERS.get("bönor", []), True)
test("specialty-qualifier bönor has svarta",
     "svarta" in SPECIALTY_QUALIFIERS.get("bönor", []), True)
test("specialty-qualifier bönor has brun",
     "brun" in SPECIALTY_QUALIFIERS.get("bönor", []), True)
test("specialty-qualifier bönor has bruna",
     "bruna" in SPECIALTY_QUALIFIERS.get("bönor", []), True)
test("specialty-qualifier bönor has grön",
     "grön" in SPECIALTY_QUALIFIERS.get("bönor", []), True)
test("specialty-qualifier bönor has gröna",
     "gröna" in SPECIALTY_QUALIFIERS.get("bönor", []), True)
from languages.sv.ingredient_matching.specialty_rules import BIDIRECTIONAL_PER_KEYWORD
test("specialty bidirectional bönor has vit",
     "vit" in BIDIRECTIONAL_PER_KEYWORD.get("bönor", set()), True)
test("specialty bidirectional bönor has vita",
     "vita" in BIDIRECTIONAL_PER_KEYWORD.get("bönor", set()), True)
test("specialty bidirectional bönor has röd",
     "röd" in BIDIRECTIONAL_PER_KEYWORD.get("bönor", set()), True)
test("specialty bidirectional bönor has röda",
     "röda" in BIDIRECTIONAL_PER_KEYWORD.get("bönor", set()), True)
test("specialty bidirectional bönor has kidney",
     "kidney" in BIDIRECTIONAL_PER_KEYWORD.get("bönor", set()), True)
test("specialty bidirectional bönor has svart",
     "svart" in BIDIRECTIONAL_PER_KEYWORD.get("bönor", set()), True)
test("specialty bidirectional bönor has svarta",
     "svarta" in BIDIRECTIONAL_PER_KEYWORD.get("bönor", set()), True)
test("specialty bidirectional bönor has brun",
     "brun" in BIDIRECTIONAL_PER_KEYWORD.get("bönor", set()), True)
test("specialty bidirectional bönor has bruna",
     "bruna" in BIDIRECTIONAL_PER_KEYWORD.get("bönor", set()), True)
test("specialty bidirectional bönor has grön",
     "grön" in BIDIRECTIONAL_PER_KEYWORD.get("bönor", set()), True)
test("specialty bidirectional bönor has gröna",
     "gröna" in BIDIRECTIONAL_PER_KEYWORD.get("bönor", set()), True)
# keyword_synonym_peperoni_fefferoni: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_peperoni_fefferoni
test("Keyword synonym fefferoni matches peperoni",
     match("Peperoni Mild 600g Sevan", "fefferoni", "pantry"), "peperoni")
test("Keyword synonym fefferonis matches peperoni",
     match("Peperoni Mild 600g Sevan", "fefferonis", "pantry"), "peperoni")
# runtime_pnb_kalrot_kal: removed — "kål" moved from PNB to FPB (see runtime_fpb_kal_kalrot)
# PNB on kålrot→kål did not block correctly because match fires on product keyword "kål",
# not ingredient keyword "kålrot". Correct fix is FPB kål→kålrot (ingredient-side context).
# runtime_pnb_kottfars_salsiccia: generated by dm matcher add pnb
# sanity-id: runtime_pnb_kottfars_salsiccia
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB köttfärs has salsiccia",
     "salsiccia" in PRODUCT_NAME_BLOCKERS.get("köttfärs", set()), True)
# runtime_pnb_fars_salsiccia: generated by dm matcher add pnb
# sanity-id: runtime_pnb_fars_salsiccia
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB färs has salsiccia",
     "salsiccia" in PRODUCT_NAME_BLOCKERS.get("färs", set()), True)
# runtime_fpb_selleri_sellerisalt: generated by dm matcher add fpb
# sanity-id: runtime_fpb_selleri_sellerisalt
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB selleri has sellerisalt",
     "sellerisalt" in FALSE_POSITIVE_BLOCKERS.get("selleri", set()), True)
# runtime_fpb_kal_kalrot: generated by dm matcher add fpb
# sanity-id: runtime_fpb_kal_kalrot
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB kål has kålrot",
     "kålrot" in FALSE_POSITIVE_BLOCKERS.get("kål", set()), True)
# runtime_fpb_citron_citronmajonnas: generated by dm matcher add fpb
# sanity-id: runtime_fpb_citron_citronmajonnas
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB citron has citronmajonnäs",
     "citronmajonnäs" in FALSE_POSITIVE_BLOCKERS.get("citron", set()), True)
# runtime_pnb_wrap_asian: generated by dm matcher add pnb
# sanity-id: runtime_pnb_wrap_asian
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB wrap has asian",
     "asian" in PRODUCT_NAME_BLOCKERS.get("wrap", set()), True)
# runtime_ksbc_lax_rimmad: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_lax_rimmad
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC lax has rimmad",
     "rimmad" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("lax", set()), True)
# runtime_fpb_formbrod_fullkorns: generated by dm matcher add fpb
# sanity-id: runtime_fpb_formbrod_fullkorns
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB formbröd has fullkorns",
     "fullkorns" in FALSE_POSITIVE_BLOCKERS.get("formbröd", set()), True)
# runtime_pnb_soygurt_persika: generated by dm matcher add pnb
# sanity-id: runtime_pnb_soygurt_persika
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB soygurt has persika",
     "persika" in PRODUCT_NAME_BLOCKERS.get("soygurt", set()), True)
test("PNB soygurt has hallon",
     "hallon" in PRODUCT_NAME_BLOCKERS.get("soygurt", set()), True)
test("PNB soygurt has jordgubb",
     "jordgubb" in PRODUCT_NAME_BLOCKERS.get("soygurt", set()), True)
test("PNB soygurt has blåbär",
     "blåbär" in PRODUCT_NAME_BLOCKERS.get("soygurt", set()), True)
test("PNB soygurt has mango",
     "mango" in PRODUCT_NAME_BLOCKERS.get("soygurt", set()), True)
test("PNB soygurt has banan",
     "banan" in PRODUCT_NAME_BLOCKERS.get("soygurt", set()), True)
test("PNB soygurt has ananas",
     "ananas" in PRODUCT_NAME_BLOCKERS.get("soygurt", set()), True)
test("PNB soygurt has kiwi",
     "kiwi" in PRODUCT_NAME_BLOCKERS.get("soygurt", set()), True)
test("PNB soygurt has apelsin",
     "apelsin" in PRODUCT_NAME_BLOCKERS.get("soygurt", set()), True)
test("PNB soygurt has citron",
     "citron" in PRODUCT_NAME_BLOCKERS.get("soygurt", set()), True)
test("PNB soygurt has körsbär",
     "körsbär" in PRODUCT_NAME_BLOCKERS.get("soygurt", set()), True)
test("PNB soygurt has aprikos",
     "aprikos" in PRODUCT_NAME_BLOCKERS.get("soygurt", set()), True)
test("PNB soygurt has plommon",
     "plommon" in PRODUCT_NAME_BLOCKERS.get("soygurt", set()), True)
test("PNB soygurt has päron",
     "päron" in PRODUCT_NAME_BLOCKERS.get("soygurt", set()), True)
test("PNB soygurt has lingon",
     "lingon" in PRODUCT_NAME_BLOCKERS.get("soygurt", set()), True)
test("PNB soygurt has hjortron",
     "hjortron" in PRODUCT_NAME_BLOCKERS.get("soygurt", set()), True)
test("PNB soygurt has choklad",
     "choklad" in PRODUCT_NAME_BLOCKERS.get("soygurt", set()), True)
test("PNB soygurt has frukt",
     "frukt" in PRODUCT_NAME_BLOCKERS.get("soygurt", set()), True)
# parent_match_only_falukorv_falukorvsring: generated by dm matcher add parent-match-only
# sanity-id: parent_match_only_falukorv_falukorvsring
test("parent-match-only Falukorv 800g ICA matches falukorv",
     match("Falukorv 800g ICA", "Falukorv", "pantry"), "falukorv")
# runtime_fpb_tomat_tomattapenad: generated by dm matcher add fpb
# sanity-id: runtime_fpb_tomat_tomattapenad
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB tomat has tomattapenad",
     "tomattapenad" in FALSE_POSITIVE_BLOCKERS.get("tomat", set()), True)
# keyword_synonym_tapenade_tapenad: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_tapenade_tapenad
test("Keyword synonym tapenad matches tapenade",
     match("Tapenade av Oliver Svarta 140g Zeta", "tapenad", "pantry"), "tapenade")
test("Keyword synonym tomattapenad matches tapenade",
     match("Tapenade av Oliver Svarta 140g Zeta", "tomattapenad", "pantry"), "tapenade")
test("Keyword synonym olivtapenad matches tapenade",
     match("Tapenade av Oliver Svarta 140g Zeta", "olivtapenad", "pantry"), "tapenade")
# runtime_stop_word_add_fardigskivad: generated by dm matcher add stop-word
# sanity-id: runtime_stop_word_add_fardigskivad
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add färdigskivad",
     "färdigskivad" in STOP_WORDS, True)
test("stop-word filters extraction färdigskivad",
     "färdigskivad" in kw("färdigskivad"), False)
test("stop-word add färdigskivade",
     "färdigskivade" in STOP_WORDS, True)
test("stop-word filters extraction färdigskivade",
     "färdigskivade" in kw("färdigskivade"), False)
test("stop-word add färdigskivat",
     "färdigskivat" in STOP_WORDS, True)
test("stop-word filters extraction färdigskivat",
     "färdigskivat" in kw("färdigskivat"), False)
# runtime_pnb_kaviar_sill: generated by dm matcher add pnb
# sanity-id: runtime_pnb_kaviar_sill
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB kaviar has sill",
     "sill" in PRODUCT_NAME_BLOCKERS.get("kaviar", set()), True)
# runtime_pnb_ceviche_sill: generated by dm matcher add pnb
# sanity-id: runtime_pnb_ceviche_sill
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB ceviche has sill",
     "sill" in PRODUCT_NAME_BLOCKERS.get("ceviche", set()), True)
# runtime_pnb_alspansrokt_kalkon: generated by dm matcher add pnb
# sanity-id: runtime_pnb_alspansrokt_kalkon
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB alspånsrökt has kalkon",
     "kalkon" in PRODUCT_NAME_BLOCKERS.get("alspånsrökt", set()), True)
# runtime_pnb_alspansrokt_tempeh: generated by dm matcher add pnb
# sanity-id: runtime_pnb_alspansrokt_tempeh
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB alspånsrökt has tempeh",
     "tempeh" in PRODUCT_NAME_BLOCKERS.get("alspånsrökt", set()), True)
# runtime_pnb_rimmad_oxbringa: generated by dm matcher add pnb
# sanity-id: runtime_pnb_rimmad_oxbringa
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB rimmad has oxbringa",
     "oxbringa" in PRODUCT_NAME_BLOCKERS.get("rimmad", set()), True)
# runtime_fpb_kal_kalrabbi: generated by dm matcher add fpb
# sanity-id: runtime_fpb_kal_kalrabbi
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB kål has kålrabbi",
     "kålrabbi" in FALSE_POSITIVE_BLOCKERS.get("kål", set()), True)
# runtime_pnb_sillfile_5_minuters: generated by dm matcher add pnb
# sanity-id: runtime_pnb_sillfile_5_minuters
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB sillfilé has 5-minuters",
     "5-minuters" in PRODUCT_NAME_BLOCKERS.get("sillfilé", set()), True)
...
test("parent-match-only Salsiccia Färsk 200g Ingelsta matches salsiccia",
     match("Salsiccia Färsk 200g Ingelsta", "Salsiccia", "pantry"), "salsiccia")
# ingredient_parent_mjolk_lattmjolk: generated by dm matcher add ingredient-parent
# sanity-id: ingredient_parent_mjolk_lattmjolk
test("ingredient-parent Mellanmjölk 1,5% 1l ICA matches mjölk",
     match("Mellanmjölk 1,5% 1l ICA", "Lättmjölk", "pantry"), "lättmjölk")
# ingredient_parent_mjolk_mellanmjolk: generated by dm matcher add ingredient-parent
# sanity-id: ingredient_parent_mjolk_mellanmjolk
test("ingredient-parent Lättmjölk 0,5% 1l ICA matches mjölk",
     match("Lättmjölk 0,5% 1l ICA", "Mellanmjölk", "pantry"), "mellanmjölk")
# runtime_space_normalization_svart_eller_svart_eller: generated by dm matcher add space-normalization
...
# NOTE 2026-05-26: KSBC korv now intentionally INCLUDES specific sausage-type
# names (chorizo, salsiccia, kabanoss, bratwurst, falukorv etc) per a deliberate
# policy expansion — when a recipe specifies a sausage type, only matching
# products should surface, not the full 220-product korv family.
test("KSBC korv has plain falukorv (deliberate expansion)",
     "falukorv" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korv", set()), True)
...
# keyword_synonym_matjessill_matjesfile: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_matjessill_matjesfile
test("Keyword synonym matjesfilé matches matjessill",
     match("Matjessill 215g Klädesholmen", "matjesfilé", "pantry"), "matjessill")
test("Keyword synonym matjesfileer matches matjessill",
     match("Matjessill 215g Klädesholmen", "matjesfileer", "pantry"), "matjessill")
test("Keyword synonym bohusmatjes matches matjessill",
     match("Matjessill 215g Klädesholmen", "bohusmatjes", "pantry"), "matjessill")
test("Keyword synonym mästarmatjes matches matjessill",
     match("Matjessill 215g Klädesholmen", "mästarmatjes", "pantry"), "matjessill")
test("Keyword synonym matjessillfilé matches matjessill",
     match("Matjessill 215g Klädesholmen", "matjessillfilé", "pantry"), "matjessill")
# runtime_fpb_kal_skalla: generated by dm matcher add fpb
# sanity-id: runtime_fpb_kal_skalla
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB kål has skålla",
     "skålla" in FALSE_POSITIVE_BLOCKERS.get("kål", set()), True)
test("FPB kål has skålla mandel",
     "skålla mandel" in FALSE_POSITIVE_BLOCKERS.get("kål", set()), True)
test("FPB kål has skåll",
     "skåll" in FALSE_POSITIVE_BLOCKERS.get("kål", set()), True)
# runtime_pnb_majsena_redning: generated by dm matcher add pnb
# sanity-id: runtime_pnb_majsena_redning
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB majsena has redning",
     "redning" in PRODUCT_NAME_BLOCKERS.get("majsena", set()), True)
# runtime_ksbc_pasta_sfoglia: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_pasta_sfoglia
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC pasta has sfoglia",
     "sfoglia" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("pasta", set()), True)
# runtime_non_food_keyword_add_glasburk: generated by dm matcher add non-food-keyword
# sanity-id: runtime_non_food_keyword_add_glasburk
from languages.sv.ingredient_matching.keywords import NON_FOOD_KEYWORDS
test("non-food-keyword add glasburk",
     "glasburk" in NON_FOOD_KEYWORDS, True)
test("non-food-keyword filters product glasburk",
     kw("glasburk"), [])
test("non-food-keyword add glasburkar",
     "glasburkar" in NON_FOOD_KEYWORDS, True)
test("non-food-keyword filters product glasburkar",
     kw("glasburkar"), [])
# runtime_pnb_ryggbit_clarias: generated by dm matcher add pnb
# sanity-id: runtime_pnb_ryggbit_clarias
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB ryggbit has clarias",
     "clarias" in PRODUCT_NAME_BLOCKERS.get("ryggbit", set()), True)
# runtime_pnb_vetesurdeg_levainfralla: generated by dm matcher add pnb
# sanity-id: runtime_pnb_vetesurdeg_levainfralla
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB vetesurdeg has levainfralla",
     "levainfralla" in PRODUCT_NAME_BLOCKERS.get("vetesurdeg", set()), True)
test("PNB vetesurdeg has levainbröd",
     "levainbröd" in PRODUCT_NAME_BLOCKERS.get("vetesurdeg", set()), True)
test("PNB vetesurdeg has fralla",
     "fralla" in PRODUCT_NAME_BLOCKERS.get("vetesurdeg", set()), True)
test("PNB vetesurdeg has surdegsbröd",
     "surdegsbröd" in PRODUCT_NAME_BLOCKERS.get("vetesurdeg", set()), True)
test("PNB vetesurdeg has knäckebröd",
     "knäckebröd" in PRODUCT_NAME_BLOCKERS.get("vetesurdeg", set()), True)
test("PNB vetesurdeg has rågbröd",
     "rågbröd" in PRODUCT_NAME_BLOCKERS.get("vetesurdeg", set()), True)
test("PNB vetesurdeg has pizzadeg",
     "pizzadeg" in PRODUCT_NAME_BLOCKERS.get("vetesurdeg", set()), True)
# extraction_helper_croissantdeg: generated by dm matcher add extraction-helper
# sanity-id: extraction_helper_croissantdeg
test("KW Croissants Deg product -> croissantdeg", extract_keywords_from_product("Croissants Deg", ""), ["croissantdeg"])
# runtime_pnb_grytbas_red_curry: generated by dm matcher add pnb
# sanity-id: runtime_pnb_grytbas_red_curry
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB grytbas has red curry",
     "red curry" in PRODUCT_NAME_BLOCKERS.get("grytbas", set()), True)
test("PNB grytbas has thai red curry",
     "thai red curry" in PRODUCT_NAME_BLOCKERS.get("grytbas", set()), True)
# runtime_fpb_raksas_gratang: generated by dm matcher add fpb
# sanity-id: runtime_fpb_raksas_gratang
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB räksås has gratäng",
     "gratäng" in FALSE_POSITIVE_BLOCKERS.get("räksås", set()), True)
test("FPB räksås has fiskgratäng",
     "fiskgratäng" in FALSE_POSITIVE_BLOCKERS.get("räksås", set()), True)
# runtime_fpb_agg_marsipanagg: generated by dm matcher add fpb
# sanity-id: runtime_fpb_agg_marsipanagg
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB ägg has marsipanägg",
     "marsipanägg" in FALSE_POSITIVE_BLOCKERS.get("ägg", set()), True)
test("FPB ägg has marsipanagg",
     "marsipanagg" in FALSE_POSITIVE_BLOCKERS.get("ägg", set()), True)
test("FPB ägg has godispåskägg",
     "godispåskägg" in FALSE_POSITIVE_BLOCKERS.get("ägg", set()), True)
test("FPB ägg has godispaskagg",
     "godispaskagg" in FALSE_POSITIVE_BLOCKERS.get("ägg", set()), True)
test("FPB ägg has godisägg",
     "godisägg" in FALSE_POSITIVE_BLOCKERS.get("ägg", set()), True)
test("FPB ägg has godisagg",
     "godisagg" in FALSE_POSITIVE_BLOCKERS.get("ägg", set()), True)
# runtime_fpb_limpa_rostbrod: generated by dm matcher add fpb
# sanity-id: runtime_fpb_limpa_rostbrod
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB limpa has rostbröd",
     "rostbröd" in FALSE_POSITIVE_BLOCKERS.get("limpa", set()), True)
# runtime_fpb_kal_surkal: generated by dm matcher add fpb
# sanity-id: runtime_fpb_kal_surkal
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB kål has surkål",
     "surkål" in FALSE_POSITIVE_BLOCKERS.get("kål", set()), True)
# runtime_ksbc_limpa_rostbrod: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_limpa_rostbrod
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC limpa has rostbröd",
     "rostbröd" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("limpa", set()), True)
# ingredient-parent granatäppelkärnor→granatäpple verified via `dm matcher probe`;
# match()-based sanity is unreliable for parent mappings (different pipeline layer).
# runtime_pnb_burrata_tryffel: generated by dm matcher add pnb
# sanity-id: runtime_pnb_burrata_tryffel
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB burrata has tryffel",
     "tryffel" in PRODUCT_NAME_BLOCKERS.get("burrata", set()), True)
# policy_rostbiff_deli_size_brostbiff_b_b_1_9_d_1_d_d_2_d_d_s_g_b: generated by dm matcher add no-match-policy
# sanity-id: policy_rostbiff_deli_size_brostbiff_b_b_1_9_d_1_d_d_2_d_d_s_g_b
test("no-match-policy blocks Rostbiff 100g",
     match("Rostbiff 100g", "750 g Rostbiff", "meat"), None)
# Tabasco brand-aware extraction (classic 57ml red flask only)
test("classic Tabasco matches plain tabasco ingredient",
     match("Pepparsås 57ml Tabasco", "2 tsk Tabasco", "pantry"), "tabasco")
test("Chipotle Tabasco does NOT match plain tabasco ingredient",
     match("Chipotle pepper sauce 60ml Tabasco", "2 tsk Tabasco", "pantry"), None)
test("Grönpepparsås Tabasco does NOT match plain tabasco ingredient",
     match("Grönpepparsås Mild 57ml Tabasco", "2 tsk Tabasco", "pantry"), None)
# extraction_helper_tabasco: generated by dm matcher add extraction-helper
# sanity-id: extraction_helper_tabasco
test("KW Pepparsås 57ml Tabasco product -> tabasco,pepparsås", extract_keywords_from_product("Pepparsås 57ml Tabasco", "pantry"), ["tabasco", "pepparsås"])
# runtime_pnb_fond_vegetarisk: generated by dm matcher add pnb
# sanity-id: runtime_pnb_fond_vegetarisk
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB fond has vegetarisk",
     "vegetarisk" in PRODUCT_NAME_BLOCKERS.get("fond", set()), True)
# Dragon SVF rule (fresh/dried discrimination, mirrors koriander pattern)
test("torkad dragon does NOT match fresh dragon Klass 1",
     match("Fransk dragon Ekologisk 1-p KRAV Klass 1 ICA", "2 msk torkad dragon", "vegetables"), None)
test("torkad dragon matches dried dragon spice",
     match("Dragon Torkad 10g ICA", "2 msk torkad dragon", "pantry"), "estragon")
# Plant-based grädde extraction (allows explicit plant-recipe ingredients to match)
test("soja-grädde matches soja recipe ingredient",
     match("Matlagningsgrädde Soja 14% 250ml Alpro", "3 dl soyabaserad matlagning", "dairy"), "sojagrädde")
test("havre-grädde matches havre recipe ingredient",
     match("Havregrädde iMat 13% 250ml KRAV Oatly", "2 dl havregrädde", "dairy"), "havregrädde")
# runtime_ksbc_farskost_havre: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_farskost_havre
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC färskost has havre",
     "havre" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("färskost", set()), True)
test("KSBC färskost has soja",
     "soja" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("färskost", set()), True)
test("KSBC färskost has havrebaserad",
     "havrebaserad" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("färskost", set()), True)
test("KSBC färskost has växtbaserad",
     "växtbaserad" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("färskost", set()), True)
# runtime_specialty_qualifier_surdegsbrod: generated by dm matcher add specialty-qualifier
# sanity-id: runtime_specialty_qualifier_surdegsbrod
from languages.sv.ingredient_matching.specialty_rules import SPECIALTY_QUALIFIERS
test("specialty-qualifier surdegsbröd has mörk",
     "mörk" in SPECIALTY_QUALIFIERS.get("surdegsbröd", []), True)
test("specialty-qualifier surdegsbröd has mörkt",
     "mörkt" in SPECIALTY_QUALIFIERS.get("surdegsbröd", []), True)
test("specialty-qualifier surdegsbröd has råg",
     "råg" in SPECIALTY_QUALIFIERS.get("surdegsbröd", []), True)
test("specialty-qualifier surdegsbröd has kavring",
     "kavring" in SPECIALTY_QUALIFIERS.get("surdegsbröd", []), True)
test("specialty-qualifier surdegsbröd has fullkorn",
     "fullkorn" in SPECIALTY_QUALIFIERS.get("surdegsbröd", []), True)
test("specialty-qualifier surdegsbröd has grov",
     "grov" in SPECIALTY_QUALIFIERS.get("surdegsbröd", []), True)
from languages.sv.ingredient_matching.specialty_rules import BIDIRECTIONAL_PER_KEYWORD
test("specialty bidirectional surdegsbröd has mörk",
     "mörk" in BIDIRECTIONAL_PER_KEYWORD.get("surdegsbröd", set()), True)
test("specialty bidirectional surdegsbröd has mörkt",
     "mörkt" in BIDIRECTIONAL_PER_KEYWORD.get("surdegsbröd", set()), True)
test("specialty bidirectional surdegsbröd has råg",
     "råg" in BIDIRECTIONAL_PER_KEYWORD.get("surdegsbröd", set()), True)
test("specialty bidirectional surdegsbröd has kavring",
     "kavring" in BIDIRECTIONAL_PER_KEYWORD.get("surdegsbröd", set()), True)
test("specialty bidirectional surdegsbröd has fullkorn",
     "fullkorn" in BIDIRECTIONAL_PER_KEYWORD.get("surdegsbröd", set()), True)
test("specialty bidirectional surdegsbröd has grov",
     "grov" in BIDIRECTIONAL_PER_KEYWORD.get("surdegsbröd", set()), True)
# policy_chiliolja_bean_peanut_block_b_sojabonor_jordnotter_svarta_s_bonor_b: generated by dm matcher add no-match-policy
# sanity-id: policy_chiliolja_bean_peanut_block_b_sojabonor_jordnotter_svarta_s_bonor_b
test("no-match-policy blocks Sojabönor fermenterade i chiliolja 280g Laoganma",
     match("Sojabönor fermenterade i chiliolja 280g Laoganma", "1.5 msk chiliolja", "pantry"), None)
# runtime_ksbc_gradde_soja: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_gradde_soja
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC grädde has soja",
     "soja" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("grädde", set()), True)
test("KSBC grädde has soya",
     "soya" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("grädde", set()), True)
test("KSBC grädde has havre",
     "havre" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("grädde", set()), True)
test("KSBC grädde has havrebaserad",
     "havrebaserad" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("grädde", set()), True)
test("KSBC grädde has sojabaserad",
     "sojabaserad" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("grädde", set()), True)
# runtime_specialty_qualifier_bonor: generated by dm matcher add specialty-qualifier
# sanity-id: runtime_specialty_qualifier_bonor
from languages.sv.ingredient_matching.specialty_rules import SPECIALTY_QUALIFIERS
test("specialty-qualifier bönor has adzuki",
     "adzuki" in SPECIALTY_QUALIFIERS.get("bönor", []), True)
test("specialty-qualifier bönor has edamame",
     "edamame" in SPECIALTY_QUALIFIERS.get("bönor", []), True)
test("specialty-qualifier bönor has pinto",
     "pinto" in SPECIALTY_QUALIFIERS.get("bönor", []), True)
test("specialty-qualifier bönor has lupin",
     "lupin" in SPECIALTY_QUALIFIERS.get("bönor", []), True)
test("specialty-qualifier bönor has lupinbönor",
     "lupinbönor" in SPECIALTY_QUALIFIERS.get("bönor", []), True)
test("specialty-qualifier bönor has black-eye",
     "black-eye" in SPECIALTY_QUALIFIERS.get("bönor", []), True)
test("specialty-qualifier bönor has black eye",
     "black eye" in SPECIALTY_QUALIFIERS.get("bönor", []), True)
test("specialty-qualifier bönor has blackeye",
     "blackeye" in SPECIALTY_QUALIFIERS.get("bönor", []), True)
from languages.sv.ingredient_matching.specialty_rules import BIDIRECTIONAL_PER_KEYWORD
test("specialty bidirectional bönor has adzuki",
     "adzuki" in BIDIRECTIONAL_PER_KEYWORD.get("bönor", set()), True)
test("specialty bidirectional bönor has edamame",
     "edamame" in BIDIRECTIONAL_PER_KEYWORD.get("bönor", set()), True)
test("specialty bidirectional bönor has pinto",
     "pinto" in BIDIRECTIONAL_PER_KEYWORD.get("bönor", set()), True)
test("specialty bidirectional bönor has lupin",
     "lupin" in BIDIRECTIONAL_PER_KEYWORD.get("bönor", set()), True)
test("specialty bidirectional bönor has lupinbönor",
     "lupinbönor" in BIDIRECTIONAL_PER_KEYWORD.get("bönor", set()), True)
test("specialty bidirectional bönor has black-eye",
     "black-eye" in BIDIRECTIONAL_PER_KEYWORD.get("bönor", set()), True)
test("specialty bidirectional bönor has black eye",
     "black eye" in BIDIRECTIONAL_PER_KEYWORD.get("bönor", set()), True)
test("specialty bidirectional bönor has blackeye",
     "blackeye" in BIDIRECTIONAL_PER_KEYWORD.get("bönor", set()), True)
# runtime_specialty_qualifier_ris: generated by dm matcher add specialty-qualifier
# sanity-id: runtime_specialty_qualifier_ris
from languages.sv.ingredient_matching.specialty_rules import SPECIALTY_QUALIFIERS
test("specialty-qualifier ris has rundkornigt",
     "rundkornigt" in SPECIALTY_QUALIFIERS.get("ris", []), True)
test("specialty-qualifier ris has rundkorn",
     "rundkorn" in SPECIALTY_QUALIFIERS.get("ris", []), True)
test("specialty-qualifier ris has rundkornsris",
     "rundkornsris" in SPECIALTY_QUALIFIERS.get("ris", []), True)
from languages.sv.ingredient_matching.specialty_rules import BIDIRECTIONAL_PER_KEYWORD
test("specialty bidirectional ris has rundkornigt",
     "rundkornigt" in BIDIRECTIONAL_PER_KEYWORD.get("ris", set()), True)
test("specialty bidirectional ris has rundkorn",
     "rundkorn" in BIDIRECTIONAL_PER_KEYWORD.get("ris", set()), True)
test("specialty bidirectional ris has rundkornsris",
     "rundkornsris" in BIDIRECTIONAL_PER_KEYWORD.get("ris", set()), True)
# runtime_ksbc_kex_tuc: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_kex_tuc
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC kex has tuc",
     "tuc" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kex", set()), True)
test("KSBC kex has bake rolls",
     "bake rolls" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kex", set()), True)
test("KSBC kex has bakerolls",
     "bakerolls" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kex", set()), True)
# runtime_ksbc_korvar_chorizo: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_korvar_chorizo
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC korvar has chorizo",
     "chorizo" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korvar", set()), True)
test("KSBC korvar has salsiccia",
     "salsiccia" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korvar", set()), True)
test("KSBC korvar has kabanoss",
     "kabanoss" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korvar", set()), True)
test("KSBC korvar has kabanos",
     "kabanos" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korvar", set()), True)
test("KSBC korvar has bratwurst",
     "bratwurst" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korvar", set()), True)
test("KSBC korvar has falukorv",
     "falukorv" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korvar", set()), True)
test("KSBC korvar has prinskorv",
     "prinskorv" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korvar", set()), True)
test("KSBC korvar has isterband",
     "isterband" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korvar", set()), True)
test("KSBC korvar has blodkorv",
     "blodkorv" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korvar", set()), True)
test("KSBC korvar has merguez",
     "merguez" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korvar", set()), True)
test("KSBC korvar has andouille",
     "andouille" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korvar", set()), True)
test("KSBC korvar has kielbasa",
     "kielbasa" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korvar", set()), True)
test("KSBC korvar has fläskkorv",
     "fläskkorv" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korvar", set()), True)
test("KSBC korvar has grillkorv",
     "grillkorv" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korvar", set()), True)
test("KSBC korvar has julkorv",
     "julkorv" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korvar", set()), True)
test("KSBC korvar has leverkorv",
     "leverkorv" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korvar", set()), True)
test("KSBC korvar has medisterkorv",
     "medisterkorv" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korvar", set()), True)
test("KSBC korvar has morcilla",
     "morcilla" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korvar", set()), True)
test("KSBC korvar has weisswurst",
     "weisswurst" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korvar", set()), True)
test("KSBC korvar has currywurst",
     "currywurst" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korvar", set()), True)
# runtime_ksbc_korv_kabanoss: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_korv_kabanoss
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC korv has kabanoss",
     "kabanoss" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korv", set()), True)
test("KSBC korv has kabanos",
     "kabanos" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korv", set()), True)
test("KSBC korv has bratwurst",
     "bratwurst" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korv", set()), True)
test("KSBC korv has falukorv",
     "falukorv" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korv", set()), True)
test("KSBC korv has prinskorv",
     "prinskorv" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korv", set()), True)
test("KSBC korv has isterband",
     "isterband" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korv", set()), True)
test("KSBC korv has blodkorv",
     "blodkorv" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korv", set()), True)
test("KSBC korv has merguez",
     "merguez" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korv", set()), True)
test("KSBC korv has andouille",
     "andouille" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korv", set()), True)
test("KSBC korv has kielbasa",
     "kielbasa" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korv", set()), True)
test("KSBC korv has fläskkorv",
     "fläskkorv" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korv", set()), True)
test("KSBC korv has grillkorv",
     "grillkorv" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korv", set()), True)
test("KSBC korv has julkorv",
     "julkorv" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korv", set()), True)
test("KSBC korv has leverkorv",
     "leverkorv" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korv", set()), True)
test("KSBC korv has medisterkorv",
     "medisterkorv" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korv", set()), True)
test("KSBC korv has morcilla",
     "morcilla" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korv", set()), True)
test("KSBC korv has weisswurst",
     "weisswurst" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korv", set()), True)
test("KSBC korv has currywurst",
     "currywurst" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("korv", set()), True)
# runtime_specialty_qualifier_bakchoklad: generated by dm matcher add specialty-qualifier
# sanity-id: runtime_specialty_qualifier_bakchoklad
from languages.sv.ingredient_matching.specialty_rules import SPECIALTY_QUALIFIERS
test("specialty-qualifier bakchoklad has vit",
     "vit" in SPECIALTY_QUALIFIERS.get("bakchoklad", []), True)
test("specialty-qualifier bakchoklad has vita",
     "vita" in SPECIALTY_QUALIFIERS.get("bakchoklad", []), True)
test("specialty-qualifier bakchoklad has mörk",
     "mörk" in SPECIALTY_QUALIFIERS.get("bakchoklad", []), True)
test("specialty-qualifier bakchoklad has mörka",
     "mörka" in SPECIALTY_QUALIFIERS.get("bakchoklad", []), True)
test("specialty-qualifier bakchoklad has ljus",
     "ljus" in SPECIALTY_QUALIFIERS.get("bakchoklad", []), True)
test("specialty-qualifier bakchoklad has ljusa",
     "ljusa" in SPECIALTY_QUALIFIERS.get("bakchoklad", []), True)
test("specialty-qualifier bakchoklad has mjölk",
     "mjölk" in SPECIALTY_QUALIFIERS.get("bakchoklad", []), True)
test("specialty-qualifier bakchoklad has mjölk",
     "mjölk" in SPECIALTY_QUALIFIERS.get("bakchoklad", []), True)
from languages.sv.ingredient_matching.specialty_rules import BIDIRECTIONAL_PER_KEYWORD
test("specialty bidirectional bakchoklad has vit",
     "vit" in BIDIRECTIONAL_PER_KEYWORD.get("bakchoklad", set()), True)
test("specialty bidirectional bakchoklad has vita",
     "vita" in BIDIRECTIONAL_PER_KEYWORD.get("bakchoklad", set()), True)
test("specialty bidirectional bakchoklad has mörk",
     "mörk" in BIDIRECTIONAL_PER_KEYWORD.get("bakchoklad", set()), True)
test("specialty bidirectional bakchoklad has mörka",
     "mörka" in BIDIRECTIONAL_PER_KEYWORD.get("bakchoklad", set()), True)
test("specialty bidirectional bakchoklad has ljus",
     "ljus" in BIDIRECTIONAL_PER_KEYWORD.get("bakchoklad", set()), True)
test("specialty bidirectional bakchoklad has ljusa",
     "ljusa" in BIDIRECTIONAL_PER_KEYWORD.get("bakchoklad", set()), True)
test("specialty bidirectional bakchoklad has mjölk",
     "mjölk" in BIDIRECTIONAL_PER_KEYWORD.get("bakchoklad", set()), True)
test("specialty bidirectional bakchoklad has mjölk",
     "mjölk" in BIDIRECTIONAL_PER_KEYWORD.get("bakchoklad", set()), True)
# runtime_specialty_qualifier_chokladknappar: generated by dm matcher add specialty-qualifier
# sanity-id: runtime_specialty_qualifier_chokladknappar
from languages.sv.ingredient_matching.specialty_rules import SPECIALTY_QUALIFIERS
test("specialty-qualifier chokladknappar has vit",
     "vit" in SPECIALTY_QUALIFIERS.get("chokladknappar", []), True)
test("specialty-qualifier chokladknappar has vita",
     "vita" in SPECIALTY_QUALIFIERS.get("chokladknappar", []), True)
test("specialty-qualifier chokladknappar has mörk",
     "mörk" in SPECIALTY_QUALIFIERS.get("chokladknappar", []), True)
test("specialty-qualifier chokladknappar has mörka",
     "mörka" in SPECIALTY_QUALIFIERS.get("chokladknappar", []), True)
test("specialty-qualifier chokladknappar has ljus",
     "ljus" in SPECIALTY_QUALIFIERS.get("chokladknappar", []), True)
test("specialty-qualifier chokladknappar has ljusa",
     "ljusa" in SPECIALTY_QUALIFIERS.get("chokladknappar", []), True)
test("specialty-qualifier chokladknappar has mjölk",
     "mjölk" in SPECIALTY_QUALIFIERS.get("chokladknappar", []), True)
test("specialty-qualifier chokladknappar has mjölk",
     "mjölk" in SPECIALTY_QUALIFIERS.get("chokladknappar", []), True)
from languages.sv.ingredient_matching.specialty_rules import BIDIRECTIONAL_PER_KEYWORD
test("specialty bidirectional chokladknappar has vit",
     "vit" in BIDIRECTIONAL_PER_KEYWORD.get("chokladknappar", set()), True)
test("specialty bidirectional chokladknappar has vita",
     "vita" in BIDIRECTIONAL_PER_KEYWORD.get("chokladknappar", set()), True)
test("specialty bidirectional chokladknappar has mörk",
     "mörk" in BIDIRECTIONAL_PER_KEYWORD.get("chokladknappar", set()), True)
test("specialty bidirectional chokladknappar has mörka",
     "mörka" in BIDIRECTIONAL_PER_KEYWORD.get("chokladknappar", set()), True)
test("specialty bidirectional chokladknappar has ljus",
     "ljus" in BIDIRECTIONAL_PER_KEYWORD.get("chokladknappar", set()), True)
test("specialty bidirectional chokladknappar has ljusa",
     "ljusa" in BIDIRECTIONAL_PER_KEYWORD.get("chokladknappar", set()), True)
test("specialty bidirectional chokladknappar has mjölk",
     "mjölk" in BIDIRECTIONAL_PER_KEYWORD.get("chokladknappar", set()), True)
test("specialty bidirectional chokladknappar has mjölk",
     "mjölk" in BIDIRECTIONAL_PER_KEYWORD.get("chokladknappar", set()), True)
# runtime_pnb_appelmust_hallon: generated by dm matcher add pnb
# sanity-id: runtime_pnb_appelmust_hallon
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB äppelmust has hallon",
     "hallon" in PRODUCT_NAME_BLOCKERS.get("äppelmust", set()), True)
test("PNB äppelmust has päron",
     "päron" in PRODUCT_NAME_BLOCKERS.get("äppelmust", set()), True)
test("PNB äppelmust has bär",
     "bär" in PRODUCT_NAME_BLOCKERS.get("äppelmust", set()), True)
test("PNB äppelmust has multifrukt",
     "multifrukt" in PRODUCT_NAME_BLOCKERS.get("äppelmust", set()), True)
test("PNB äppelmust has blandfrukt",
     "blandfrukt" in PRODUCT_NAME_BLOCKERS.get("äppelmust", set()), True)
test("PNB äppelmust has svartavinbär",
     "svartavinbär" in PRODUCT_NAME_BLOCKERS.get("äppelmust", set()), True)
test("PNB äppelmust has svarta vinbär",
     "svarta vinbär" in PRODUCT_NAME_BLOCKERS.get("äppelmust", set()), True)
test("PNB äppelmust has röda vinbär",
     "röda vinbär" in PRODUCT_NAME_BLOCKERS.get("äppelmust", set()), True)
test("PNB äppelmust has blåbär",
     "blåbär" in PRODUCT_NAME_BLOCKERS.get("äppelmust", set()), True)
# runtime_ksbc_choklad_daim: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_choklad_daim
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC choklad has daim",
     "daim" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("choklad", set()), True)
# extraction_helper_syltadingefara: generated by dm matcher add extraction-helper
# sanity-id: extraction_helper_syltadingefara
test("KW Syltad Ingefära 100g product -> syltadingefära", extract_keywords_from_product("Syltad Ingefära 100g", "pantry"), ["syltadingefära"])
# extraction_helper_pickladingefara: generated by dm matcher add extraction-helper
# sanity-id: extraction_helper_pickladingefara
test("KW Garisushi Ingefära 200g Itamae product -> pickladingefära", extract_keywords_from_product("Garisushi Ingefära 200g Itamae", "pantry"), ["pickladingefära"])
# extraction_helper_syltadingefara: generated by dm matcher add extraction-helper
# sanity-id: extraction_helper_syltadingefara
test("KW 3 msk finhackad syltad ingefära ingredient -> syltadingefära", extract_keywords_from_ingredient("3 msk finhackad syltad ingefära"), ["syltadingefära"])
# runtime_pnb_frallor_bakmix: generated by dm matcher add pnb
# sanity-id: runtime_pnb_frallor_bakmix
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB frallor has bakmix",
     "bakmix" in PRODUCT_NAME_BLOCKERS.get("frallor", set()), True)
# runtime_pnb_havre_havregradde: generated by dm matcher add pnb
# sanity-id: runtime_pnb_havre_havregradde
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB havre has havregrädde",
     "havregrädde" in PRODUCT_NAME_BLOCKERS.get("havre", set()), True)
test("PNB havre has havredryck",
     "havredryck" in PRODUCT_NAME_BLOCKERS.get("havre", set()), True)
test("PNB havre has havremjölk",
     "havremjölk" in PRODUCT_NAME_BLOCKERS.get("havre", set()), True)
# keyword_synonym_pistagenotter_pistagenot: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_pistagenotter_pistagenot
test("Keyword synonym pistagenöt matches pistagenötter",
     match("Lättrostade skalade Pistagenötter 70g ICA Gott Liv", "pistagenöt", "nuts"), "pistagenötter")
# runtime_ksbc_chili_sin_carne: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_chili_sin_carne
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC chili has sin carne",
     "sin carne" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("chili", set()), True)
# keyword_synonym_svartrot_svartrotter: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_svartrot_svartrotter
test("Keyword synonym svartrötter matches svartrot",
     match("Svartrot Ekologisk 375g KRAV Klass 1 ICA", "svartrötter", "vegetables"), "svartrot")
# extraction_word_boundary_fisk: bläckfisk compound should NOT trigger fiskfilé fallback
# sanity-id: extraction_word_boundary_fisk_bleckfisk
test("bläckfisk fryst ingredient extracts bläckfisk, not fiskfilé (word boundary fix)",
     extract_keywords_from_ingredient("300 g Bläckfisk Fryst"), ["bläckfisk"])
# Regression: standalone fisk + fryst still routes to fiskfilé
test("standalone fisk fryst ingredient still extracts fiskfilé fallback (regression)",
     extract_keywords_from_ingredient("400 g tärnad djupfryst fisk"), ["fiskfilé"])
# keyword_synonym_chevre_chevrette: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_chevre_chevrette
test("Keyword synonym chevrette matches chevre",
     match("Ost Chevrette Mild 260g Falbygdens", "chevrette", "dairy"), "chevre")
# runtime_context_word_exemption_chevre: generated by dm matcher add context-word-exemption
# sanity-id: runtime_context_word_exemption_chevre
from languages.sv.ingredient_matching.carrier_context import CONTEXT_WORD_KEYWORD_EXEMPTIONS
test("context-word-exemption chevre has chevrette",
     "chevrette" in CONTEXT_WORD_KEYWORD_EXEMPTIONS.get("chevre", set()), True)

# Q99-2 paren_som_list parser: "(som X, Y och/eller Z)" expands to listed items
# sanity-id: extraction_paren_som_list_herbs
test("paren (som ...) herb list expands to listed items",
     extract_keywords_from_ingredient("gröna örter (som mejram, oregano och/eller körvel)"),
     ["mejram", "oregano", "körvel"])
# Regression: parenthetical without comma (preparation instruction) is NOT expanded
test("paren (som ...) without comma stays unexpanded (preparation instruction)",
     extract_keywords_from_ingredient("tomater (som varit fryst minst 2 dygn)"), ["tomat"])
# keyword_synonym_lonnsirap_pannkakssirap: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_lonnsirap_pannkakssirap
test("Keyword synonym pannkakssirap matches lönnsirap",
     match("Pannkakassirap 710ml Mississippi Belle", "pannkakssirap", "pantry"), "lönnsirap")
test("Keyword synonym pannkakassirap matches lönnsirap",
     match("Pannkakassirap 710ml Mississippi Belle", "pannkakassirap", "pantry"), "lönnsirap")

# Q99-4: pannkakssirap ingredient is strict — does NOT match generic baksirap
# sanity-id: pannkakssirap_strict_no_generic_sirap
test("pannkakssirap ingredient does NOT match generic sirap (strict policy)",
     match("Sirap Ljus 750g Dansukker", "1 msk pannkakssirap", "pantry"), None)
test("pannkakssirap ingredient matches Lönnsirap product",
     match("Lönnsirap 250g ICA", "1 msk pannkakssirap", "pantry"), "lönnsirap")
# runtime_processed-exemption_chili_explosion: generated by dm matcher add processed-exemption
# sanity-id: runtime_processed-exemption_chili_explosion
from languages.sv.ingredient_matching.processed_rules import PROCESSED_RULES_COMPOUND_EXEMPTIONS
test("PROCESSED-EXEMPTION chili has explosion",
     "explosion" in PROCESSED_RULES_COMPOUND_EXEMPTIONS.get("chili", set()), True)
# runtime_ksbc_morot_marmelad: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_morot_marmelad
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC morot has marmelad",
     "marmelad" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("morot", set()), True)
# Q100-1 FP: Morötter no longer matches 'marmelad, gärna morotsmarmelad' ingredient
test("Q100-1: Morötter does not FP-match marmelad+morotsmarmelad ingredient",
     match("Morötter 2kg Klass 1", "marmelad, gärna morotsmarmelad", "vegetables"), None)
# Regression: plain morötter still matches morot ingredient
test("Q100-1 regression: Morötter still matches plain morötter ingredient",
     match("Morötter 2kg Klass 1", "2 morötter", "vegetables"), "morot")

# Q100-2: 'Skinka pålägg' ingredient matches plain skinka deli products
# sanity-id: q100_2_skinka_palagg_deli_exemption
test("Q100-2: Skinka pålägg ingredient matches Skinka product (pålägg-deli exemption)",
     match("Skinka Klassisk 200g Scan", "150 g Skinka pålägg", "deli"), "skinka")
...
# runtime_fpb_rabarber_rabarbersirap: generated by dm matcher add fpb
# sanity-id: runtime_fpb_rabarber_rabarbersirap
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB rabarber has rabarbersirap",
     "rabarbersirap" in FALSE_POSITIVE_BLOCKERS.get("rabarber", set()), True)
# runtime_fpb_sirap_rabarber: generated by dm matcher add fpb
# sanity-id: runtime_fpb_sirap_rabarber
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB sirap has rabarber",
     "rabarber" in FALSE_POSITIVE_BLOCKERS.get("sirap", set()), True)
# runtime_pnb_gradde_oat_lok: generated by dm matcher add pnb
# sanity-id: runtime_pnb_gradde_oat_lok
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB grädde has oat lök",
     "oat lök" in PRODUCT_NAME_BLOCKERS.get("grädde", set()), True)
# runtime_pnb_halkidiki_grillade: generated by dm matcher add pnb
# sanity-id: runtime_pnb_halkidiki_grillade
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB halkidiki has grillade",
     "grillade" in PRODUCT_NAME_BLOCKERS.get("halkidiki", set()), True)
test("PNB halkidiki has grillad",
     "grillad" in PRODUCT_NAME_BLOCKERS.get("halkidiki", set()), True)
test("PNB halkidiki has chili",
     "chili" in PRODUCT_NAME_BLOCKERS.get("halkidiki", set()), True)
test("PNB halkidiki has vitlök",
     "vitlök" in PRODUCT_NAME_BLOCKERS.get("halkidiki", set()), True)
test("PNB halkidiki has citron",
     "citron" in PRODUCT_NAME_BLOCKERS.get("halkidiki", set()), True)
test("PNB halkidiki has ört",
     "ört" in PRODUCT_NAME_BLOCKERS.get("halkidiki", set()), True)
test("PNB halkidiki has örter",
     "örter" in PRODUCT_NAME_BLOCKERS.get("halkidiki", set()), True)
test("PNB halkidiki has paprika",
     "paprika" in PRODUCT_NAME_BLOCKERS.get("halkidiki", set()), True)
test("PNB halkidiki has fetaost",
     "fetaost" in PRODUCT_NAME_BLOCKERS.get("halkidiki", set()), True)
test("PNB halkidiki has torkad",
     "torkad" in PRODUCT_NAME_BLOCKERS.get("halkidiki", set()), True)
test("PNB halkidiki has torkade",
     "torkade" in PRODUCT_NAME_BLOCKERS.get("halkidiki", set()), True)
test("PNB halkidiki has marinerad",
     "marinerad" in PRODUCT_NAME_BLOCKERS.get("halkidiki", set()), True)
# runtime_fpb_socker_vanillinsocker: generated by dm matcher add fpb
# sanity-id: runtime_fpb_socker_vanillinsocker
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB socker has vanillinsocker",
     "vanillinsocker" in FALSE_POSITIVE_BLOCKERS.get("socker", set()), True)
# Behavior regression: ingredient asking for vanillinsocker (concentrated vanilla
# flavoring, ~2 tsk per cake) must NOT pull plain Strösocker via the bleed of
# "socker" being a substring of "vanillinsocker". Plain socker recipe must still
# match Strösocker normally.
test("Strösocker blocked from vanillinsocker recipe",
     match_kw("Strösocker 2kg Dansukker", "2 tsk Vanillinsocker", "baking"), None)
test("Strösocker still matches plain socker recipe",
     match_kw("Strösocker 2kg Dansukker", "1 dl socker", "baking"), "socker")
test("Vanillinsocker product still matches vanillinsocker recipe",
     match_kw("Vanillinsocker 140g ICA", "2 tsk Vanillinsocker", "baking"), "vanillinsocker")
# runtime_pnb_soja_matlagningsgradde: generated by dm matcher add pnb
# sanity-id: runtime_pnb_soja_matlagningsgradde
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB soja has matlagningsgrädde",
     "matlagningsgrädde" in PRODUCT_NAME_BLOCKERS.get("soja", set()), True)
test("PNB soja has vispgrädde",
     "vispgrädde" in PRODUCT_NAME_BLOCKERS.get("soja", set()), True)
test("PNB soja has cuisine",
     "cuisine" in PRODUCT_NAME_BLOCKERS.get("soja", set()), True)
# runtime_stop_word_add_roliga: generated by dm matcher add stop-word
# sanity-id: runtime_stop_word_add_roliga
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add roliga",
     "roliga" in STOP_WORDS, True)
test("stop-word filters extraction roliga",
     "roliga" in kw("roliga"), False)
test("stop-word add rolig",
     "rolig" in STOP_WORDS, True)
test("stop-word filters extraction rolig",
     "rolig" in kw("rolig"), False)
test("stop-word add roligt",
     "roligt" in STOP_WORDS, True)
test("stop-word filters extraction roligt",
     "roligt" in kw("roligt"), False)
# runtime_stop_word_add_giftiga: generated by dm matcher add stop-word
# sanity-id: runtime_stop_word_add_giftiga
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add giftiga",
     "giftiga" in STOP_WORDS, True)
test("stop-word filters extraction giftiga",
     "giftiga" in kw("giftiga"), False)
test("stop-word add giftig",
     "giftig" in STOP_WORDS, True)
test("stop-word filters extraction giftig",
     "giftig" in kw("giftig"), False)
test("stop-word add giftigt",
     "giftigt" in STOP_WORDS, True)
test("stop-word filters extraction giftigt",
     "giftigt" in kw("giftigt"), False)
# runtime_pnb_sodavatten_stilla: generated by dm matcher add pnb
# sanity-id: runtime_pnb_sodavatten_stilla
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB sodavatten has stilla",
     "stilla" in PRODUCT_NAME_BLOCKERS.get("sodavatten", set()), True)
# runtime_pnb_pesto_tomat: generated by dm matcher add pnb
# sanity-id: runtime_pnb_pesto_tomat
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB pesto has tomat",
     "tomat" in PRODUCT_NAME_BLOCKERS.get("pesto", set()), True)
test("PNB pesto has rosso",
     "rosso" in PRODUCT_NAME_BLOCKERS.get("pesto", set()), True)
test("PNB pesto has rödpesto",
     "rödpesto" in PRODUCT_NAME_BLOCKERS.get("pesto", set()), True)
test("PNB pesto has röd",
     "röd" in PRODUCT_NAME_BLOCKERS.get("pesto", set()), True)
test("PNB pesto has soltorkad",
     "soltorkad" in PRODUCT_NAME_BLOCKERS.get("pesto", set()), True)
test("PNB pesto has soltorkade",
     "soltorkade" in PRODUCT_NAME_BLOCKERS.get("pesto", set()), True)
# policy_pain_au_chocolat_chocolat: generated by dm matcher add no-match-policy
# sanity-id: policy_pain_au_chocolat_chocolat
test("no-match-policy blocks Hazelnut Milk Chocolat 100g Treatville",
     match("Hazelnut Milk Chocolat 100g Treatville", "pain au chocolat eller croissanter", "pantry"), None)
# policy_gochugaru_not_korean_bbq_sauce_bulgogi_bbq_marinad_korean_wrap_kyckling_korean_koreansk_morotssallad: generated by dm matcher add no-match-policy
# sanity-id: policy_gochugaru_not_korean_bbq_sauce_bulgogi_bbq_marinad_korean_wrap_kyckling_korean_koreansk_morotssallad
test("no-match-policy blocks Marinad Bulgogi 225g Risberg",
     match("Marinad Bulgogi 225g Risberg", "2 tsk gochugaru, koreansk chili", "pantry"), None)
# runtime_stop_word_add_bonspadet: generated by dm matcher add stop-word
# sanity-id: runtime_stop_word_add_bonspadet
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add bönspadet",
     "bönspadet" in STOP_WORDS, True)
test("stop-word filters extraction bönspadet",
     "bönspadet" in kw("bönspadet"), False)
test("stop-word add bönpad",
     "bönpad" in STOP_WORDS, True)
test("stop-word filters extraction bönpad",
     "bönpad" in kw("bönpad"), False)
test("stop-word add bönvatten",
     "bönvatten" in STOP_WORDS, True)
test("stop-word filters extraction bönvatten",
     "bönvatten" in kw("bönvatten"), False)
# runtime_pnb_oliver_snack: generated by dm matcher add pnb
# sanity-id: runtime_pnb_oliver_snack
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB oliver has snack",
     "snack" in PRODUCT_NAME_BLOCKERS.get("oliver", set()), True)
test("PNB oliver has havssalt",
     "havssalt" in PRODUCT_NAME_BLOCKERS.get("oliver", set()), True)
test("PNB oliver has provencal",
     "provencal" in PRODUCT_NAME_BLOCKERS.get("oliver", set()), True)
# runtime_fpb_socker_sockerbitar: generated by dm matcher add fpb
# sanity-id: runtime_fpb_socker_sockerbitar
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB socker has sockerbitar",
     "sockerbitar" in FALSE_POSITIVE_BLOCKERS.get("socker", set()), True)
# runtime_ksbc_potatis_nypotatis: REVERTED — nypotatis = färskpotatis (same product)
# nypotatis should NOT suppress potatis keyword; keyword_synonym handles routing instead
test("KSBC potatis does NOT suppress on nypotatis (same as färskpotatis)",
     "nypotatis" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("potatis", set()), False)
# runtime_fpb_lingon_lingonvodka: generated by dm matcher add fpb
# sanity-id: runtime_fpb_lingon_lingonvodka
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB lingon has lingonvodka",
     "lingonvodka" in FALSE_POSITIVE_BLOCKERS.get("lingon", set()), True)
# runtime_fpb_brod_brodkummin: generated by dm matcher add fpb
# sanity-id: runtime_fpb_brod_brodkummin
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB bröd has brödkummin",
     "brödkummin" in FALSE_POSITIVE_BLOCKERS.get("bröd", set()), True)
test("FPB bröd has brodkummin",
     "brodkummin" in FALSE_POSITIVE_BLOCKERS.get("bröd", set()), True)
# keyword_synonym_farskpotatis_nypotatis: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_farskpotatis_nypotatis
test("Keyword synonym nypotatis matches färskpotatis",
     match("Nypotatis Klass 1 ICA", "nypotatis", "vegetables"), "färskpotatis")
# runtime_stop_word_add_smarta: generated by dm matcher add stop-word
# sanity-id: runtime_stop_word_add_smarta
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add smarta",
     "smarta" in STOP_WORDS, True)
test("stop-word filters extraction smarta",
     "smarta" in kw("smarta"), False)
test("stop-word add smartare",
     "smartare" in STOP_WORDS, True)
test("stop-word filters extraction smartare",
     "smartare" in kw("smartare"), False)
test("stop-word add kluriga",
     "kluriga" in STOP_WORDS, True)
test("stop-word filters extraction kluriga",
     "kluriga" in kw("kluriga"), False)
test("stop-word add mysterier",
     "mysterier" in STOP_WORDS, True)
test("stop-word filters extraction mysterier",
     "mysterier" in kw("mysterier"), False)
test("stop-word add finurliga",
     "finurliga" in STOP_WORDS, True)
test("stop-word filters extraction finurliga",
     "finurliga" in kw("finurliga"), False)
test("stop-word add mattegåtor",
     "mattegåtor" in STOP_WORDS, True)
test("stop-word filters extraction mattegåtor",
     "mattegåtor" in kw("mattegåtor"), False)
test("stop-word add detektiver",
     "detektiver" in STOP_WORDS, True)
test("stop-word filters extraction detektiver",
     "detektiver" in kw("detektiver"), False)
# runtime_pnb_salsa_verde: generated by dm matcher add pnb
# sanity-id: runtime_pnb_salsa_verde
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB salsa has verde",
     "verde" in PRODUCT_NAME_BLOCKERS.get("salsa", set()), True)
test("PNB salsa has tomatillo",
     "tomatillo" in PRODUCT_NAME_BLOCKERS.get("salsa", set()), True)
test("PNB salsa has tropical",
     "tropical" in PRODUCT_NAME_BLOCKERS.get("salsa", set()), True)
test("PNB salsa has pineapple",
     "pineapple" in PRODUCT_NAME_BLOCKERS.get("salsa", set()), True)
test("PNB salsa has ananas",
     "ananas" in PRODUCT_NAME_BLOCKERS.get("salsa", set()), True)
test("PNB salsa has garlic",
     "garlic" in PRODUCT_NAME_BLOCKERS.get("salsa", set()), True)
test("PNB salsa has roast",
     "roast" in PRODUCT_NAME_BLOCKERS.get("salsa", set()), True)
test("PNB salsa has chipotle",
     "chipotle" in PRODUCT_NAME_BLOCKERS.get("salsa", set()), True)
# runtime_pnb_noodles_nudelkopp: generated by dm matcher add pnb
# sanity-id: runtime_pnb_noodles_nudelkopp
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB noodles has nudelkopp",
     "nudelkopp" in PRODUCT_NAME_BLOCKERS.get("noodles", set()), True)
test("PNB noodles has kopp",
     "kopp" in PRODUCT_NAME_BLOCKERS.get("noodles", set()), True)
# runtime_pnb_sobanudlar_nissin: generated by dm matcher add pnb
# sanity-id: runtime_pnb_sobanudlar_nissin
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB sobanudlar has nissin",
     "nissin" in PRODUCT_NAME_BLOCKERS.get("sobanudlar", set()), True)
test("PNB sobanudlar has chicken",
     "chicken" in PRODUCT_NAME_BLOCKERS.get("sobanudlar", set()), True)
test("PNB sobanudlar has kyckling",
     "kyckling" in PRODUCT_NAME_BLOCKERS.get("sobanudlar", set()), True)
test("PNB sobanudlar has yakitori",
     "yakitori" in PRODUCT_NAME_BLOCKERS.get("sobanudlar", set()), True)
test("PNB sobanudlar has smak",
     "smak" in PRODUCT_NAME_BLOCKERS.get("sobanudlar", set()), True)
# keyword_synonym_kraftstjartar_kraftstjart: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_kraftstjartar_kraftstjart
test("Keyword synonym kräftstjärt matches kräftstjärtar",
     match("Kräftstjärtar i lake 170g ICA", "kräftstjärt", "seafood"), "kräftstjärtar")
# runtime_pnb_bulgur_vermicelli: generated by dm matcher add pnb
# sanity-id: runtime_pnb_bulgur_vermicelli
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB bulgur has vermicelli",
     "vermicelli" in PRODUCT_NAME_BLOCKERS.get("bulgur", set()), True)
test("PNB bulgur has vermecelli",
     "vermecelli" in PRODUCT_NAME_BLOCKERS.get("bulgur", set()), True)
test("PNB bulgur has nudlar",
     "nudlar" in PRODUCT_NAME_BLOCKERS.get("bulgur", set()), True)
# runtime_pnb_oreo_tarta: generated by dm matcher add pnb
# sanity-id: runtime_pnb_oreo_tarta
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB oreo has tårta",
     "tårta" in PRODUCT_NAME_BLOCKERS.get("oreo", set()), True)
test("PNB oreo has fryst",
     "fryst" in PRODUCT_NAME_BLOCKERS.get("oreo", set()), True)
# runtime_fpb_ingefara_inlagd: generated by dm matcher add fpb
# sanity-id: runtime_fpb_ingefara_inlagd
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB ingefära has inlagd",
     "inlagd" in FALSE_POSITIVE_BLOCKERS.get("ingefära", set()), True)
test("FPB ingefära has gari",
     "gari" in FALSE_POSITIVE_BLOCKERS.get("ingefära", set()), True)
test("FPB ingefära has picklad",
     "picklad" in FALSE_POSITIVE_BLOCKERS.get("ingefära", set()), True)
test("FPB ingefära has syltad",
     "syltad" in FALSE_POSITIVE_BLOCKERS.get("ingefära", set()), True)
# runtime_fpb_ingefara_pickladingefara: generated by dm matcher add fpb
# sanity-id: runtime_fpb_ingefara_pickladingefara
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB ingefära has pickladingefära",
     "pickladingefära" in FALSE_POSITIVE_BLOCKERS.get("ingefära", set()), True)
test("FPB ingefära has pickladingefara",
     "pickladingefara" in FALSE_POSITIVE_BLOCKERS.get("ingefära", set()), True)
test("FPB ingefära has syltadingefära",
     "syltadingefära" in FALSE_POSITIVE_BLOCKERS.get("ingefära", set()), True)
test("FPB ingefära has syltadingefara",
     "syltadingefara" in FALSE_POSITIVE_BLOCKERS.get("ingefära", set()), True)
# runtime_pnb_proteinpudding_choklad: generated by dm matcher add pnb
# sanity-id: runtime_pnb_proteinpudding_choklad
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB proteinpudding has choklad",
     "choklad" in PRODUCT_NAME_BLOCKERS.get("proteinpudding", set()), True)
test("PNB proteinpudding has caramel",
     "caramel" in PRODUCT_NAME_BLOCKERS.get("proteinpudding", set()), True)
test("PNB proteinpudding has karamell",
     "karamell" in PRODUCT_NAME_BLOCKERS.get("proteinpudding", set()), True)
test("PNB proteinpudding has delicato",
     "delicato" in PRODUCT_NAME_BLOCKERS.get("proteinpudding", set()), True)
test("PNB proteinpudding has gräddkola",
     "gräddkola" in PRODUCT_NAME_BLOCKERS.get("proteinpudding", set()), True)
test("PNB proteinpudding has jordgubb",
     "jordgubb" in PRODUCT_NAME_BLOCKERS.get("proteinpudding", set()), True)
test("PNB proteinpudding has salted",
     "salted" in PRODUCT_NAME_BLOCKERS.get("proteinpudding", set()), True)
test("PNB proteinpudding has delicatoboll",
     "delicatoboll" in PRODUCT_NAME_BLOCKERS.get("proteinpudding", set()), True)
test("PNB proteinpudding has vit choklad",
     "vit choklad" in PRODUCT_NAME_BLOCKERS.get("proteinpudding", set()), True)
# ingredient_parent_rodbeta_betor: betor → rödbeta-familj routing
test("ingredient-parent betor matches rödbeta-products",
     match("Rödbetor i bit Ekologiska 250g Felix", "Betor", "pantry"), "betor")
# ingredient_parent_grotris_rundkornigt_ris: rundkornigt ris → grötris-familj routing
test("ingredient-parent rundkornigt ris matches grötris-products",
     match("Grötris 1kg ICA", "Rundkornigt Ris", "pantry"), "rundkornigt ris")
# keyword_synonym_pepparkakskryddor_pepparkakskrydda: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_pepparkakskryddor_pepparkakskrydda
test("Keyword synonym pepparkakskrydda matches pepparkakskryddor",
     match("Pepparkakskryddor påse 18g Santa Maria", "pepparkakskrydda", "spices"), "pepparkakskryddor")
# keyword_synonym_polkagrisar_polkagriskarameller: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_polkagrisar_polkagriskarameller
test("Keyword synonym polkagriskarameller matches polkagrisar",
     match("Polkagrisar 140g Treatville", "polkagriskarameller", "sweets"), "polkagrisar")
# keyword_synonym_hamburgerbrod_hamburgarbrod: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_hamburgerbrod_hamburgarbrod
test("Keyword synonym hamburgarbröd matches hamburgerbröd",
     match("Hamburgerbröd 8-p 416g Pågen", "hamburgarbröd", "bread"), "hamburgerbröd")
# runtime_fpb_mjol_mjolig: generated by dm matcher add fpb
# sanity-id: runtime_fpb_mjol_mjolig
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB mjöl has mjölig",
     "mjölig" in FALSE_POSITIVE_BLOCKERS.get("mjöl", set()), True)
test("FPB mjöl has mjolig",
     "mjolig" in FALSE_POSITIVE_BLOCKERS.get("mjöl", set()), True)
# runtime_fpb_ost_ostfylld: generated by dm matcher add fpb
# sanity-id: runtime_fpb_ost_ostfylld
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB ost has ostfylld",
     "ostfylld" in FALSE_POSITIVE_BLOCKERS.get("ost", set()), True)
test("FPB ost has ostfyllda",
     "ostfyllda" in FALSE_POSITIVE_BLOCKERS.get("ost", set()), True)
# keyword_synonym_cornichons_cornichon: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_cornichons_cornichon
test("Keyword synonym cornichon matches cornichons",
     match("Cornichons 350g Felix", "cornichon", "other"), "cornichons")
# keyword_synonym_snabbkaffe_pulverkaffe: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_snabbkaffe_pulverkaffe
test("Keyword synonym pulverkaffe matches snabbkaffe",
     match("Snabbkaffe Classic 200g Gevalia", "pulverkaffe", "pantry"), "snabbkaffe")
# runtime_fpb_smor_frosmor: generated by dm matcher add fpb
# sanity-id: runtime_fpb_smor_frosmor
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB smör has frösmör",
     "frösmör" in FALSE_POSITIVE_BLOCKERS.get("smör", set()), True)
test("FPB smör has frosmor",
     "frosmor" in FALSE_POSITIVE_BLOCKERS.get("smör", set()), True)
# runtime_fpb_paprika_paprikapure: generated by dm matcher add fpb
# sanity-id: runtime_fpb_paprika_paprikapure
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB paprika has paprikapure",
     "paprikapure" in FALSE_POSITIVE_BLOCKERS.get("paprika", set()), True)
# runtime_stop_word_add_american: generated by dm matcher add stop-word
# sanity-id: runtime_stop_word_add_american
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add american",
     "american" in STOP_WORDS, True)
test("stop-word filters extraction american",
     "american" in kw("american"), False)
# runtime_stop_word_add_randig: generated by dm matcher add stop-word
# sanity-id: runtime_stop_word_add_randig
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add randig",
     "randig" in STOP_WORDS, True)
test("stop-word filters extraction randig",
     "randig" in kw("randig"), False)
test("stop-word add randiga",
     "randiga" in STOP_WORDS, True)
test("stop-word filters extraction randiga",
     "randiga" in kw("randiga"), False)
test("stop-word add randigt",
     "randigt" in STOP_WORDS, True)
test("stop-word filters extraction randigt",
     "randigt" in kw("randigt"), False)
# "Kaviar randig Kalles" must not match a craft-supply line ("randiga sugror")
# now that the striped/pattern adjective no longer leaks as a keyword.
test("randig stop-word removes shared keyword — no match between kaviar and craft-supply ingredient",
     match_kw("Kaviar randig 185g Kalles", "20 klubbpinnar el randiga sugrör"), None)
# runtime_fpb_ost_ostkrame: generated by dm matcher add fpb
# sanity-id: runtime_fpb_ost_ostkrame
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB ost has ostkräme",
     "ostkräme" in FALSE_POSITIVE_BLOCKERS.get("ost", set()), True)
test("FPB ost has ostkrämen",
     "ostkrämen" in FALSE_POSITIVE_BLOCKERS.get("ost", set()), True)
# runtime_ksbc_pommes_sotpotatis: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_pommes_sotpotatis
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC pommes has sötpotatis",
     "sötpotatis" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("pommes", set()), True)
# runtime_pnb_kantareller_torkad: generated by dm matcher add pnb
# sanity-id: runtime_pnb_kantareller_torkad
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB kantareller has torkad",
     "torkad" in PRODUCT_NAME_BLOCKERS.get("kantareller", set()), True)
# runtime_pnb_pinsasas_chili: generated by dm matcher add pnb
# sanity-id: runtime_pnb_pinsasas_chili
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB pinsasås has chili",
     "chili" in PRODUCT_NAME_BLOCKERS.get("pinsasås", set()), True)
test("PNB pinsasås has nduja",
     "nduja" in PRODUCT_NAME_BLOCKERS.get("pinsasås", set()), True)
# runtime_ksbc_brod_piadina: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_brod_piadina
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC bröd has piadina",
     "piadina" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("bröd", set()), True)
# runtime_ksbc_bullar_pease: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_bullar_pease
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC bullar has pease",
     "pease" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("bullar", set()), True)
# runtime_compound_protection_suffix_strict_kal: generated by dm matcher add compound-protection
# sanity-id: runtime_compound_protection_suffix_strict_kal
from languages.sv.ingredient_matching.compound_text import _COMPOUND_STRICT_KEYWORDS
test("compound-protection suffix_strict kål",
     "kål" in _COMPOUND_STRICT_KEYWORDS, True)
# runtime_ksbc_vitlok_grillolja: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_vitlok_grillolja
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC vitlök has grillolja",
     "grillolja" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("vitlök", set()), True)
# offer_extra_keyword_bacon_vegobacon: generated by dm matcher add offer-extra-keyword
# sanity-id: offer_extra_keyword_bacon_vegobacon
test("offer-extra-keyword Vegobacon matches bacon",
     match("Vegobacon", "bacon", "pantry"), "bacon")
# offer_extra_keyword_schnitzel_vegoschnitzel: generated by dm matcher add offer-extra-keyword
# sanity-id: offer_extra_keyword_schnitzel_vegoschnitzel
test("offer-extra-keyword Vegoschnitzel matches schnitzel",
     match("Vegoschnitzel", "schnitzel", "pantry"), "schnitzel")
# offer_extra_keyword_nuggets_vegonuggets: generated by dm matcher add offer-extra-keyword
# sanity-id: offer_extra_keyword_nuggets_vegonuggets
test("offer-extra-keyword Vegonuggets matches nuggets",
     match("Vegonuggets", "nuggets", "pantry"), "nuggets")
# runtime_pnb_pepparkaka_skumtomte: generated by dm matcher add pnb
# sanity-id: runtime_pnb_pepparkaka_skumtomte
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB pepparkaka has skumtomte",
     "skumtomte" in PRODUCT_NAME_BLOCKERS.get("pepparkaka", set()), True)
# runtime_fpb_kottfars_kottfarssas: generated by dm matcher add fpb
# sanity-id: runtime_fpb_kottfars_kottfarssas
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB köttfärs has köttfärssås",
     "köttfärssås" in FALSE_POSITIVE_BLOCKERS.get("köttfärs", set()), True)
test("FPB köttfärs has köttfärssåser",
     "köttfärssåser" in FALSE_POSITIVE_BLOCKERS.get("köttfärs", set()), True)
# runtime_stop_word_add_vargarda: generated by dm matcher add stop-word
# sanity-id: runtime_stop_word_add_vargarda
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add vårgårda",
     "vårgårda" in STOP_WORDS, True)
test("stop-word filters extraction vårgårda",
     "vårgårda" in kw("vårgårda"), False)
# Q139-1: "Vårgårda Ris" (AXA) is puffed/roasted rice despite the plain name.
# extraction.py detects the brand + ris pattern → rispuffar only (no ris fallback).
# Puffed-rice recipes match; plain-rice recipes do not.
test("Vårgårda Ris AXA extracts rispuffar",
     extract_keywords_from_product("Vårgårda Ris 250g AXA", "pantry"), ["rispuffar"])
test("Vårgårda Ris matches puffed-rice recipe",
     recipe_match_num(["3 dl puffat rostat ris"], {"name": "Vårgårda Ris 250g AXA", "category": "pantry"}), 1)
test("Vårgårda Ris does not match plain ris recipe",
     recipe_match_num(["2 dl ris"], {"name": "Vårgårda Ris 250g AXA", "category": "pantry"}), 0)
# runtime_pnb_smatomat_vongole: generated by dm matcher add pnb
# sanity-id: runtime_pnb_smatomat_vongole
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB småtomat has vongole",
     "vongole" in PRODUCT_NAME_BLOCKERS.get("småtomat", set()), True)
# runtime_stop_word_add_reserva: generated by dm matcher add stop-word
# sanity-id: runtime_stop_word_add_reserva
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add reserva",
     "reserva" in STOP_WORDS, True)
test("stop-word filters extraction reserva",
     "reserva" in kw("reserva"), False)
...
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB mango has mangosorbet",
     "mangosorbet" in FALSE_POSITIVE_BLOCKERS.get("mango", set()), True)
# runtime_stop_word_add_fruktskalen: generated by dm matcher add stop-word
# sanity-id: runtime_stop_word_add_fruktskalen
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add fruktskålen",
     "fruktskålen" in STOP_WORDS, True)
test("stop-word filters extraction fruktskålen",
     "fruktskålen" in kw("fruktskålen"), False)
# runtime_fpb_kal_fruktskal: generated by dm matcher add fpb
# sanity-id: runtime_fpb_kal_fruktskal
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
...
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB mango has mangojuice",
     "mangojuice" in FALSE_POSITIVE_BLOCKERS.get("mango", set()), True)
test("FPB mango has mangojuicen",
     "mangojuicen" in FALSE_POSITIVE_BLOCKERS.get("mango", set()), True)
# runtime_fpb_pumpa_pumpafro: generated by dm matcher add fpb
# sanity-id: runtime_fpb_pumpa_pumpafro
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB pumpa has pumpafrö",
     "pumpafrö" in FALSE_POSITIVE_BLOCKERS.get("pumpa", set()), True)
test("FPB pumpa has pumpafro",
     "pumpafro" in FALSE_POSITIVE_BLOCKERS.get("pumpa", set()), True)
# Whole-squash products must not match a pumpkin-seed ingredient.
# "pumpafrö" (singular pumpkin seed) shares the prefix "pumpa" with the
...
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC ravioli has chokladravioli",
     "chokladravioli" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("ravioli", set()), True)
# runtime_fpb_choklad_chokladravioli: generated by dm matcher add fpb
# sanity-id: runtime_fpb_choklad_chokladravioli
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
...
# runtime_fpb_citron_citronmuffins: generated by dm matcher add fpb
# sanity-id: runtime_fpb_citron_citronmuffins
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB citron has citronmuffins",
     "citronmuffins" in FALSE_POSITIVE_BLOCKERS.get("citron", set()), True)
from languages.sv.ingredient_matching.matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC chili has con carne context",
     "con carne" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("chili", set()), True)
test(
    "MAT chili con carne burk blocks fresh chili peppar",
    match_kw("Chili 75g ICA", "560 g Chili Con Carne Burk", "vegetables"),
    None,
)
# runtime_fpb_entrecote_lammentrecote: generated by dm matcher add fpb
# sanity-id: runtime_fpb_entrecote_lammentrecote
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB entrecote has lammentrecote",
     "lammentrecote" in FALSE_POSITIVE_BLOCKERS.get("entrecote", set()), True)
# Beef "entrecote" is a substring of the lamb-cut keyword "lammentrecote".
# A lamb-entrecote recipe must not pull in plain beef entrecote products,
# while genuine lamb products and plain beef-entrecote recipes still match.
test("FPB entrecote blocks beef entrecote from lamb-entrecote ingredient",
     match_kw("Entrecote 180g Scan", "500 g lammentrecôte", "meat"), None)
test("Beef entrecote still matches a plain beef-entrecote ingredient",
     match_kw("Entrecote 180g Scan", "500 g entrecote", "meat") is not None, True)
# runtime_pnb_paprikapulver_lax: generated by dm matcher add pnb
# sanity-id: runtime_pnb_paprikapulver_lax
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB paprikapulver has lax",
     "lax" in PRODUCT_NAME_BLOCKERS.get("paprikapulver", set()), True)
test("PNB paprikapulver has laxfilé",
     "laxfilé" in PRODUCT_NAME_BLOCKERS.get("paprikapulver", set()), True)
test("PNB paprikapulver has laxfilet",
     "laxfilet" in PRODUCT_NAME_BLOCKERS.get("paprikapulver", set()), True)
test("PNB paprikapulver has fisk",
     "fisk" in PRODUCT_NAME_BLOCKERS.get("paprikapulver", set()), True)
test("PNB paprikapulver has fiskfilé",
     "fiskfilé" in PRODUCT_NAME_BLOCKERS.get("paprikapulver", set()), True)
test("PNB paprikapulver has fiskfilet",
     "fiskfilet" in PRODUCT_NAME_BLOCKERS.get("paprikapulver", set()), True)
# runtime_ksbc_apple_plant_gurt: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_apple_plant_gurt
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC äpple has plant gurt",
     "plant gurt" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("äpple", set()), True)
test("KSBC äpple has plantgurt",
     "plantgurt" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("äpple", set()), True)
test("KSBC äpple has havregurt",
     "havregurt" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("äpple", set()), True)
test("KSBC äpple has kokosgurt",
     "kokosgurt" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("äpple", set()), True)
test("KSBC äpple has sojagurt",
     "sojagurt" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("äpple", set()), True)
# runtime_pnb_bacon_hamburgare: generated by dm matcher add pnb
# sanity-id: runtime_pnb_bacon_hamburgare
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB bacon has hamburgare",
     "hamburgare" in PRODUCT_NAME_BLOCKERS.get("bacon", set()), True)
# runtime_stop_word_add_tjock: generated by dm matcher add stop-word
# sanity-id: runtime_stop_word_add_tjock
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add tjock",
     "tjock" in STOP_WORDS, True)
test("stop-word filters extraction tjock",
     "tjock" in kw("tjock"), False)
test("stop-word add tjocka",
     "tjocka" in STOP_WORDS, True)
test("stop-word filters extraction tjocka",
     "tjocka" in kw("tjocka"), False)
test("stop-word add tjockt",
     "tjockt" in STOP_WORDS, True)
test("stop-word filters extraction tjockt",
     "tjockt" in kw("tjockt"), False)
test("stop-word add tjocke",
     "tjocke" in STOP_WORDS, True)
test("stop-word filters extraction tjocke",
     "tjocke" in kw("tjocke"), False)
...
test("Keyword synonym shitakesvamp matches shiitake",
     match("Shiitake 100g ICA", "shitakesvamp", "vegetables"), "shiitake")
test("Keyword synonym shitakesvampar matches shiitake",
     match("Shiitake 100g ICA", "shitakesvampar", "vegetables"), "shiitake")
# keyword_synonym_shiitake_shitake: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_shiitake_shitake
test("Keyword synonym shitake matches shiitake",
     match("Shiitake 100g ICA", "shitake", "vegetables"), "shiitake")
# runtime_pnb_sillfileer_5_minuters: generated by dm matcher add pnb
# sanity-id: runtime_pnb_sillfileer_5_minuters
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB sillfileer has 5-minuters",
...
test("Keyword synonym scharlottenlök matches schalottenlök",
     match("Schalottenlök 250g ICA", "scharlottenlök", "vegetables"), "schalottenlök")
test("Keyword synonym scharlottenlökar matches schalottenlök",
     match("Schalottenlök 250g ICA", "scharlottenlökar", "vegetables"), "schalottenlök")
test("Keyword synonym schalottenlökar matches schalottenlök",
     match("Schalottenlök 250g ICA", "schalottenlökar", "vegetables"), "schalottenlök")
# runtime_fpb_majonnas_citronmajonnas: generated by dm matcher add fpb
# sanity-id: runtime_fpb_majonnas_citronmajonnas
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB majonnäs has citronmajonnäs",
     "citronmajonnäs" in FALSE_POSITIVE_BLOCKERS.get("majonnäs", set()), True)
test("FPB majonnäs has citronmajonnas",
     "citronmajonnas" in FALSE_POSITIVE_BLOCKERS.get("majonnäs", set()), True)
test("FPB majonnäs has currymajonnäs",
     "currymajonnäs" in FALSE_POSITIVE_BLOCKERS.get("majonnäs", set()), True)
test("FPB majonnäs has currymajonnas",
     "currymajonnas" in FALSE_POSITIVE_BLOCKERS.get("majonnäs", set()), True)
# runtime_fpb_choklad_schweizernot: generated by dm matcher add fpb
# sanity-id: runtime_fpb_choklad_schweizernot
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB choklad has schweizernöt",
     "schweizernöt" in FALSE_POSITIVE_BLOCKERS.get("choklad", set()), True)
test("FPB choklad has schweizernot",
     "schweizernot" in FALSE_POSITIVE_BLOCKERS.get("choklad", set()), True)
# keyword_synonym_langpasta_spaghettoni: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_langpasta_spaghettoni
test("Keyword synonym spaghettoni matches långpasta",
     match("Spaghettoni pasta 500g", "spaghettoni", "pasta"), "långpasta")
# runtime_fpb_sylt_bjornbarssylt: generated by dm matcher add fpb
# sanity-id: runtime_fpb_sylt_bjornbarssylt
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB sylt has björnbärssylt",
     "björnbärssylt" in FALSE_POSITIVE_BLOCKERS.get("sylt", set()), True)
# runtime_fpb_ris_granris: generated by dm matcher add fpb
# sanity-id: runtime_fpb_ris_granris
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB ris has granris",
     "granris" in FALSE_POSITIVE_BLOCKERS.get("ris", set()), True)
# runtime_ksbc_svamp_dragonsas: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_svamp_dragonsas
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC svamp has dragonsås",
     "dragonsås" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("svamp", set()), True)
# runtime_fpb_svamp: generated by dm matcher add fpb
# sanity-id: runtime_fpb_svamp
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB svamp has karl-johan",
     "karl-johan" in FALSE_POSITIVE_BLOCKERS.get("svamp", set()), True)
test("FPB svamp has karljohan",
     "karljohan" in FALSE_POSITIVE_BLOCKERS.get("svamp", set()), True)
test("FPB svamp has karl-johansvamp",
     "karl-johansvamp" in FALSE_POSITIVE_BLOCKERS.get("svamp", set()), True)
test("FPB svamp has karljohansvamp",
     "karljohansvamp" in FALSE_POSITIVE_BLOCKERS.get("svamp", set()), True)
test("FPB svamp has svampburgare",
     "svampburgare" in FALSE_POSITIVE_BLOCKERS.get("svamp", set()), True)
# runtime_ksbc_schalottenlok_fond: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_schalottenlok_fond
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC schalottenlök has fond",
     "fond" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("schalottenlök", set()), True)
# runtime_stop_word_add_urbenad: generated by dm matcher add stop-word
# sanity-id: runtime_stop_word_add_urbenad
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add urbenad",
     "urbenad" in STOP_WORDS, True)
test("stop-word filters extraction urbenad",
     "urbenad" in kw("urbenad"), False)
# runtime_pnb_paprika_lax: generated by dm matcher add pnb
# sanity-id: runtime_pnb_paprika_lax
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB paprika has lax",
     "lax" in PRODUCT_NAME_BLOCKERS.get("paprika", set()), True)
test("PNB paprika has fisk",
     "fisk" in PRODUCT_NAME_BLOCKERS.get("paprika", set()), True)
# keyword_synonym_salladslok_varlok: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_salladslok_varlok
test("Keyword synonym vårlök matches salladslök",
     match("Salladslök 125g ICA", "vårlök", "vegetables"), "salladslök")
test("Keyword synonym vårlökar matches salladslök",
     match("Salladslök 125g ICA", "vårlökar", "vegetables"), "salladslök")
# keyword_synonym_dadlar_dadel: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_dadlar_dadel
test("Keyword synonym dadel matches dadlar",
     match("Dadlar Färska 400g Klass 1 ICA", "dadel", "fruit"), "dadlar")
# runtime_product_name_substitution_bakmargarin_margarin: generated by dm matcher add product-name-substitution
# sanity-id: runtime_product_name_substitution_bakmargarin_margarin
from languages.sv.ingredient_matching.match_filters import PRODUCT_NAME_SUBSTITUTIONS
test("product-name-substitution bakmargarin -> margarin",
     any(
         set(required_words) == set(["bakmargarin"])
         and old_keyword == "bakmargarin"
         and new_keyword == "margarin"
         for required_words, old_keyword, new_keyword in PRODUCT_NAME_SUBSTITUTIONS
     ), True)
# runtime_product_name_substitution_stekmargarin_margarin: generated by dm matcher add product-name-substitution
# sanity-id: runtime_product_name_substitution_stekmargarin_margarin
from languages.sv.ingredient_matching.match_filters import PRODUCT_NAME_SUBSTITUTIONS
test("product-name-substitution stekmargarin -> margarin",
     any(
         set(required_words) == set(["stekmargarin"])
         and old_keyword == "stekmargarin"
         and new_keyword == "margarin"
         for required_words, old_keyword, new_keyword in PRODUCT_NAME_SUBSTITUTIONS
     ), True)
# runtime_product_name_substitution_matlagningsmargarin_margarin: generated by dm matcher add product-name-substitution
# sanity-id: runtime_product_name_substitution_matlagningsmargarin_margarin
from languages.sv.ingredient_matching.match_filters import PRODUCT_NAME_SUBSTITUTIONS
test("product-name-substitution matlagningsmargarin -> margarin",
     any(
         set(required_words) == set(["matlagningsmargarin"])
         and old_keyword == "matlagningsmargarin"
         and new_keyword == "margarin"
         for required_words, old_keyword, new_keyword in PRODUCT_NAME_SUBSTITUTIONS
     ), True)
# runtime_product_name_substitution_bordsmargarin_margarin: generated by dm matcher add product-name-substitution
# sanity-id: runtime_product_name_substitution_bordsmargarin_margarin
from languages.sv.ingredient_matching.match_filters import PRODUCT_NAME_SUBSTITUTIONS
test("product-name-substitution bordsmargarin -> margarin",
     any(
         set(required_words) == set(["bak"])
         and old_keyword == "bordsmargarin"
         and new_keyword == "margarin"
         for required_words, old_keyword, new_keyword in PRODUCT_NAME_SUBSTITUTIONS
     ), True)
# policy_margarin_i_b_bak_stek_margarin: generated by dm matcher add no-match-policy
# sanity-id: policy_margarin_i_b_bak_stek_margarin
test("no-match-policy blocks Margarin Flytande 80% 500ml ICA",
     match("Margarin Flytande 80% 500ml ICA", "margarin", "dairy"), None)
# runtime_pnb_salladslok_farskost: generated by dm matcher add pnb
# sanity-id: runtime_pnb_salladslok_farskost
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB salladslök has färskost",
     "färskost" in PRODUCT_NAME_BLOCKERS.get("salladslök", set()), True)
test("PNB salladslök has färskost",
     "färskost" in PRODUCT_NAME_BLOCKERS.get("salladslök", set()), True)
test("PNB salladslök has cream cheese",
     "cream cheese" in PRODUCT_NAME_BLOCKERS.get("salladslök", set()), True)
# runtime_fpb_innanlar_alginnanlar: generated by dm matcher add fpb
# sanity-id: runtime_fpb_innanlar_alginnanlar
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB innanlår has älginnanlår",
     "älginnanlår" in FALSE_POSITIVE_BLOCKERS.get("innanlår", set()), True)
test("FPB innanlår has älginnanlar",
     "älginnanlar" in FALSE_POSITIVE_BLOCKERS.get("innanlår", set()), True)
# runtime_fpb_koreansk_gochugaru: generated by dm matcher add fpb
# sanity-id: runtime_fpb_koreansk_gochugaru
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB koreansk has gochugaru",
     "gochugaru" in FALSE_POSITIVE_BLOCKERS.get("koreansk", set()), True)
# keyword_synonym_snackpaprika_snackpaprikor: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_snackpaprika_snackpaprikor
test("Keyword synonym snackpaprikor matches snackpaprika",
     match("Snackpaprika 150g Klass 1 ICA", "snackpaprikor", "vegetables"), "snackpaprika")
# runtime_ksbc_paprikor_snackpaprika: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_paprikor_snackpaprika
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC paprikor has snackpaprika",
     "snackpaprika" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("paprikor", set()), True)
# runtime_fpb_fond_fondue: generated by dm matcher add fpb
# sanity-id: runtime_fpb_fond_fondue
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB fond has fondue",
     "fondue" in FALSE_POSITIVE_BLOCKERS.get("fond", set()), True)
# keyword_synonym_vortmix_vortkrydda: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_vortmix_vortkrydda
test("Keyword synonym vörtkrydda matches vörtmix",
     match("Krydda Vörtmix påse 75g Santa Maria", "vörtkrydda", "pantry"), "vörtmix")
# runtime_stop_word_add_mojligt: generated by dm matcher add stop-word
# sanity-id: runtime_stop_word_add_mojligt
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add möjligt",
     "möjligt" in STOP_WORDS, True)
test("stop-word filters extraction möjligt",
     "möjligt" in kw("möjligt"), False)
# keyword_synonym_pilgrimsmussla_pilgrimsmusslor: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_pilgrimsmussla_pilgrimsmusslor
test("Keyword synonym pilgrimsmusslor matches pilgrimsmussla",
     match("Kammusslor 150g ICA", "pilgrimsmusslor", "fish"), "pilgrimsmussla")

# keyword_synonym_dinkelmjol_dinkelsikt: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_dinkelmjol_dinkelsikt
test("Keyword synonym dinkelsikt matches dinkelmjöl",
     match("Dinkelmjöl Siktat 1kg KRAV ICA I love eco", "dinkelsikt", "pantry"), "dinkelmjöl")
# ingredient_parent_kokos_kokoschips: generated by dm matcher add ingredient-parent
# sanity-id: ingredient_parent_kokos_kokoschips
test("ingredient-parent Kokosflakes Rostade 150g ICA Gott liv matches kokos",
     match("Kokosflakes Rostade 150g ICA Gott liv", "Kokoschips", "pantry"), "kokoschips")
# keyword_synonym_viltfars_algkottfars: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_viltfars_algkottfars
test("Keyword synonym älgköttfärs matches viltfärs",
     match("Älgköttfärs Fryst 500g Norrlandsvilt", "älgköttfärs", "meat"), "viltfärs")
test("Keyword synonym renfärs matches viltfärs",
     match("Älgköttfärs Fryst 500g Norrlandsvilt", "renfärs", "meat"), "viltfärs")
# ingredient_parent_broccoli_bellaverde: generated by dm matcher add ingredient-parent
# sanity-id: ingredient_parent_broccoli_bellaverde
test("ingredient-parent Broccoli 250g Klass 1 ICA matches broccoli",
     match("Broccoli 250g Klass 1 ICA", "Bellaverde", "pantry"), "bellaverde")
# runtime_pnb_yuzu_korv: generated by dm matcher add pnb
# sanity-id: runtime_pnb_yuzu_korv
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB yuzu has korv",
     "korv" in PRODUCT_NAME_BLOCKERS.get("yuzu", set()), True)
test("PNB yuzu has kryddkorv",
     "kryddkorv" in PRODUCT_NAME_BLOCKERS.get("yuzu", set()), True)
# runtime_pnb_trattkantarell_torkad: generated by dm matcher add pnb
# sanity-id: runtime_pnb_trattkantarell_torkad
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB trattkantarell has torkad",
     "torkad" in PRODUCT_NAME_BLOCKERS.get("trattkantarell", set()), True)
test("PNB trattkantarell has torkade",
     "torkade" in PRODUCT_NAME_BLOCKERS.get("trattkantarell", set()), True)
# runtime_stop_word_add_lattrokt: generated by dm matcher add stop-word
# sanity-id: runtime_stop_word_add_lattrokt
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add lättrökt",
     "lättrökt" in STOP_WORDS, True)
test("stop-word filters extraction lättrökt",
     "lättrökt" in kw("lättrökt"), False)
test("stop-word add lattrokt",
     "lattrokt" in STOP_WORDS, True)
test("stop-word filters extraction lattrokt",
     "lattrokt" in kw("lattrokt"), False)
# Behavioral regression: lightly-smoked elk shavings must not match a cured-pork
...
from languages.sv.ingredient_matching.specialty_rules import SPECIALTY_QUALIFIERS
test("specialty-qualifier mayo has chipotle",
     "chipotle" in SPECIALTY_QUALIFIERS.get("mayo", []), True)
test("specialty-qualifier mayo has sriracha",
     "sriracha" in SPECIALTY_QUALIFIERS.get("mayo", []), True)
test("specialty-qualifier mayo has mango",
     "mango" in SPECIALTY_QUALIFIERS.get("mayo", []), True)
test("specialty-qualifier mayo has tryffel",
     "tryffel" in SPECIALTY_QUALIFIERS.get("mayo", []), True)
test("specialty-qualifier mayo has vitlök",
     "vitlök" in SPECIALTY_QUALIFIERS.get("mayo", []), True)
test("specialty-qualifier mayo has curry",
     "curry" in SPECIALTY_QUALIFIERS.get("mayo", []), True)
test("specialty-qualifier mayo has wasabi",
     "wasabi" in SPECIALTY_QUALIFIERS.get("mayo", []), True)
test("specialty-qualifier mayo has citron",
     "citron" in SPECIALTY_QUALIFIERS.get("mayo", []), True)
test("specialty-qualifier mayo has dijon",
     "dijon" in SPECIALTY_QUALIFIERS.get("mayo", []), True)
from languages.sv.ingredient_matching.specialty_rules import BIDIRECTIONAL_PER_KEYWORD
test("specialty bidirectional mayo has chipotle",
     "chipotle" in BIDIRECTIONAL_PER_KEYWORD.get("mayo", set()), True)
test("specialty bidirectional mayo has sriracha",
     "sriracha" in BIDIRECTIONAL_PER_KEYWORD.get("mayo", set()), True)
test("specialty bidirectional mayo has mango",
     "mango" in BIDIRECTIONAL_PER_KEYWORD.get("mayo", set()), True)
test("specialty bidirectional mayo has tryffel",
     "tryffel" in BIDIRECTIONAL_PER_KEYWORD.get("mayo", set()), True)
test("specialty bidirectional mayo has vitlök",
     "vitlök" in BIDIRECTIONAL_PER_KEYWORD.get("mayo", set()), True)
test("specialty bidirectional mayo has curry",
     "curry" in BIDIRECTIONAL_PER_KEYWORD.get("mayo", set()), True)
test("specialty bidirectional mayo has wasabi",
     "wasabi" in BIDIRECTIONAL_PER_KEYWORD.get("mayo", set()), True)
test("specialty bidirectional mayo has citron",
     "citron" in BIDIRECTIONAL_PER_KEYWORD.get("mayo", set()), True)
test("specialty bidirectional mayo has dijon",
     "dijon" in BIDIRECTIONAL_PER_KEYWORD.get("mayo", set()), True)
# runtime_specialty_qualifier_majonnas: generated by dm matcher add specialty-qualifier
# sanity-id: runtime_specialty_qualifier_majonnas
from languages.sv.ingredient_matching.specialty_rules import SPECIALTY_QUALIFIERS
test("specialty-qualifier majonnäs has chipotle",
     "chipotle" in SPECIALTY_QUALIFIERS.get("majonnäs", []), True)
test("specialty-qualifier majonnäs has sriracha",
     "sriracha" in SPECIALTY_QUALIFIERS.get("majonnäs", []), True)
test("specialty-qualifier majonnäs has mango",
     "mango" in SPECIALTY_QUALIFIERS.get("majonnäs", []), True)
test("specialty-qualifier majonnäs has tryffel",
     "tryffel" in SPECIALTY_QUALIFIERS.get("majonnäs", []), True)
test("specialty-qualifier majonnäs has vitlök",
     "vitlök" in SPECIALTY_QUALIFIERS.get("majonnäs", []), True)
test("specialty-qualifier majonnäs has curry",
     "curry" in SPECIALTY_QUALIFIERS.get("majonnäs", []), True)
test("specialty-qualifier majonnäs has wasabi",
     "wasabi" in SPECIALTY_QUALIFIERS.get("majonnäs", []), True)
test("specialty-qualifier majonnäs has citron",
     "citron" in SPECIALTY_QUALIFIERS.get("majonnäs", []), True)
test("specialty-qualifier majonnäs has dijon",
     "dijon" in SPECIALTY_QUALIFIERS.get("majonnäs", []), True)
from languages.sv.ingredient_matching.specialty_rules import BIDIRECTIONAL_PER_KEYWORD
test("specialty bidirectional majonnäs has chipotle",
     "chipotle" in BIDIRECTIONAL_PER_KEYWORD.get("majonnäs", set()), True)
test("specialty bidirectional majonnäs has sriracha",
     "sriracha" in BIDIRECTIONAL_PER_KEYWORD.get("majonnäs", set()), True)
test("specialty bidirectional majonnäs has mango",
     "mango" in BIDIRECTIONAL_PER_KEYWORD.get("majonnäs", set()), True)
test("specialty bidirectional majonnäs has tryffel",
     "tryffel" in BIDIRECTIONAL_PER_KEYWORD.get("majonnäs", set()), True)
test("specialty bidirectional majonnäs has vitlök",
     "vitlök" in BIDIRECTIONAL_PER_KEYWORD.get("majonnäs", set()), True)
test("specialty bidirectional majonnäs has curry",
     "curry" in BIDIRECTIONAL_PER_KEYWORD.get("majonnäs", set()), True)
test("specialty bidirectional majonnäs has wasabi",
     "wasabi" in BIDIRECTIONAL_PER_KEYWORD.get("majonnäs", set()), True)
test("specialty bidirectional majonnäs has citron",
     "citron" in BIDIRECTIONAL_PER_KEYWORD.get("majonnäs", set()), True)
test("specialty bidirectional majonnäs has dijon",
     "dijon" in BIDIRECTIONAL_PER_KEYWORD.get("majonnäs", set()), True)

# Behavioral proof for the flavored-mayo isolation (specialty-qualifier mayo/majonnäs).
# A flavored-mayo ingredient (chipotle) must not match a plain mayo product, and a
# plain mayonnaise ingredient must not match a flavored mayo product. Flavored mayo is
# a distinct flavor-defined sauce, not interchangeable with plain mayonnaise.
test(
    "chipotle mayo ingredient does not match plain mayo product",
    recipe_match_num(
        ["1½ dl Santa Maria chipotle chili mayo"],
        {"name": "Mayo 250ml Hellmanns", "category": "sauces"},
    ),
    0,
)
test(
    "plain majonnäs ingredient does not match chipotle mayo product",
    recipe_match_num(
        ["2 dl majonnäs"],
        {"name": "Chipotle Mayo 250ml Test", "category": "sauces"},
    ),
    0,
)
test(
    "plain majonnäs ingredient still matches plain mayo product",
    recipe_match_num(
        ["2 dl majonnäs"],
        {"name": "Mayo 250ml Hellmanns", "category": "sauces"},
    ),
...
# runtime_specialty_qualifier_margarin: generated by dm matcher add specialty-qualifier
# sanity-id: runtime_specialty_qualifier_margarin
from languages.sv.ingredient_matching.specialty_rules import SPECIALTY_QUALIFIERS
test("specialty-qualifier margarin has flytande",
     "flytande" in SPECIALTY_QUALIFIERS.get("margarin", []), True)
from languages.sv.ingredient_matching.specialty_rules import BIDIRECTIONAL_PER_KEYWORD
test("specialty bidirectional margarin has flytande",
     "flytande" in BIDIRECTIONAL_PER_KEYWORD.get("margarin", set()), True)

# Behavioral proof for liquid-margarine isolation. A flytande-margarin recipe must
# match liquid-margarine products (not solid bak/stek margarine), and a plain margarin
# recipe must keep matching only bak/stek margarine and must not match liquid margarine.
test(
    "flytande margarin ingredient matches liquid margarine product",
    recipe_match_num(
        ["1 tsk Flytande Margarin"],
        {"name": "Flytande Margarin 750ml Test", "category": "dairy"},
    ),
    1,
)
test(
    "flytande margarin ingredient does not match bak/stek margarine product",
    recipe_match_num(
        ["1 tsk Flytande Margarin"],
        {"name": "Mat & Bakmargarin 80% Test", "category": "dairy"},
    ),
    0,
)
test(
    "plain margarin ingredient still matches bak/stek margarine product",
    recipe_match_num(
        ["2 msk margarin"],
        {"name": "Mat & Bakmargarin 80% Test", "category": "dairy"},
    ),
...
test(
    "plain margarin ingredient does not match liquid margarine product",
    recipe_match_num(
        ["2 msk margarin"],
        {"name": "Flytande Margarin 750ml Test", "category": "dairy"},
    ),
...
# runtime_pnb_vitlok_majonnas: generated by dm matcher add pnb
# sanity-id: runtime_pnb_vitlok_majonnas
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB vitlök has majonnäs",
     "majonnäs" in PRODUCT_NAME_BLOCKERS.get("vitlök", set()), True)
test("PNB vitlök has majonnas",
     "majonnas" in PRODUCT_NAME_BLOCKERS.get("vitlök", set()), True)
test("PNB vitlök has mayo",
     "mayo" in PRODUCT_NAME_BLOCKERS.get("vitlök", set()), True)
test("PNB vitlök: garlic mayo does not match fresh garlic ingredient",
     recipe_match_num(["vitlöksklyfta"], {"name": "Majonnäs med vitlök 250ml Hellmanns", "category": "pantry"}), 0)
test("PNB vitlök: plain fresh garlic still matches vitlök ingredient",
     recipe_match_num(["vitlöksklyfta"], {"name": "Vitlök 3-pack 150g ICA", "category": "vegetables"}), 1)
# keyword_synonym_vaxtdryck_vaxtmjolk: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_vaxtdryck_vaxtmjolk
test("Keyword synonym växtmjölk matches växtdryck",
     match("Havredryck Naturell 1l ICA", "växtmjölk", "dairy"), "växtdryck")
test("Keyword synonym växtmjolk matches växtdryck",
     match("Havredryck Naturell 1l ICA", "växtmjolk", "dairy"), "växtdryck")
test("Keyword synonym vaxtdryck matches växtdryck",
     match("Havredryck Naturell 1l ICA", "vaxtdryck", "dairy"), "växtdryck")
# keyword_synonym_bongroddar_bongrodd: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_bongroddar_bongrodd
test("Keyword synonym böngrodd matches böngroddar",
     match("Böngroddar Ekologiska 180g KRAV", "böngrodd", "vegetables"), "böngroddar")
test("Keyword synonym bongrodd matches böngroddar",
     match("Böngroddar Ekologiska 180g KRAV", "bongrodd", "vegetables"), "böngroddar")
test("Keyword synonym bongroddar matches böngroddar",
     match("Böngroddar Ekologiska 180g KRAV", "bongroddar", "vegetables"), "böngroddar")
# runtime_ksbc_ost_broccolibollar: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_ost_broccolibollar
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC ost has broccolibollar",
     "broccolibollar" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("ost", set()), True)
# runtime_ksbc_basilika_broccolibollar: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_basilika_broccolibollar
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC basilika has broccolibollar",
     "broccolibollar" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("basilika", set()), True)
test("KSBC broccolibollar: ost blocked from prepared broccoli ball ingredient",
     recipe_match_num(["560 g Broccolibollar med ost och basilika"], {"name": "Hushållsost Götene ca 500g Arla", "category": "dairy"}), 0)
test("KSBC broccolibollar: basilika blocked from prepared broccoli ball ingredient",
     recipe_match_num(["560 g Broccolibollar med ost och basilika"], {"name": "Basilika 13g Eldorado", "category": "pantry"}), 0)
# runtime_stop_word_add_organiskt: generated by dm matcher add stop-word
# sanity-id: runtime_stop_word_add_organiskt
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add organiskt",
     "organiskt" in STOP_WORDS, True)
test("stop-word filters extraction organiskt",
     "organiskt" in kw("organiskt"), False)
test("stop-word add organisk",
     "organisk" in STOP_WORDS, True)
test("stop-word filters extraction organisk",
     "organisk" in kw("organisk"), False)
# runtime_ksbc_chili_snackoliver: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_chili_snackoliver
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC chili has snackoliver",
     "snackoliver" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("chili", set()), True)
# runtime_pnb_chipotle_majonnas: generated by dm matcher add pnb
# sanity-id: runtime_pnb_chipotle_majonnas
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB chipotle has majonnäs",
     "majonnäs" in PRODUCT_NAME_BLOCKERS.get("chipotle", set()), True)
test("PNB chipotle has majonnas",
     "majonnas" in PRODUCT_NAME_BLOCKERS.get("chipotle", set()), True)
test("PNB chipotle has mayo",
     "mayo" in PRODUCT_NAME_BLOCKERS.get("chipotle", set()), True)
# offer_extra_keyword_pomerans_pomeransskal: generated by dm matcher add offer-extra-keyword
# sanity-id: offer_extra_keyword_pomerans_pomeransskal
test("offer-extra-keyword Pomeransskal matches pomerans",
     match("Pomeransskal", "pomerans", "pantry"), "pomerans")
# runtime_important_short_keyword_add_ponzu: generated by dm matcher add important-short-keyword
# sanity-id: runtime_important_short_keyword_add_ponzu
from languages.sv.ingredient_matching.keywords import IMPORTANT_SHORT_KEYWORDS
test("important-short-keyword add ponzu",
     "ponzu" in IMPORTANT_SHORT_KEYWORDS, True)
# runtime_important_short_keyword_add_talg: generated by dm matcher add important-short-keyword
# sanity-id: runtime_important_short_keyword_add_talg
from languages.sv.ingredient_matching.keywords import IMPORTANT_SHORT_KEYWORDS
test("important-short-keyword add talg",
     "talg" in IMPORTANT_SHORT_KEYWORDS, True)
# keyword_synonym_vaniljstang_vaniljstanger: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_vaniljstang_vaniljstanger
test("Keyword synonym vaniljstänger matches vaniljstång",
     match("Vaniljstång 3,5g Dr.Oetker", "vaniljstänger", "pantry"), "vaniljstång")
test("Keyword synonym vaniljstanger matches vaniljstång",
     match("Vaniljstång 3,5g Dr.Oetker", "vaniljstanger", "pantry"), "vaniljstång")
# runtime_fpb_broccoli_broccolibollar: generated by dm matcher add fpb
# sanity-id: runtime_fpb_broccoli_broccolibollar
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB broccoli has broccolibollar",
     "broccolibollar" in FALSE_POSITIVE_BLOCKERS.get("broccoli", set()), True)
# Raw broccoli must not match the formed veggie-ball half-product "broccolibollar"
test("FPB broccolibollar: raw broccoli blocked from prepared broccoli ball ingredient",
     recipe_match_num(["560 g Broccolibollar med ost och basilika"], {"name": "Broccoli 600g Findus", "category": "vegetables"}), 0)
# Plain broccoli ingredient must still match raw broccoli offers
test("FPB broccolibollar: plain broccoli still matches raw broccoli",
     recipe_match_num(["200 g broccoli"], {"name": "Broccoli 600g Findus", "category": "vegetables"}) >= 1, True)
# runtime_fpb_lok_loktrav: generated by dm matcher add fpb
# sanity-id: runtime_fpb_lok_loktrav
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB lök has löktrav",
     "löktrav" in FALSE_POSITIVE_BLOCKERS.get("lök", set()), True)
# löktrav (garlic mustard, a wild foraged herb) must not match plain onion offers
test("FPB lök: löktrav herb does not match plain onion",
     recipe_match_num(["löktrav"], {"name": "Lök gul ca 180g Klass 1 ICA", "category": "vegetables"}), 0)
# Plain onion ingredient must still match onion offers
test("FPB lök: plain gul lök still matches onion",
     recipe_match_num(["1 gul lök"], {"name": "Lök gul ca 180g Klass 1 ICA", "category": "vegetables"}) >= 1, True)
# runtime_ksbc_vitlok_krutonger: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_vitlok_krutonger
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC vitlök has krutonger",
     "krutonger" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("vitlök", set()), True)
# Garlic croutons (Zeta Krutonger Vitlök & Persilja) are a crouton carrier product;
# the garlic-flavor keyword must not pull raw garlic bulbs into the recipe.
test("KSBC vitlök krutonger: garlic croutons do not match raw garlic bulbs",
     recipe_match_num(["2 dl Zeta Krutonger Vitlök & Persilja"], {"name": "Vitlök 250g Klass 1 ICA", "category": "vegetables"}), 0)
# The actual crouton product must still match via the krutonger keyword.
test("KSBC vitlök krutonger: garlic crouton ingredient still matches crouton product",
     recipe_match_num(["2 dl Zeta Krutonger Vitlök & Persilja"], {"name": "Krutonger Vitlök & persilja 120g Zeta", "category": "other"}) >= 1, True)
# Plain garlic cloves must still match raw garlic.
test("KSBC vitlök krutonger: plain garlic cloves still match raw garlic",
     recipe_match_num(["2 hackade vitlöksklyftor"], {"name": "Vitlök 250g Klass 1 ICA", "category": "vegetables"}) >= 1, True)
# runtime_ksbc_kraftstjartar_havskraft: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_kraftstjartar_havskraft
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC kräftstjärtar has havskräft",
     "havskräft" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kräftstjärtar", set()), True)
test("KSBC kräftstjärtar has havskrafta",
     "havskrafta" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("kräftstjärtar", set()), True)
# keyword_synonym_drottningsylt_drottningssylt: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_drottningsylt_drottningssylt
test("Keyword synonym drottningssylt matches drottningsylt",
     match("Drottningsylt Extra fin 400g ICA", "drottningssylt", "pantry"), "drottningsylt")
# keyword_synonym_frityrolja_friteringsolja: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_frityrolja_friteringsolja
test("Keyword synonym friteringsolja matches frityrolja",
     match("Frityrolja 2l ICA", "friteringsolja", "pantry"), "frityrolja")
# runtime_pnb_kokoschips_aminos: generated by dm matcher add pnb
# sanity-id: runtime_pnb_kokoschips_aminos
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB kokoschips has aminos",
     "aminos" in PRODUCT_NAME_BLOCKERS.get("kokoschips", set()), True)
test("PNB kokoschips has nick",
     "nick" in PRODUCT_NAME_BLOCKERS.get("kokoschips", set()), True)
test("PNB kokoschips has pineapple",
     "pineapple" in PRODUCT_NAME_BLOCKERS.get("kokoschips", set()), True)
test("PNB kokoschips has chokladbar",
     "chokladbar" in PRODUCT_NAME_BLOCKERS.get("kokoschips", set()), True)
test("PNB kokoschips has skumtoppar",
     "skumtoppar" in PRODUCT_NAME_BLOCKERS.get("kokoschips", set()), True)
# runtime_important_short_keyword_add_scampi: generated by dm matcher add important-short-keyword
# sanity-id: runtime_important_short_keyword_add_scampi
from languages.sv.ingredient_matching.keywords import IMPORTANT_SHORT_KEYWORDS
test("important-short-keyword add scampi",
     "scampi" in IMPORTANT_SHORT_KEYWORDS, True)
# keyword_extra_parent_scampi_family: generated by dm matcher add keyword-extra-parent
# sanity-id: keyword_extra_parent_scampi_family
test("Scampi recipe matches tigerräkor",
     match("Tigerräkor", "300 g tinad scampi", "pantry"), "scampi")
test("Scampi recipe matches jätteräkor",
     match("Jätteräkor", "300 g tinad scampi", "pantry"), "scampi")
# keyword_extra_parent_notdryck_family: generated by dm matcher add keyword-extra-parent
# sanity-id: keyword_extra_parent_notdryck_family
test("Mandeldryck ≠ 'nötdryck' (mandel är botaniskt en stenfrukt, ingen nöt)",
     match("Mandeldryck", "2 dl naturell nötdryck", "pantry"), None)
test("Nötdryck recipe matches hasselnötsdryck",
     match("Hasselnötsdryck", "2 dl naturell nötdryck", "pantry"), "nötdryck")
test("Nötdryck recipe matches cashewdryck",
     match("Cashewdryck", "2 dl naturell nötdryck", "pantry"), "nötdryck")
test("Nötdryck recipe matches makadamiadryck",
     match("Makadamiadryck", "2 dl naturell nötdryck", "pantry"), "nötdryck")

# Optional plant-based wording ("gärna vegansk", "välj växtbaserad") only
# SUGGESTS a plant swap — the ordinary dairy base must still match. A bare
# cue with no optional marker ("vegansk grädde") stays a hard requirement.
test("Gräddfil 'gärna vegansk' still matches dairy gräddfil",
     match("Gräddfil 5dl Arla", "2 dl gräddfil, gärna vegansk", "dairy"), "gräddfil")
test("Yoghurt 'välj växtbaserad' still matches dairy yoghurt",
     match("Yoghurt Naturell 1000g Arla", "3 dl yoghurt (välj växtbaserad för en vegansk måltid)", "dairy"), "yoghurt")
test("Yoghurt 'välj växtbaserad' also matches a plant yoghurt",
     match("iMat Yoghurt Naturell 1000g", "3 dl yoghurt (välj växtbaserad för en vegansk måltid)", "dairy"), "yoghurt")
test("Hard requirement 'vegansk grädde' still blocks dairy grädde",
     match("Vispgrädde 5dl Arla", "2 dl vegansk grädde", "dairy"), None)
test("Hard requirement 'växtbaserad yoghurt' still blocks dairy yoghurt",
     match("Yoghurt Naturell 1000g Arla", "3 dl växtbaserad yoghurt", "dairy"), None)

# Plain "fänkål" is the vegetable bulb and must not match fänkålsfrö (the spice
# seed); a recipe meaning the seed should spell it out. Deliberate FN, locked.
test("Plain 'fänkål' (vegetable) does not match fänkålsfrö seed jars",
     match("Fänkålsfrö hela 35g Santa Maria", "8 g fänkål", "pantry"), None)

# "havrebaserad dryck/mjölk" should reach ordinary havredryck products, but the
# phrase-specific normalization must not pull oat CREAM ("matlagningsgrädde") in.
test("Havrebaserad dryck matches havredryck",
     match("Havredryck 1l Oatly", "2 dl havrebaserad dryck", "dairy"), "havredryck")
test("Havrebaserad matlagningsgrädde does not match havredryck (cream, not drink)",
     match("Havredryck 1l Oatly", "2 dl havrebaserad matlagningsgrädde", "dairy"), None)
# keyword_synonym_rodbeta_beta: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_rodbeta_beta
test("Keyword synonym beta matches rödbeta",
     match("Rödbetor 1kg Klass 1", "beta", "vegetables"), "rödbeta")

# Bare "beta" routes to rödbeta, but the specific beets keep their own canonical
# and must stay isolated from rödbeta in both directions.
test("Bare 'beta' does not match gulbeta product",
     match("Gulbeta 500g Klass 1", "400 g beta", "vegetables"), None)
test("Gulbeta ingredient does not match rödbeta product",
     match("Rödbetor 1kg Klass 1", "2 gulbeta", "vegetables"), None)
# runtime_stop_word_add_fruktsmak: generated by dm matcher add stop-word
# sanity-id: runtime_stop_word_add_fruktsmak
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add fruktsmak",
     "fruktsmak" in STOP_WORDS, True)
test("stop-word filters extraction fruktsmak",
     "fruktsmak" in kw("fruktsmak"), False)
# "fruktsmak" is a generic flavor descriptor, not a food noun. Without the
# stop word it let a candy/chocolate bag match a fruit-flavored tea-bag
# ingredient (chocolate is a different product type than tea).
test("Chokladpåse Fruktsmak does not match fruit-flavored tea bags",
     match("Chokladpåse Fruktsmak 245g Wawel",
           "2 portionspåsar te med fruktsmak", "pantry"), None)
# ingredient_parent_chili_gochugaru: generated by dm matcher add ingredient-parent
# sanity-id: ingredient_parent_chili_gochugaru
test("ingredient-parent gochugaru routes to chili (frozen chopped chili matches via gochugaru reverse-child)",
     match("Röd chili hackad Fryst 50g ICA", "2 tsk gochugaru", "spices"), "gochugaru")
# runtime_stop_word_add_stenugnsbakad: generated by dm matcher add stop-word
# sanity-id: runtime_stop_word_add_stenugnsbakad
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add stenugnsbakad",
     "stenugnsbakad" in STOP_WORDS, True)
test("stop-word filters extraction stenugnsbakad",
     "stenugnsbakad" in kw("stenugnsbakad"), False)
# Behavioral lock: a complete topped frozen pizza must NOT match a plain pizza-base
# ingredient just because both say "stenugnsbakad". The base ingredient still matches
# real pizza-base products via the "pizzabotten" keyword.
test("stenugnsbakad complete pizza does not match pizzabotten ingredient",
     match("Pizza Capricciosa stenugnsbakad 370g ICA",
           "2 Fontana stenugnsbakad pizzabotten 30 cm (2 pack à 360 g)", "frozen"), None)
test("real pizzabotten still matches pizzabotten ingredient",
     match("Pizzabotten Stenugnsbakad Vete 30cm 2-pack 360g Fontana",
           "2 Fontana stenugnsbakad pizzabotten 30 cm (2 pack à 360 g)", "deli"), "pizzabotten")
# keyword_synonym_aprikos_aprikoser: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_aprikos_aprikoser
test("Keyword synonym aprikos/aprikoser: singular aprikos ingredient matches plural Aprikoser product",
     match("Torkade Aprikoser 250g ICA Basic", "strimlade torkade aprikos", "pantry"), "aprikos")
test("Keyword synonym aprikos/aprikoser: plural aprikoser ingredient matches plural Aprikoser product",
     match("Torkade Aprikoser 250g ICA Basic", "strimlade torkade aprikoser", "pantry"), "aprikos")
# keyword_synonym_zaatar_za_atar: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_zaatar_za_atar
test("Keyword synonym za'atar matches zaatar",
     match("Krydda Zaatar Manakish Burk 500g Sevan", "za'atar", "spices"), "zaatar")
# runtime_pnb_pepparkaka_basic: generated by dm matcher add pnb
# sanity-id: runtime_pnb_pepparkaka_basic
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB pepparkaka has basic",
     "basic" in PRODUCT_NAME_BLOCKERS.get("pepparkaka", set()), True)
# runtime_fpb_kal_skal: generated by dm matcher add fpb
# sanity-id: runtime_fpb_kal_skal
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB kål has skål",
     "skål" in FALSE_POSITIVE_BLOCKERS.get("kål", set()), True)
# runtime_pnb_tortelloni_fardigmat: generated by dm matcher add pnb
# sanity-id: runtime_pnb_tortelloni_fardigmat
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB tortelloni has färdigmat",
     "färdigmat" in PRODUCT_NAME_BLOCKERS.get("tortelloni", set()), True)
# runtime_pnb_tacoskal_kyckling: generated by dm matcher add pnb
# sanity-id: runtime_pnb_tacoskal_kyckling
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB tacoskal has kyckling",
     "kyckling" in PRODUCT_NAME_BLOCKERS.get("tacoskal", set()), True)
# runtime_fpb_taco_tacoskal: generated by dm matcher add fpb
# sanity-id: runtime_fpb_taco_tacoskal
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB taco has tacoskal",
     "tacoskal" in FALSE_POSITIVE_BLOCKERS.get("taco", set()), True)
# runtime_pnb_notkott_hamburgerfars: generated by dm matcher add pnb
# sanity-id: runtime_pnb_notkott_hamburgerfars
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB nötkött has hamburgerfärs",
     "hamburgerfärs" in PRODUCT_NAME_BLOCKERS.get("nötkött", set()), True)
# offer_extra_keyword_entrecote_lammentrecote: generated by dm matcher add offer-extra-keyword
# sanity-id: offer_extra_keyword_entrecote_lammentrecote
# A lamb entrecôte recipe reaches the Lammentrecote product via the exposed
# 'entrecote' keyword (plain beef "entrecote" is correctly blocked by the
# bidirectional lamm species qualifier, so it cannot be used here).
test("offer-extra-keyword Lammentrecote exposes entrecote (lamb recipe)",
     match("Lammentrecote ca 300g Scan", "500 g entrecôte av lamm", "meat"), "entrecote")
# runtime_specialty_qualifier_entrecote: generated by dm matcher add specialty-qualifier
# sanity-id: runtime_specialty_qualifier_entrecote
from languages.sv.ingredient_matching.specialty_rules import SPECIALTY_QUALIFIERS
test("specialty-qualifier entrecote has lamm",
     "lamm" in SPECIALTY_QUALIFIERS.get("entrecote", []), True)
from languages.sv.ingredient_matching.specialty_rules import BIDIRECTIONAL_PER_KEYWORD
test("specialty bidirectional entrecote has lamm",
     "lamm" in BIDIRECTIONAL_PER_KEYWORD.get("entrecote", set()), True)
# runtime_ksbc_hamburgare_burger_mix: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_hamburgare_burger_mix
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC hamburgare has burger mix",
     "burger mix" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("hamburgare", set()), True)
test("KSBC hamburgare has burgermix",
     "burgermix" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("hamburgare", set()), True)
# runtime_ksbc_burgare_burger_mix: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_burgare_burger_mix
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC burgare has burger mix",
     "burger mix" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("burgare", set()), True)
test("KSBC burgare has burgermix",
     "burgermix" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("burgare", set()), True)

# ---------------------------------------------------------------------------
# Batch-review decisions — regression coverage for manual backend/code edits.
# Uses the full backend pipeline (recipe_match_num) because several fixes are
# backend-only (the fast path may still match while validation rejects).
# ---------------------------------------------------------------------------

# Dried/ground spice forms are interchangeable in the strict spice rules:
# "Torkad gurkmeja" must match ground "Gurkmeja Malen"; fresh stays blocked.
test("torkad gurkmeja matches ground Gurkmeja Malen",
     recipe_match_num(["1 msk torkad gurkmeja"],
                      {"name": "Gurkmeja Malen 43g ICA", "category": "spices"}) > 0, True)
test("färsk gurkmeja stays blocked from ground Gurkmeja Malen",
     recipe_match_num(["10 g färsk gurkmeja"],
                      {"name": "Gurkmeja Malen 43g ICA", "category": "spices"}), 0)

# An "X t ex ..." example list must not let pasta-shape words (rigate/gnocchi)
# trip carrier_flavor_context and suppress the broad pasta family.
test("pasta t ex-list matches plain pasta",
     recipe_match_num(["300 g pasta t ex fusilli, penner rigate, gnocchi"],
                      {"name": "Pasta Fusilli 500g Barilla", "category": "pantry"}) > 0, True)

# gochugaru is definitionally Korean dried chili flakes and must satisfy the
# strict chili processed-product rule for chiliflakes products.
test("gochugaru matches Koreanska Chiliflingor",
     recipe_match_num(["2 tsk gochugaru"],
                      {"name": "Koreanska Chiliflingor 50g Risberg", "category": "spices"}) > 0, True)

# Margarin is a vegetable-fat butter substitute = inherently dairy-free, so
# "mjölkfritt margarin" matches plain bak/stek-margarin; a dairy-free MILK
# recipe must still not match a margarin product.
test("mjölkfritt margarin matches bak/stek-margarin",
     recipe_match_num(["50 g mjölkfritt margarin"],
                      {"name": "Mat & bakmargarin 80% ICA", "category": "dairy"}) > 0, True)
test("mjölkfri mjölk does not match Margarin Mjölkfritt",
     recipe_match_num(["3 dl mjölkfri mjölk"],
                      {"name": "Margarin Mjölkfritt 600g", "category": "dairy"}), 0)

# Freeze-dried fruit with a frystorkad specialty qualifier exposes the fruit
# keyword, isolated bidirectionally to freeze-dried recipes/products.
test("frystorkade jordgubbar matches freeze-dried product",
     recipe_match_num(["2 dl frystorkade jordgubbar"],
                      {"name": "Jordgubbar Frystorkade 45g Butiksodlat", "category": "pantry"}) > 0, True)
test("plain färska jordgubbar do not match freeze-dried product",
     recipe_match_num(["250 g färska jordgubbar"],
                      {"name": "Jordgubbar Frystorkade 45g Butiksodlat", "category": "fruit"}), 0)
test("frystorkade jordgubbar do not match fresh product",
     recipe_match_num(["2 dl frystorkade jordgubbar"],
                      {"name": "Jordgubbar Färska 250g Klass 1", "category": "fruit"}), 0)

# Blåbär frystorkad: same bidirectional frystorkad specialty qualifier as jordgubb/hallon.
# "Blåbär frystorkade" product exposes blåbär keyword; plain blåbär recipe is isolated from it.
test("frystorkade blåbär ingredient matches freeze-dried blåbär product",
     recipe_match_num(["1 dl frystorkade blåbär"],
                      {"name": "Blåbär frystorkade 25g Panfruit", "category": "pantry"}) > 0, True)
test("plain blåbär recipe does not match freeze-dried blåbär product",
     recipe_match_num(["1 dl blåbär"],
                      {"name": "Blåbär frystorkade 25g Panfruit", "category": "pantry"}), 0)

# Plain "persilja" (no färsk/torkad qualifier) accepts both fresh and dried;
# explicit qualifiers still isolate the form.
test("plain persilja matches fresh Persilja Flowpack",
     recipe_match_num(["1 msk persilja"],
                      {"name": "Persilja Flowpack 20g Klass 1 ICA", "category": "vegetables"}) > 0, True)
test("plain persilja matches dried persilja",
     recipe_match_num(["1 msk persilja"],
                      {"name": "Persilja Torkad 12g Santa Maria", "category": "spices"}) > 0, True)
test("torkad persilja does not match fresh Persilja Flowpack",
     recipe_match_num(["1 msk torkad persilja"],
                      {"name": "Persilja Flowpack 20g Klass 1 ICA", "category": "vegetables"}), 0)
test("färsk persilja does not match dried persilja",
     recipe_match_num(["1 msk färsk persilja"],
                      {"name": "Persilja Torkad 12g Santa Maria", "category": "spices"}), 0)

# A satisfied product-name blocker ("rökt") must not bypass an unrelated one
# ("lax"): smoked-paprika spice must not match smoked salmon, but must still
# match a genuine smoked-paprika spice product.
test("rökt paprikapulver does not match varmrökt lax",
     recipe_match_num(["1 tsk Rökt paprikapulver"],
                      {"name": "Lax Färsk Varmrökt Paprika & Peppar ICA", "category": "fish"}), 0)
test("rökt paprikapulver still matches smoked paprika spice",
     recipe_match_num(["1 tsk Rökt paprikapulver"],
                      {"name": "Paprikapulver Rökt 40g Santa Maria", "category": "spices"}) > 0, True)

# Entrecôte species-split: lamb entrecôte matches lamb products, not beef, and
# a plain (beef) entrecôte recipe must not match a lamb entrecôte product.
test("lammentrecôte matches Lammentrecote product",
     recipe_match_num(["500 g lammentrecôte"],
                      {"name": "Lammentrecote ca 300g Scan", "category": "meat"}) > 0, True)
test("plain entrecôte does not match Lammentrecote product",
     recipe_match_num(["600 g entrecôte"],
                      {"name": "Lammentrecote ca 300g Scan", "category": "meat"}), 0)
# keyword_synonym_salladsmix_mixsallad: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_salladsmix_mixsallad
test("Keyword synonym mixsallad matches salladsmix",
     match("Salladsmix Sköljd 225g ICA", "mixsallad", "vegetables"), "salladsmix")
# runtime_fpb_biff_laxbiffar: generated by dm matcher add fpb
# sanity-id: runtime_fpb_biff_laxbiffar
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB biff has laxbiffar",
     "laxbiffar" in FALSE_POSITIVE_BLOCKERS.get("biff", set()), True)
test("FPB biff has laxbiff",
     "laxbiff" in FALSE_POSITIVE_BLOCKERS.get("biff", set()), True)
# runtime_pnb_betor_juice: generated by dm matcher add pnb
# sanity-id: runtime_pnb_betor_juice
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB betor has juice",
     "juice" in PRODUCT_NAME_BLOCKERS.get("betor", set()), True)
# runtime_ksbc_taco_lax: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_taco_lax
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC taco has lax",
     "lax" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("taco", set()), True)
...
from languages.sv.ingredient_matching.specialty_rules import SPECIALTY_QUALIFIERS
test("specialty-qualifier pepparsås has hot",
     "hot" in SPECIALTY_QUALIFIERS.get("pepparsås", []), True)
from languages.sv.ingredient_matching.specialty_rules import BIDIRECTIONAL_PER_KEYWORD
test("specialty bidirectional pepparsås has hot",
     "hot" in BIDIRECTIONAL_PER_KEYWORD.get("pepparsås", set()), True)
# runtime_fpb_liba_libanesisk: generated by dm matcher add fpb
# sanity-id: runtime_fpb_liba_libanesisk
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB liba has libanesisk",
     "libanesisk" in FALSE_POSITIVE_BLOCKERS.get("liba", set()), True)
# runtime_space_normalization_farsk_potatis_farskpotatis: generated by dm matcher add space-normalization
# sanity-id: runtime_space_normalization_farsk_potatis_farskpotatis
test("space-normalization färsk potatis -> färskpotatis",
     _apply_space_normalizations("färsk potatis"), "färskpotatis")
# Behavioral regressions: "färsk potatis" (two-word) extraction and isolation
test("KW färsk potatis (two-word) extracts färskpotatis",
     extract_keywords_from_ingredient("500 g kokt kall färsk potatis"), ["färskpotatis"])
test("färsk potatis ingredient does NOT match generic mealy potatis",
     recipe_match_num(["500 g färsk potatis"], {"name": "Mjölig potatis 1,2kg Klass 1", "category": "produce"}), 0)
test("färsk potatis ingredient matches Färskpotatis product",
     recipe_match_num(["500 g färsk potatis"], {"name": "Färskpotatis 800g Felix", "category": "produce"}), 1)
# runtime_pnb_rapsolja_spray: generated by dm matcher add pnb
# sanity-id: runtime_pnb_rapsolja_spray
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB rapsolja has spray",
     "spray" in PRODUCT_NAME_BLOCKERS.get("rapsolja", set()), True)
# runtime_ksbc_woksas_phad_thai: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_woksas_phad_thai
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC woksås has phad thai",
     "phad thai" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("woksås", set()), True)
test("KSBC woksås has phadthai",
     "phadthai" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("woksås", set()), True)
# runtime_ksbc_taco_kryddblandning: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_taco_kryddblandning
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC taco has kryddblandning",
     "kryddblandning" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("taco", set()), True)
# ingredient_parent_styckbrod_frallor: frallor (plural) maps to styckbröd via ingredient_parent
# sanity-id: ingredient_parent_styckbrod_frallor
test("ingredient-parent frallor → styckbröd: Fralla Rundstycke matches frallor ingredient",
     match("Fralla Rundstycke 60g Eget Bageri", "Frallor", "bread"), "frallor")
test("KW frallor extracts styckbröd keyword",
     extract_keywords_from_ingredient("4 frallor"), ["styckbröd"])
# runtime_pnb_plommontomater_passerad: generated by dm matcher add pnb
# sanity-id: runtime_pnb_plommontomater_passerad
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB plommontomater has passerad",
     "passerad" in PRODUCT_NAME_BLOCKERS.get("plommontomater", set()), True)
test("PNB plommontomater has passerade",
     "passerade" in PRODUCT_NAME_BLOCKERS.get("plommontomater", set()), True)
test("PNB plommontomater: Passerade tomater (passata) blocked from fresh plommontomater",
     "passerade" in PRODUCT_NAME_BLOCKERS.get("plommontomater", set()), True)
test("PNB plommontomater: real plommon tomato still matches (fast path)",
     match("Plommontomater 400g Mutti", "plommontomater", "produce") is not None, True)
# ingredient_parent_pumpafron_pumpakarna: generated by dm matcher add ingredient-parent
# sanity-id: ingredient_parent_pumpafron_pumpakarna
test("ingredient-parent pumpakärna → pumpafrön: Pumpakärnor matches pumpakärna ingredient",
     match("Pumpakärnor 300g ICA", "Pumpakärna", "pantry"), "pumpakärna")
test("KW pumpakärna extracts pumpafrön",
     extract_keywords_from_ingredient("0,7 dl pumpakärna"), ["pumpafrön"])
# runtime_pnb_pumpakarna_musli: generated by dm matcher add pnb
# sanity-id: runtime_pnb_pumpakarna_musli
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
...
# keyword_extra_parent_vegokott_family: generated by dm matcher add keyword-extra-parent
# sanity-id: keyword_extra_parent_vegokott_family
test("Vegokött recipe matches quorn",
     match("Quorn Färs 300g", "600 g vegokött eller liknande", "frozen"), "vegokött")
test("Vegokött recipe matches anamma",
     match("Anamma Vego Bitar 350g", "600 g vegokött eller liknande", "frozen"), "vegokött")
test("Vegokött recipe matches hälsans",
     match("Hälsans Kök Färs 350g", "600 g vegokött eller liknande", "frozen"), "vegokött")
test("Vegokött recipe matches beyond",
     match("Beyond Mince 350g", "600 g vegokött eller liknande", "frozen"), "vegokött")
test("Vegokött recipe matches vegofärs",
     match("Vegofärs ICA 350g", "600 g vegokött eller liknande", "frozen"), "vegokött")
test("Vegokött recipe matches vegobitar",
     match("Vegobitar 200g", "600 g vegokött eller liknande", "frozen"), "vegokött")
# keyword_extra_parent_vegokott_family: generated by dm matcher add keyword-extra-parent
# sanity-id: keyword_extra_parent_vegokott_family
test("Vegopulled no longer matches vegokött parent",
     match("Pulled Oumph BBQ 280g", "600 g vegokött eller liknande", "frozen"), None)
# keyword_extra_parent_vegokott_family: generated by dm matcher add keyword-extra-parent
# sanity-id: keyword_extra_parent_vegokott_family
test("Oumph no longer matches vegokött parent",
     match("Oumph The Chunk 280g", "600 g vegokött eller liknande", "frozen"), None)

# Vego-protein form bridge: a "vegofärs"/"vegobitar" recipe reaches plant-protein
# brand products of the SAME form (Quorn Färs, Hälsans Kök Färs, Beyond Mince), but
# not a different form (filé/bitar) and never real meat. Mirrors "färs matchar färs".
test("vegofärs matches Quorn Färs (form bridge)",
     recipe_match_num(["400 g vegofärs"], {"name": "Quorn Färs 300g", "category": "frozen"}), 1)
test("vegofärs matches Hälsans Kök Färs",
     recipe_match_num(["400 g vegofärs"], {"name": "Hälsans Kök Färs 350g", "category": "frozen"}), 1)
test("vegofärs matches Beyond Mince",
     recipe_match_num(["400 g vegofärs"], {"name": "Beyond Mince 350g", "category": "frozen"}), 1)
test("vegofärs does NOT match Quorn Filé (form respected)",
     recipe_match_num(["400 g vegofärs"], {"name": "Quorn Filé 250g", "category": "frozen"}), 0)
test("vegofärs does NOT match Quorn Bitar (form respected)",
     recipe_match_num(["400 g vegofärs"], {"name": "Quorn Bitar 300g", "category": "frozen"}), 0)
test("vegobitar matches Quorn Bitar (form bridge)",
...
test("generic vegokött matches Beyond Mince",
     recipe_match_num(["600 g vegokött eller liknande"], {"name": "Beyond Mince 350g", "category": "frozen"}), 1)
test("generic vegokött does NOT match real köttfärs",
     recipe_match_num(["600 g vegokött eller liknande"], {"name": "Köttfärs 1kg Garant", "category": "meat"}), 0)
# runtime_pnb_cheddar_crackers: generated by dm matcher add pnb
# sanity-id: runtime_pnb_cheddar_crackers
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB cheddar has crackers",
     "crackers" in PRODUCT_NAME_BLOCKERS.get("cheddar", set()), True)
# runtime_pnb_hallon_chokladpase: generated by dm matcher add pnb
# sanity-id: runtime_pnb_hallon_chokladpase
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB hallon has chokladpåse",
     "chokladpåse" in PRODUCT_NAME_BLOCKERS.get("hallon", set()), True)
# runtime_pnb_ost_ostbagar: generated by dm matcher add pnb
# sanity-id: runtime_pnb_ost_ostbagar
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB ost has ostbågar",
     "ostbågar" in PRODUCT_NAME_BLOCKERS.get("ost", set()), True)
test("PNB ost has ostbåge",
     "ostbåge" in PRODUCT_NAME_BLOCKERS.get("ost", set()), True)
# runtime_pnb_styckbrod_bakmix: generated by dm matcher add pnb
# sanity-id: runtime_pnb_styckbrod_bakmix
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB styckbröd has bakmix",
     "bakmix" in PRODUCT_NAME_BLOCKERS.get("styckbröd", set()), True)
# runtime_pnb_vitlok_flaskytterfile: generated by dm matcher add pnb
# sanity-id: runtime_pnb_vitlok_flaskytterfile
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB vitlök has fläskytterfilé",
     "fläskytterfilé" in PRODUCT_NAME_BLOCKERS.get("vitlök", set()), True)
# ingredient_parent_viltfars_algfars: generated by dm matcher add ingredient-parent
# sanity-id: ingredient_parent_viltfars_algfars
test("ingredient-parent Hjortfärs Fryst 500g GW:s utvalda matches viltfärs",
     match("Hjortfärs Fryst 500g GW:s utvalda", "Älgfärs", "pantry"), "älgfärs")
# policy_kornris_basmati_jasmin_langkorn_langkorn_rundkorn_parboiled_raris_raris_fullkornsris_bris_b: generated by dm matcher add no-match-policy
# sanity-id: policy_kornris_basmati_jasmin_langkorn_langkorn_rundkorn_parboiled_raris_raris_fullkornsris_bris_b
test("no-match-policy blocks Basmatiris Uncle Bens 1kg",
     match("Basmatiris Uncle Bens 1kg", "kornris", "pantry"), None)
# ingredient_parent_kottbuljong_notbuljong: generated by dm matcher add ingredient-parent
# sanity-id: ingredient_parent_kottbuljong_notbuljong
test("ingredient-parent Oxbuljong Kockens 1l matches köttbuljong",
     match("Oxbuljong Kockens 1l", "Nötbuljong", "pantry"), "buljong")
# runtime_context_word_exemption_salchichon: generated by dm matcher add context-word-exemption
# sanity-id: runtime_context_word_exemption_salchichon
from languages.sv.ingredient_matching.carrier_context import CONTEXT_WORD_KEYWORD_EXEMPTIONS
...
test(
    "senap broad family: plain senap recipe matches Sotstark product (Q126-3 no bidirectional)",
    recipe_match_num(
        ["2 msk senap"],
...
test(
    "senap ingredient-specific: dijonsenap ingredient does NOT match plain Original (ingredient-side qualifier)",
    recipe_match_num(
        ["1 msk dijonsenap"],
        {"name": "Senap Original Johnny's", "category": "other"},
    ),
...
# Skånsk and dijon mustard are isolated to themselves in BOTH directions: a
# skånsk/dijon PRODUCT must not satisfy a plain "senap" recipe (offer-side,
# bidirectional). All other varieties (sötstark, grovkornig ...) stay broad.
test(
    "senap isolering: Skånsk Senap-produkt matchar INTE plain senap-recept (offer-side bidirectional)",
    recipe_match_num(
        ["2 msk senap"],
        {"name": "Skånsk Senap Slotts", "category": "other"},
    ),
    0,
)
test(
    "senap isolering: Skånsk Senap-produkt matchar INTE plain senap-recept (cached path)",
    recipe_match_num_cached(
        ["2 msk senap"],
        {"name": "Skånsk Senap Slotts", "category": "other"},
    ),
...
test(
    "senap isolering: Senap Dijon-produkt matchar INTE plain senap-recept (offer-side bidirectional)",
    recipe_match_num(
        ["2 msk senap"],
        {"name": "Senap Dijon Original Slotts", "category": "other"},
    ),
    0,
)
test(
    "senap isolering: skånsk senap-recept matchar Skånsk Senap-produkt (self-match positiv)",
    recipe_match_num(
        ["1 msk skånsk senap"],
        {"name": "Skånsk Senap Slotts", "category": "other"},
    ),
...
test(
    "senap bred: sötstark senap-recept matchar plain Senap Original (övriga sorter förblir breda)",
    recipe_match_num(
...
# current_review_pasta_family: generated by dm matcher add ingredient-parent
# sanity-id: current_review_pasta_family
test("ingredient-parent Pasta Fusilli 500g matches pasta",
     recipe_match_num_cached(["idealmakaroner"], {"name": "Pasta Fusilli 500g", "category": "pantry"}), 1)
test("ingredient-parent Pasta Fusilli 500g matches gammaldags makaroner as pasta",
     recipe_match_num_cached(["gammaldags makaroner"], {"name": "Pasta Fusilli 500g", "category": "pantry"}), 1)
test("ingredient-parent Pasta Fusilli 500g matches Idealmakaroner Gammaldags as pasta",
     recipe_match_num_cached(["idealmakaroner gammaldags"], {"name": "Pasta Fusilli 500g", "category": "pantry"}), 1)
# runtime_pnb_svartvinbar_kompott: generated by dm matcher add pnb
# sanity-id: runtime_pnb_svartvinbar_kompott
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB svartvinbär has kompott",
     "kompott" in PRODUCT_NAME_BLOCKERS.get("svartvinbär", set()), True)
# Behavioral proof for the svartvinbär kompott blocker: svartvinbärsblad är ett
# örtkrydd-blad som smaksätter inläggningslag, inte den färska bär-frukten. En
# processad bärkompott (Råggyberry Äpple Svartvinbär Kompott) är fel produkttyp
# och ska inte matcha bladet via compound-suffixet svartvinbär.
test("svartvinbärsblad herb does not match svartvinbär kompott product",
     recipe_match_num_cached(["5 svartvinbärsblad eller 1 kvist timjan + en bit citronskal"],
                             {"name": "Råggyberry Äpple Svartvinbär Kompott Garant", "category": "fruit"}), 0)
# But a fresh/frozen svartvinbär recipe must still match real berries.
test("plain svartvinbär still matches fresh berry product",
     recipe_match_num_cached(["2 dl svartvinbär"],
                             {"name": "Svartvinbär Frysta", "category": "frozen"}), 1)

# --- Extraction fixes: frozen raw seafood carrier and Dafgårds bread ---
# "Räkor Frysta" / "Ishavsräkor Frysta" must extract 'räkor' even when frozen.
# Frozen exclusion in safety-net-2 was blocking raw frozen shrimp products.
test("räkor extraction: Räkor Frysta extracts räkor keyword",
     recipe_match_num(["räkor"], {"name": "Räkor Frysta", "category": "seafood"}), 1)
test("räkor extraction: Ishavsräkor Frysta Garant extracts räkor keyword",
     recipe_match_num(["räkor"], {"name": "Ishavsräkor Frysta Garant", "category": "seafood"}), 1)
test("räkor extraction: Ishavsräkor med Skal Frysta extracts räkor keyword",
     recipe_match_num(["räkor"], {"name": "Ishavsräkor med Skal 70/90 Frysta Garant", "category": "seafood"}), 1)
# "Lantbröd Dafgårds" must extract 'lantbröd' — Dafgårds brand block was too broad.
# PROCESSED_FOODS_EXEMPTIONS 'lantbröd' bypass + 'dafgårds' added to STOP_WORDS.
test("lantbröd extraction: Lantbröd Dafgårds extracts lantbröd keyword",
     recipe_match_num(["lantbröd"], {"name": "Lantbröd Dafgårds", "category": "bread"}), 1)
test("lantbröd extraction: Lantbröd Grekiskt Dafgårds extracts lantbröd keyword",
     recipe_match_num(["lantbröd"], {"name": "Lantbröd Grekiskt Dafgårds", "category": "bread"}), 1)
# Dafgårds ready meals still blocked despite the exemption.
test("lantbröd extraction: Pannbiff Dafgårds still blocked (ready meal)",
     recipe_match_num(["pannbiff"], {"name": "Pannbiff Dafgårds", "category": "frozen"}), 0)

# --- Backend fix: 'ris eller kokt potatis' carrier_flavor_context over-reject ---
# parse_eller_alternatives('ris eller kokt potatis') returned ['ris potatis', 'kokt potatis']
# (wrongly distributing 'potatis' as an adjective of 'ris'). The simple-split fallback
# in the backend now correctly sees arm 'ris' which clears the carrier block.
test("ris backend: 'ris eller kokt potatis' must match jasminris",
     recipe_match_num(["ris eller kokt potatis"], {"name": "Jasminris Långkornigt Sevan", "category": "pantry"}), 1)
test("ris backend: 'ris eller kokt potatis' must match basmatiris",
     recipe_match_num(["ris eller kokt potatis"], {"name": "Basmatiris Ekologisk Sevan", "category": "pantry"}), 1)
# Ensure the eller-arm fix did not break normal ris matching
test("ris backend: plain 'ris' still matches jasminris",
     recipe_match_num(["ris"], {"name": "Jasminris Långkornigt Sevan", "category": "pantry"}), 1)
# runtime_pnb_cider_smultron: generated by dm matcher add pnb
# sanity-id: runtime_pnb_cider_smultron
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB cider has smultron",
     "smultron" in PRODUCT_NAME_BLOCKERS.get("cider", set()), True)
test("PNB cider has hallon",
     "hallon" in PRODUCT_NAME_BLOCKERS.get("cider", set()), True)
test("PNB cider has blåbär",
     "blåbär" in PRODUCT_NAME_BLOCKERS.get("cider", set()), True)
test("PNB cider has jordgubb",
     "jordgubb" in PRODUCT_NAME_BLOCKERS.get("cider", set()), True)
test("PNB cider has päron",
     "päron" in PRODUCT_NAME_BLOCKERS.get("cider", set()), True)
# keyword_synonym_digestivekex_digestives: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_digestivekex_digestives
test("Keyword synonym digestives matches digestivekex",
     match("Digestives The Original Mcvities", "digestives", "pantry"), "digestivekex")
# offer_extra_keyword_krispsallat_kruksallat: generated by dm matcher add offer-extra-keyword
# sanity-id: offer_extra_keyword_krispsallat_kruksallat
test("offer-extra-keyword Kruksallat matches krispsallat",
     match("Kruksallat", "krispsallat", "pantry"), "krispsallat")

# Enoki mushroom isolation: named specialty mushroom, same policy as portabello/karl-johan/shiitake.
# Uses COMPOUND_STRICT['svamp'] so ingredient "enokisvamp" cannot match via generic svamp route.
# (probe confirms backend no-match; match() returns None since no fast-path route term shared)
test("enokisvamp blocked from champinjon",
     match("Champinjoner Klass 1 Garant", "1 knippe enokisvamp", "vegetables"), None)
test("enoki blocked from champinjon",
     match("Champinjoner Klass 1 Garant", "enoki", "vegetables"), None)
test("plain champinjon still matches champinjon",
     match("Champinjoner Klass 1 Garant", "250g champinjoner", "vegetables"), "champinjoner")
# ingredient_parent_buljong_lantbuljong: generated by dm matcher add ingredient-parent
# sanity-id: ingredient_parent_buljong_lantbuljong
test("ingredient-parent Grönsaks Buljong Tärningar Garant Eko matches buljong",
     match("Grönsaks Buljong Tärningar Garant Eko", "Lantbuljong", "pantry"), "buljong")
# ingredient_parent_parmigiano_padano: generated by dm matcher add ingredient-parent
# sanity-id: ingredient_parent_parmigiano_padano
test("ingredient-parent Grana Padano 12 Månader Garant matches parmigiano",
     match("Grana Padano 12 Månader Garant", "Padano", "pantry"), "padano")
# runtime_processed_food_add_raggyberry: generated by dm matcher add processed-food
# sanity-id: runtime_processed_food_add_raggyberry
from languages.sv.ingredient_matching.keywords import PROCESSED_FOODS
test("processed-food add råggyberry",
     "råggyberry" in PROCESSED_FOODS, True)
# Behavioral proof: Råggyberry fruit squeeze-pouch snack must not match any recipe
# (it is a mellanmål, not a cooking kompott). The fruit names in the product must
# not let it surface for kompott/äpple/svartvinbär recipes.
test("Råggyberry snack matches no kompott recipe",
     recipe_match_num(["2 dl rabarberkompott"], {"name": "Råggyberry Äpple Svartvinbär Kompott Garant", "category": "fruit"}), 0)
test("Råggyberry snack matches no fresh-fruit recipe",
     recipe_match_num(["3 äpplen"], {"name": "Råggyberry Äpple Svartvinbär Kompott Garant", "category": "fruit"}), 0)
# Real per-fruit kompott isolation is unaffected.
test("real rabarberkompott still matches rabarberkompott offer",
     recipe_match_num(["2 dl rabarberkompott"], {"name": "Rabarberkompott 400g Garant", "category": "pantry"}), 1)
test("real kompott stays fruit-isolated (jordgubb not äpple)",
     recipe_match_num(["2 dl jordgubbskompott"], {"name": "Äpplekompott 400g", "category": "pantry"}), 0)
# keyword_synonym_fetaost_feta: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_fetaost_feta
test("Keyword synonym feta matches fetaost",
     match("Fetaost Kassos", "feta", "dairy"), "fetaost")
# runtime_pnb_salami_wudy: generated by dm matcher add pnb
# sanity-id: runtime_pnb_salami_wudy
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB salami has wudy",
     "wudy" in PRODUCT_NAME_BLOCKERS.get("salami", set()), True)
# runtime_pnb_koreansk_wrap: generated by dm matcher add pnb
# sanity-id: runtime_pnb_koreansk_wrap
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB koreansk has wrap",
     "wrap" in PRODUCT_NAME_BLOCKERS.get("koreansk", set()), True)
# runtime_pnb_mjolkchoklad_lindorkulor: generated by dm matcher add pnb
# sanity-id: runtime_pnb_mjolkchoklad_lindorkulor
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB mjölkchoklad has lindorkulor",
     "lindorkulor" in PRODUCT_NAME_BLOCKERS.get("mjölkchoklad", set()), True)
# runtime_pnb_pasta_chitarra: generated by dm matcher add pnb
# sanity-id: runtime_pnb_pasta_chitarra
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB pasta has chitarra",
     "chitarra" in PRODUCT_NAME_BLOCKERS.get("pasta", set()), True)
# offer_extra_keyword_langpasta_chitarra: generated by dm matcher add offer-extra-keyword
# sanity-id: offer_extra_keyword_langpasta_chitarra
test("offer-extra-keyword La Chitarra Pasta Garant matches långpasta",
     match("La Chitarra Pasta Garant", "spaghetti", "pantry"), "långpasta")
# runtime_fpb_parmigiano_krutonger: generated by dm matcher add fpb
# sanity-id: runtime_fpb_parmigiano_krutonger
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB parmigiano has krutonger",
     "krutonger" in FALSE_POSITIVE_BLOCKERS.get("parmigiano", set()), True)
# runtime_ksbc_parmigiano_krutonger: generated by dm matcher add ksbc
# sanity-id: runtime_ksbc_parmigiano_krutonger
from languages.sv.ingredient_matching import KEYWORD_SUPPRESSED_BY_CONTEXT
test("KSBC parmigiano has krutonger",
     "krutonger" in KEYWORD_SUPPRESSED_BY_CONTEXT.get("parmigiano", set()), True)
# runtime_stop_word_add_strimlor: generated by dm matcher add stop-word
# sanity-id: runtime_stop_word_add_strimlor
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add strimlor",
     "strimlor" in STOP_WORDS, True)
test("stop-word filters extraction strimlor",
     "strimlor" in kw("strimlor"), False)
# runtime_pnb_kycklingspett_teriyaki: generated by dm matcher add pnb
# sanity-id: runtime_pnb_kycklingspett_teriyaki
from languages.sv.ingredient_matching import PRODUCT_NAME_BLOCKERS
test("PNB kycklingspett has teriyaki",
     "teriyaki" in PRODUCT_NAME_BLOCKERS.get("kycklingspett", set()), True)
# runtime_stop_word_add_rumsvarm: generated by dm matcher add stop-word
# sanity-id: runtime_stop_word_add_rumsvarm
from languages.sv.ingredient_matching.keywords import STOP_WORDS
test("stop-word add rumsvarm",
     "rumsvarm" in STOP_WORDS, True)
test("stop-word filters extraction rumsvarm",
     "rumsvarm" in kw("rumsvarm"), False)
test("stop-word add utbakning",
     "utbakning" in STOP_WORDS, True)
test("stop-word filters extraction utbakning",
     "utbakning" in kw("utbakning"), False)
# keyword_synonym_kardemumma_kardemummakarnor: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_kardemumma_kardemummakarnor
# Extraction test: kardemummakärnor → kardemumma (no live offer in DB so match() not usable)
test("kardemummakärnor extracts as kardemumma",
     "kardemumma" in kw("kardemummakärnor"), True)

# "valfri X ex./t.ex. Y": the broad "valfri" head must match every cream variant;
# the trailing example must not suppress the broad keyword via context. Standalone
# "havregrädde" (no "valfri" head) must stay isolated from milk cream. Category is
# left empty on purpose: store categories are unreliable, so matching must hold on
# the product NAME regardless of category.
test("KW valfri grädde ex. havregrädde → grädde only",
     "grädde" in extract_keywords_from_ingredient("3 dl valfri grädde ex. havregrädde")
     and "havregrädde" not in extract_keywords_from_ingredient("3 dl valfri grädde ex. havregrädde"), True)
test("FAST valfri grädde ex. havregrädde matches vispgrädde",
     match("Vispgrädde 40% Garant", "3 dl valfri grädde ex. havregrädde", ""), "grädde")
test("FAST valfri grädde ex. havregrädde matches matlagningsgrädde",
     match("Matlagningsgrädde 15% Arla", "3 dl valfri grädde ex. havregrädde", ""), "grädde")
test("MAT valfri grädde ex. havregrädde matches vispgrädde",
     recipe_match_num(["3 dl valfri grädde ex. havregrädde"],
                      {"name": "Vispgrädde 40% Garant", "category": ""}), 1)
test("MAT standalone havregrädde stays isolated from milk cream",
     recipe_match_num(["2 dl havregrädde"],
                      {"name": "Vispgrädde 40% Garant", "category": ""}), 0)
# runtime_important_short_keyword_add_kaviar: generated by dm matcher add important-short-keyword
# sanity-id: runtime_important_short_keyword_add_kaviar
from languages.sv.ingredient_matching.keywords import IMPORTANT_SHORT_KEYWORDS
test("important-short-keyword add kaviar",
     "kaviar" in IMPORTANT_SHORT_KEYWORDS, True)
test("important-short-keyword add caviar",
     "caviar" in IMPORTANT_SHORT_KEYWORDS, True)
# runtime_space_normalization_rod_kaviar_rodrom: generated by dm matcher add space-normalization
# sanity-id: runtime_space_normalization_rod_kaviar_rodrom
test("space-normalization röd kaviar -> rödrom",
     _apply_space_normalizations("röd kaviar"), "rödrom")
# runtime_space_normalization_rod_caviar_rodrom: generated by dm matcher add space-normalization
# sanity-id: runtime_space_normalization_rod_caviar_rodrom
test("space-normalization röd caviar -> rödrom",
     _apply_space_normalizations("röd caviar"), "rödrom")
# runtime_space_normalization_svart_kaviar_svartrom: generated by dm matcher add space-normalization
# sanity-id: runtime_space_normalization_svart_kaviar_svartrom
test("space-normalization svart kaviar -> svartrom",
     _apply_space_normalizations("svart kaviar"), "svartrom")
# runtime_space_normalization_svart_caviar_svartrom: generated by dm matcher add space-normalization
# sanity-id: runtime_space_normalization_svart_caviar_svartrom
test("space-normalization svart caviar -> svartrom",
     _apply_space_normalizations("svart caviar"), "svartrom")
# Roe model behavior (not just the text rewrites above): plain "kaviar" (Swedish)
# is Kalles-type cod roe paste; plain "caviar" (proper spelling) is real/luxury
# caviar, isolated from both Kalles and stenbitsrom; colored "röd/svart kaviar"
# and "röd/svart caviar" are fish roe in the rom family. Category is left empty on
# purpose: store categories are unreliable, so matching must hold on the product
# NAME regardless of category.
test("MAT plain kaviar matches Kalles-type cod roe paste",
     recipe_match_num(["1 tub kaviar"],
                      {"name": "Kalles Kaviar Original Abba", "category": ""}), 1)
test("MAT plain caviar stays isolated from Kalles-type",
     recipe_match_num(["caviar"],
                      {"name": "Kalles Kaviar Original Abba", "category": ""}), 0)
test("MAT plain caviar stays isolated from stenbitsrom",
     recipe_match_num(["caviar"],
                      {"name": "Finkornig Rom Röd Garant", "category": ""}), 0)
test("MAT röd caviar matches red fish roe rom family",
     recipe_match_num(["1 förp röd caviar"],
                      {"name": "Finkornig Rom Röd Garant", "category": ""}), 1)
test("MAT röd kaviar matches red fish roe rom family",
     recipe_match_num(["röd kaviar"],
                      {"name": "Finkornig Rom Röd Garant", "category": ""}), 1)
# runtime_space_normalization_smartare_fars_vegofars: generated by dm matcher add space-normalization
# sanity-id: runtime_space_normalization_smartare_fars_vegofars
test("space-normalization smartare färs -> vegofärs",
     _apply_space_normalizations("smartare färs"), "vegofärs")
# Behavior: "Stacky's Smartare Färs" is plant-based mince. Routed to vegofärs it
# must block meat offers, yet still match the plant product on both sides. A meat
# sibling arm ("eller nötfärs") still allows meat (arms evaluated independently).
# Category left empty: matching holds on the product name regardless of category.
test("MAT smartare färs eller annan vegofärs blocks meat",
     recipe_match_num(["2 förp Stacky's smartare färs eller annan vegofärs"],
                      {"name": "Nötfärs Irland 20%", "category": ""}), 0)
test("MAT smartare färs eller annan vegofärs still matches plant mince",
     recipe_match_num(["2 förp Stacky's smartare färs eller annan vegofärs"],
                      {"name": "Stackys Smartare Färs", "category": ""}), 1)
test("MAT smartare färs eller nötfärs still allows meat via meat arm",
     recipe_match_num(["Stacky's smartare färs eller nötfärs"],
                      {"name": "Nötfärs Irland 20%", "category": ""}), 1)
# ingredient_parent_sidflask_stekflask: generated by dm matcher add ingredient-parent
# sanity-id: ingredient_parent_sidflask_stekflask
# sidfläsk and stekfläsk are the same pork cut (stekfläsk is the rimmed/sliced
# form) and are interchangeable in both directions. Category left empty: matching
# holds on the product name regardless of category.
test("ingredient-parent Stekfläsk Rimmat Skivat Prime Patrol matches sidfläsk",
     match("Stekfläsk Rimmat Skivat Prime Patrol", "sidfläsk", ""), "sidfläsk")
test("MAT sidfläsk matches stekfläsk product rimmat tolerated",
     recipe_match_num(["sidfläsk"],
                      {"name": "Stekfläsk Rimmat Skivat Prime Patrol", "category": ""}), 1)
test("MAT sidfläsk stays isolated from other pork cuts",
     recipe_match_num(["sidfläsk"],
                      {"name": "Fläskkarré Benfri", "category": ""}), 0)
# ingredient_parent_bordsmargarin_smorrapsolja: generated by dm matcher add ingredient-parent
# sanity-id: ingredient_parent_bordsmargarin_smorrapsolja
test("ingredient-parent Gårdsgoda Normalsaltat Smör & Raps Valio matches bordsmargarin",
     match("Gårdsgoda Normalsaltat Smör & Raps Valio", "bordsmargarin", ""), "bordsmargarin")
# runtime_fpb_kex_rokextrakt: generated by dm matcher add fpb
# sanity-id: runtime_fpb_kex_rokextrakt
from languages.sv.ingredient_matching import FALSE_POSITIVE_BLOCKERS
test("FPB kex has rökextrakt",
     "rökextrakt" in FALSE_POSITIVE_BLOCKERS.get("kex", set()), True)
test("FPB kex has rökarom",
     "rökarom" in FALSE_POSITIVE_BLOCKERS.get("kex", set()), True)
# ingredient_parent_kottbullar_kycklingkottbullar: generated by dm matcher add ingredient-parent
# sanity-id: ingredient_parent_kottbullar_kycklingkottbullar
test("ingredient-parent Köttbullar Kyckling Eldorado matches köttbullar",
     match("Köttbullar Kyckling Eldorado", "Kycklingköttbullar", ""), "kycklingköttbullar")
# keyword_synonym_salami_salame: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_salami_salame
test("Keyword synonym salame matches salami",
     match("Salami Skivad Garant", "salame", ""), "salami")
# runtime_important_short_keyword_add_salami: generated by dm matcher add important-short-keyword
# sanity-id: runtime_important_short_keyword_add_salami
from languages.sv.ingredient_matching.keywords import IMPORTANT_SHORT_KEYWORDS
test("important-short-keyword add salami",
     "salami" in IMPORTANT_SHORT_KEYWORDS, True)
# runtime_important_short_keyword_add_salame: generated by dm matcher add important-short-keyword
# sanity-id: runtime_important_short_keyword_add_salame
from languages.sv.ingredient_matching.keywords import IMPORTANT_SHORT_KEYWORDS
test("important-short-keyword add salame",
     "salame" in IMPORTANT_SHORT_KEYWORDS, True)
# runtime_specialty_qualifier_salami: generated by dm matcher add specialty-qualifier
# sanity-id: runtime_specialty_qualifier_salami
from languages.sv.ingredient_matching.specialty_rules import SPECIALTY_QUALIFIERS
test("specialty-qualifier salami has tryffel",
     "tryffel" in SPECIALTY_QUALIFIERS.get("salami", []), True)
from languages.sv.ingredient_matching.specialty_rules import BIDIRECTIONAL_PER_KEYWORD
test("specialty bidirectional salami has tryffel",
     "tryffel" in BIDIRECTIONAL_PER_KEYWORD.get("salami", set()), True)
test("specialty-qualifier Tryffelsalami Haga-petters no-match salami (backend)",
     recipe_match_num_named("Sanity Recipe", ["salami"], {"name": "Tryffelsalami Haga-petters"}), 0)

# --- Manual backend canaries for orchestrator-level matcher changes ---
# Optional vs hard vegan cheese cue: "gärna vegansk" is a soft suggestion so the
# ordinary dairy cheese stays valid; a hard "vegansk ost" requires an explicit
# vegan product.
test("optional 'gärna vegansk' cheese still matches dairy hushållsost (backend)",
     recipe_match_num(["1 dl riven ost, gärna vegansk"], {"name": "Hushållsost 26% Arla"}), 1)
test("hard 'vegansk ost' blocks dairy hushållsost (backend)",
     recipe_match_num(["1 dl riven vegansk ost"], {"name": "Hushållsost 26% Arla"}), 0)
test("hard 'vegansk ost' matches an explicit vegan cheese (backend)",
     recipe_match_num(["1 dl riven vegansk ost"], {"name": "Violife Riven Vegansk"}), 1)

# Broad meatball family: generic köttbullar accepts poultry meatballs (like the
# broad mince family) but fish balls stay isolated.
test("generic köttbullar matches chicken meatballs (backend)",
     recipe_match_num(["300 g köttbullar färska"], {"name": "Köttbullar Kyckling Eldorado"}), 1)
test("fiskbullar stay isolated from köttbullar (backend)",
     recipe_match_num(["300 g köttbullar"], {"name": "Fiskbullar i Hummersås Abba"}), 0)

# Salame Milano routes to plain salami but is isolated from Spanish fuet and
# truffle-flavored salami.
test("Salame Milano matches plain salami (backend)",
     recipe_match_num(["1 frp Zeta Salame Milano"], {"name": "Salami Skivad Garant"}), 1)
test("Salame Milano stays isolated from fuet (backend)",
     recipe_match_num(["1 frp Zeta Salame Milano"], {"name": "Fuet Tradicional Lufttorkad Salami Garant"}), 0)

# Whole-cut form rule: a recipe demanding a whole piece must not match a
# pre-sliced cut, while a plain recipe still does.
test("hel fläskkarré does not match pre-sliced karré (backend)",
     recipe_match_num(["2 kg hel fläskkarré"], {"name": "Karré Original Skivad Sverige Dalsjöfors"}), 0)
test("helstekt fläskkarré does not match pre-sliced karré (backend)",
     recipe_match_num(["helstekt fläskkarré"], {"name": "Karré Original Skivad Sverige Dalsjöfors"}), 0)
test("plain fläskkarré still matches sliced karré (backend)",
     recipe_match_num(["fläskkarré"], {"name": "Karré Original Skivad Sverige Dalsjöfors"}), 1)

# Marinated chicken asymmetry: a generic marinated product may satisfy a plain
# kycklingfilé recipe (extra flavor is fine), a specifically-flavored marinade
# must not, and a recipe that explicitly asks for marinated needs a marinated
# product rather than plain.
test("generic marinated chicken matches plain kycklingfilé recipe (backend)",
     recipe_match_num(["kycklingfilé"], {"name": "Marinerad Tvådelad Kycklingfilé Garant"}), 1)
test("citron-chili marinated chicken stays blocked from plain kycklingfilé (backend)",
     recipe_match_num(["kycklingfilé"], {"name": "Kycklingfilé Citron Chili Marinerad Kronfågel"}), 0)
test("marinated kycklingfilé recipe needs a marinated product, not plain (backend)",
     recipe_match_num(["marinerad kycklingfilé"], {"name": "Kycklingfilé Bröstfilé Sverige Garant"}), 0)

# Vegan recipe title propagates to dairy ingredients: a vegan-titled recipe
# blocks a dairy product even when the ingredient text carries no vegan cue,
# while a non-vegan recipe with the same ingredient still matches the dairy.
test("vegan-titled recipe blocks dairy grädde for matlagningsgrädde (backend)",
     recipe_match_num_named("Vegansk tryffeltårta", ["300 g matlagningsgrädde"], {"name": "Vispgrädde 40% Garant"}), 0)
test("non-vegan recipe still matches dairy grädde for matlagningsgrädde (backend)",
     recipe_match_num_named("Klassisk tryffeltårta", ["300 g matlagningsgrädde"], {"name": "Vispgrädde 40% Garant"}), 1)
# runtime_important_short_keyword_add_pommes: generated by dm matcher add important-short-keyword
# sanity-id: runtime_important_short_keyword_add_pommes
from languages.sv.ingredient_matching.keywords import IMPORTANT_SHORT_KEYWORDS
test("important-short-keyword add pommes",
     "pommes" in IMPORTANT_SHORT_KEYWORDS, True)
test("important-short-keyword add frites",
     "frites" in IMPORTANT_SHORT_KEYWORDS, True)
test("important-short-keyword add bamboo",
     "bamboo" in IMPORTANT_SHORT_KEYWORDS, True)
test("important-short-keyword add soya",
     "soya" in IMPORTANT_SHORT_KEYWORDS, True)
test("important-short-keyword add vårlök",
     "vårlök" in IMPORTANT_SHORT_KEYWORDS, True)
# runtime_space_normalization_burger_of_heaven_vegoburgare: generated by dm matcher add space-normalization
# sanity-id: runtime_space_normalization_burger_of_heaven_vegoburgare
test("space-normalization burger of heaven -> vegoburgare",
     _apply_space_normalizations("burger of heaven"), "vegoburgare")
# keyword_synonym_digestivekex_digestive: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_digestivekex_digestive
test("Keyword synonym digestive matches digestivekex",
     match("Digestives The Original Mcvities", "digestive", "pantry"), "digestivekex")
# runtime_space_normalization_glass_noodles_glasnudlar: generated by dm matcher add space-normalization
# sanity-id: runtime_space_normalization_glass_noodles_glasnudlar
test("space-normalization glass noodles -> glasnudlar",
     _apply_space_normalizations("glass noodles"), "glasnudlar")
# keyword_synonym_surdegsbrod_suradeg: generated by dm matcher add keyword-synonym
# sanity-id: keyword_synonym_surdegsbrod_suradeg
test("Keyword synonym surådeg matches surdegsbröd",
     match("Källarfranska Surådeg", "surådeg", ""), "surdegsbröd")
# runtime_important_short_keyword_add_notmix: generated by dm matcher add important-short-keyword
# sanity-id: runtime_important_short_keyword_add_notmix
from languages.sv.ingredient_matching.keywords import IMPORTANT_SHORT_KEYWORDS
test("important-short-keyword add nötmix",
     "nötmix" in IMPORTANT_SHORT_KEYWORDS, True)
# ingredient_parent_gronsaksbuljong_gronsaksfond: generated by dm matcher add ingredient-parent
# sanity-id: ingredient_parent_gronsaksbuljong_gronsaksfond
test("ingredient-parent Grönsaks Buljong Tärningar Garant Eko matches grönsaksbuljong",
     match("Grönsaks Buljong Tärningar Garant Eko", "Grönsaksfond", ""), "grönsaksfond")
# ingredient_parent_salsiccia_salsicciafars: generated by dm matcher add ingredient-parent
# sanity-id: ingredient_parent_salsiccia_salsicciafars
test("ingredient-parent Salsicciafärs Naturell Köttkultur matches salsiccia",
     match("Salsicciafärs Naturell Köttkultur", "Salsicciafärs", ""), "salsiccia")
# Q15-4: "rimmat sidfläsk" recipe must match the equivalent rimmat product even when it
# is sold under the stekfläsk alias name; the offer-side qualifier capture now surfaces
# the cure qualifier under sidfläsk. The same cure isolation still rejects an un-cured product.
test("rimmat sidfläsk matches rimmat stekfläsk product",
     recipe_match_num(["rimmat sidfläsk"], {"name": "Stekfläsk Rimmat Skivat Prime Patrol", "category": ""}), 1)
test("rimmat sidfläsk does not match un-cured stekfläsk product",
     recipe_match_num(["rimmat sidfläsk"], {"name": "Stekfläsk Skivat Prime Patrol", "category": ""}), 0)
# Q14-3: whole salsiccia and salsicciafärs are interchangeable (crumbled in pasta/risotto);
# the form-isolation 'färs' blocker was removed. The reverse mince blocker still holds, so a
# generic köttfärs recipe does not pull in seasoned salsicciafärs.
test("whole salsiccia matches salsicciafärs product",
...
# ingredient_parent_champinjoner_champinjon: generated by dm matcher add ingredient-parent
# sanity-id: ingredient_parent_champinjoner_champinjon
test("ingredient-parent Champinjoner Klass 1 Garant matches champinjoner",
     recipe_match_num(
         ["250 g skogschampinjon"],
         {"name": "Champinjoner Klass 1 Garant", "category": "", "savings": 10},
     ), 1)
