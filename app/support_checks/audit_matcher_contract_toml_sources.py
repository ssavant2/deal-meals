#!/usr/bin/env python3
"""Dry-run TOML source schema and round-trip audit for matcher contracts."""
...
CONTRACT_SPECS = (
    ContractSpec(
        ...
        table_name="fixtures",
        source_json_path=fixture_contract_path(),
        source_toml_filename="matcher_regression_cases.toml",
    ),
    ContractSpec(
        contract="matcher_rule_inventory",
        table_name="inventory",
        source_json_path=inventory_contract_path(),
        source_toml_filename="matcher_rule_inventory.toml",
    ),
)
...
def _source_toml(spec: ContractSpec, payload: list[dict[str, Any]]) -> str:
    lines = [
        "# AUTO-GENERATED DRY-RUN SOURCE - DO NOT EDIT BY HAND.",
        "# Generated by support_checks/audit_matcher_contract_toml_sources.py.",
        "# This is a B3 schema audit artifact, not an authoritative source yet.",
        "schema_version = 1",
        f"contract = {_toml_scalar(spec.contract)}",
        f"source_json_path = {_toml_scalar(_repo_rel(spec.source_json_path))}",
        f"source_json_sha256 = {_toml_scalar(_source_sha256(spec.source_json_path))}",
    ]
    for row in payload:
        lines.append("")
...
def _payload_from_source_toml(spec: ContractSpec, toml_text: str) -> list[dict[str, Any]]:
    ...
    if not isinstance(payload, list):
        raise ValueError(f"{spec.source_toml_filename} must contain [[{spec.table_name}]]")
    if not all(isinstance(row, dict) for row in payload):
        raise ValueError(f"{spec.source_toml_filename} rows must be TOML tables")
    return payload
...
def audit_contract_sources(output_dir: Path = DEFAULT_OUTPUT_DIR) -> list[ContractAuditResult]:
    if _is_relative_to(output_dir, REPO_DIR):
        raise ValueError(
            "B3 source TOML dry-run output must stay outside the checkout; "
            f"got {output_dir}"
        )
    ...
    for spec in CONTRACT_SPECS:
        ...
        source_toml_path = output_dir / spec.source_toml_filename
        ...
        results.append(ContractAuditResult(
            ...
            source_json_path=_repo_rel(spec.source_json_path),
...
def markdown_report(results: list[ContractAuditResult]) -> str:
    ...
    for result in results:
        lines.append(f"- `{result.source_toml_path}` from `{result.source_json_path}`")
...
def main() -> int:
    ...
    try:
        results = audit_contract_sources(args.output_dir)
