...
def _source_toml(spec: ContractSpec, payload: list[dict[str, Any]]) -> str:
    lines = [
        "# PARALLEL TOML SOURCE - JSON REMAINS AUTHORITATIVE UNTIL B5.",
        "# Generated by support_checks/audit_matcher_contract_toml_sources.py.",
        "# Refresh after JSON edits; do not hand-edit until the authority flip.",
        "schema_version = 1",
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Allow writing TOML sources inside the checkout for B4 parallel-source sync.",
    )
