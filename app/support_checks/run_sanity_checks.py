...
def _run_term_registry_gate_checks() -> None:
    print("\n--- term registry coverage gate ---", flush=True)
    ...
    test("term registry new source key count", new_term_gate.get("new_source_coverage_keys"), 0)
    test("term registry coverage failure probe", new_term_gate.get("failure_probe_passed"), True)


def _run_term_registry_add_term_checks() -> None:
    print("\n--- term registry add-term export plan ---", flush=True)
    from support_checks.run_term_registry_add_term_checks import run_checks  # noqa: E402
    ...
    test("term registry add-term coverage count", summary.get("unique_coverage_key_count"), 5356)
