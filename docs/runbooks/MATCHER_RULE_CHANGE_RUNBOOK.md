...
./bin/dm matcher remove <generated-registry-entry-id> --reason "<why>"
...
`remove <generated-registry-entry-id>` is a narrow undo for a fresh,
CLI-generated registry entry that turned out to be the wrong rule during the
same batch. It hard-removes only entries whose notes say they were generated by
`dm matcher add`, removes the matching generated sanity canary, and refuses
entries with fixture_refs or inventory ownership. Established registry policy
...
Generated canaries and fixtures default to blank offer category. Store
categories are best-effort metadata and differ between ICA/Coop/Willys/Hemköp,
so do not encode `fish`, `meat`, `dairy`, `pantry`, etc. into a matcher rule or
sanity row by habit. Pass `--offer-category` only when category-sensitive
behavior is the explicit thing being tested or debugged.
...
```bash
./bin/dm matcher why --offer "<offer>" --ingredient "<ingredient>" --recipe-name "<recipe title>"
```
...
1. Re-run with the real brand/weight and, when relevant, the real recipe title:
   `dm matcher compare-paths --offer ... --ingredient ... --brand ... --recipe-name ... --format json`.
   Do not add offer category by default. Store categories are inconsistent; if a
   result changes only because `--offer-category` is set, treat that as a
   category-dependence risk rather than a durable rule signal.
...
Pass `--recipe-name "<real title>"` when the decision may depend on recipe-title
context, such as vegan/vegetarian guards, cuisine context, or prepared-dish
context. Without it, probe/why/compare-paths use a neutral synthetic recipe
title and will say so in their output.
