...
`app/languages/<code>/matcher_contracts/` and follow
[`docs/runbooks/MATCHER_RULE_CHANGE_RUNBOOK.md`](runbooks/MATCHER_RULE_CHANGE_RUNBOOK.md).
For supported Swedish rule shapes, start with `./bin/dm matcher --help`;
otherwise use the manual Track A/Track B runbook flow. For Swedish Track B work,
fixture/inventory JSON is the authored contract; registry coverage TOML is
generated by the matcher gate wrapper. Local `app/tests/test_*.py` files and
batch-review notes are useful for debugging, but they are not the permanent
regression contract.
