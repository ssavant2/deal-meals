...
For Swedish Track B work, fixture/inventory JSON is the authored contract;
registry coverage TOML is generated by the matcher gate wrapper. Local
`app/tests/test_*.py` files and batch-review notes are useful for debugging,
but they are not the permanent regression contract.
