...
- **Pre-seasoned and marinated raw products** — products such as "Herb-seasoned Chicken Fillet", "BBQ-marinated Pork Shoulder" or "Lemon-marinated Salmon Fillet" are shown as suggestions even if the recipe does not specifically ask for that seasoning. This is intentional: these are still raw cooking ingredients and you decide whether you want to use the pre-seasoned version. Blocking them entirely would remove them as an option from every recipe, including ones where they would be a perfect fit. **Exception:** Ready-cooked chicken products (grilled, roasted, deli-sliced) are not shown for raw ingredient requirements, regardless of seasoning.
- **Cuisine-specific seasoning** — products with a clearly cuisine-specific flavour profile are only shown in recipes of a matching type. "Thai-seasoned chicken fillet" will not appear as a suggestion in a French thyme recipe, but it will show in wok and Thai recipes. "Taco-seasoned mince" is only suggested in taco and Tex-Mex recipes. Gyros-marinated meat appears in Greek-style recipes. This keeps strongly flavoured products visible and purchasable — but only for the right kind of dish.
